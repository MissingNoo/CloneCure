...
function input_source_detect_new() {
	...
	repeat (_gamepad_count) {
		if (input_source_detect_input(INPUT_GAMEPAD[_g])) {
			return INPUT_GAMEPAD[_g];
		}
		_g += _sort_order;
	}

	if (input_source_detect_input(INPUT_KEYBOARD)) {
		return INPUT_KEYBOARD;
	}
	if (input_source_detect_input(INPUT_MOUSE)) {
		return INPUT_MOUSE;
	}
	if (input_source_detect_input(INPUT_TOUCH)) {
		return INPUT_TOUCH;
	}
