...
function __input_class_cursor() constructor {
	...
	static __translate = function(_x, _y, _duration, _relative) {
		...
		if (_relative) {
			__translation_end_x = (__translation_start_x == undefined)
				? undefined
				: __translation_start_x + _x;
			__translation_end_y = (__translation_start_y == undefined)
				? undefined
				: __translation_start_y + _y;
		} else {
	...
	static __tick = function() {
		...
		var _can_use_mouse =
			__global.__game_input_allowed
			&& __player.__mouse_enabled
			&& __player.__source_contains(INPUT_MOUSE); //Automatically remapped to INPUT_TOUCH where appropriate

		//Mouse and touch
		if (
			(__global.__pointer_moved || __using_mouse)
			&& _can_use_mouse
		...
		if (
						...
						__x +=
							round(
								(
									_gyro_value_x
										* _dts
										* __player.__gyro_screen_width
										* __player.__gyro_sensitivity_x
										* 10
								)
							) / 10;
					}
							...
							(
								round(
									(
										_gyro_value_y
											* _dts
											* __player.__gyro_screen_height
											* __player.__gyro_sensitivity_y
											* 10
									)
								) / 10
							) * _y_inverted;
			...
			//Cursor bindings
			var _xy = input_xy(
				INPUT_CURSOR_VERB_LEFT,
				INPUT_CURSOR_VERB_RIGHT,
				INPUT_CURSOR_VERB_UP,
				INPUT_CURSOR_VERB_DOWN,
				__player.__index
			);
			if ((_xy.x != 0.0) || (_xy.y != 0.0)) {
				...
				if (INPUT_CURSOR_EXPONENT != 1) {
					var _coeff = power(
						point_distance(0, 0, _xy.x, _xy.y),
						INPUT_CURSOR_EXPONENT
					);
		...
		if (__translation_active) {
			var _t = clamp(
				(__input_get_time() - __translation_start_time)
					/ (__translation_end_time - __translation_start_time),
				0,
				1
			);
		...
		if (
			...
			switch (__coord_space) {
				case INPUT_COORD_SPACE.ROOM:
					var _camera = (view_enabled && view_visible[0])
						? view_camera[0]
						: undefined;
					if (_camera != undefined) {
						...
						if (_viewA != 0.0) {
							...
							_rotatedX = clamp(
								_rotatedX,
								_l + __limit_boundary_margin - _pivotX,
								_r - __limit_boundary_margin - _pivotX
							);
							_rotatedY = clamp(
								_rotatedY,
								_t + __limit_boundary_margin - _pivotY,
								_b - __limit_boundary_margin - _pivotY
							);
			...
			if (!_clamped) {
				__x = clamp(
					__x,
					_l + __limit_boundary_margin,
					_r - __limit_boundary_margin
				);
				__y = clamp(
					__y,
					_t + __limit_boundary_margin,
					_b - __limit_boundary_margin
				);
			}
		}
	};
