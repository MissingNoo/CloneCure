...
function ScribblejrShrinkExt(
	...
	if ((_string == "") || (_maxWidth < 0) || (_maxHeight < 0)) {
		return _nullWrapper;
	}
	if (_font == undefined) {
		_font = _system.__defaultFont;
	}
	_fontScale *= SCRIBBLEJR_GLOBAL_FONT_SCALE;

	var _key = string_concat(
		_string,
		":",
		_hAlign + 3 * _vAlign,
		":", //Pack these flags together
		_font,
		":",
		_fontScale,
		":",
		...
		_maxHeight,
		":",
		_system.__preprocessorName,
		":F"
	);

	var _wrapper = _cache[$ _key];
	if (_wrapper == undefined) {
		var _element = new __ScribblejrClassExtShrink(
			_key,
			_string,
			_hAlign,
			_vAlign,
