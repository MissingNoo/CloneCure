...
function __input_system_tick() {
	...
	if (__INPUT_TOUCH_SUPPORT && INPUT_MOBILE_MOUSE) {
		...
		repeat (INPUT_MAX_TOUCHPOINTS) {
			...
			if (__INPUT_ON_IOS) {
				if (
					!_held
					&& (_global.__pointer_held_time[_i] >= 0)
					&& (_global.__current_time - _global.__pointer_held_time[_i] > 20)
				) {
					if (!_global.__pointer_held_buffer[_i]) {
						_held = true;
					}
					_global.__pointer_held_buffer[_i] = !_global.__pointer_held_buffer[
						_i
					];
				}
			}
			...
			if (!_held) {
				...
				if (
					(_touch_index == undefined)
					|| (
						_global.__pointer_held_time[_i]
							> _global.__pointer_held_time[_touch_index]
					)
				) {
					_touch_index = _i;
				}
		...
		_global.__pointer_pressed = device_mouse_check_button_pressed(
			_touch_index,
			mb_left
		);
		_global.__pointer_released =
			(_global.__pointer_index_previous != undefined)
			&& device_mouse_check_button_released(
				_global.__pointer_index_previous,
				mb_left
			);
		...
		if (INPUT_TOUCH_EDGE_DEADZONE > 0) {
			...
			if (_global.__pointer_released) {
				...
				if (
					(_tx < INPUT_TOUCH_EDGE_DEADZONE)
					|| (_tx > (_w - INPUT_TOUCH_EDGE_DEADZONE))
					|| (_ty < INPUT_TOUCH_EDGE_DEADZONE)
					|| (_ty > (_h - INPUT_TOUCH_EDGE_DEADZONE))
				) {
		...
		if (os_is_paused()) {
			...
			if (_global.__window_focus) {
				if (_global.__window_focus_block_mouse) {
					...
					if (__input_mouse_button() != mb_none) {
	...
	if (_global.__mouse_capture && _global.__game_input_allowed) {
		if (__input_window_changed()) {
			...
			_global.__mouse_capture_blocked
	...
	if (
		INPUT_ALLOW_OUT_OF_FOCUS
		|| !(INPUT_DEFAULT_DEBUG_OVERLAY_BLOCKS_INPUT && _global.__overlay_focus)
	) {
		if (_global.__mouse_capture && !_global.__mouse_capture_blocked) {
			if (_global.__window_focus) {
				...
				if (_global.__frame - _global.__mouse_capture_frame > 10) {
					...
					if (
						(abs(_pointer_x - _window_width / 2) >= 1)
						...
						repeat (INPUT_COORD_SPACE.__SIZE) {
							switch (_m) {
								case INPUT_COORD_SPACE.ROOM:
									...
									var _pointer_x = device_mouse_x(
										_global.__pointer_index
									);
									var _pointer_y = device_mouse_y(
										_global.__pointer_index
									);
									break;
								...
								case INPUT_COORD_SPACE.GUI:
									...
									var _pointer_x = device_mouse_x_to_gui(
										_global.__pointer_index
									);
									var _pointer_y = device_mouse_y_to_gui(
										_global.__pointer_index
									);
									break;
								...
								case INPUT_COORD_SPACE.DEVICE:
									...
									if (__INPUT_ON_WINDOWS) {
										...
										_pointer_x = device_mouse_raw_x(
											_global.__pointer_index
										);
										_pointer_y = device_mouse_raw_y(
											_global.__pointer_index
										);
									}
									break;
							...
							var _dx =
								(_pointer_x - _old_x)
								* _global.__mouse_capture_sensitivity;
							var _dy =
								(_pointer_y - _old_y)
								* _global.__mouse_capture_sensitivity;
			...
			repeat (INPUT_COORD_SPACE.__SIZE) {
				...
				if (
					(_m == INPUT_COORD_SPACE.DEVICE)
					&& (
						point_distance(_old_x, _old_y, _pointer_x, _pointer_y)
							> INPUT_MOUSE_MOVE_DEADZONE
					)
				) {
					_moved = true;
	...
	if (_global.__keyboard_allowed && keyboard_check(vk_anykey)) {
		...
		if (INPUT_ON_WEB && __INPUT_ON_APPLE) {
			_platform = "apple_web";
		}
		...
		switch (_platform) {
			...
			case "apple_web": //This case applies on iOS, tvOS, and MacOS
				if (
					keyboard_check_released(vk_lmeta) || keyboard_check_released(vk_rmeta)
				) {
	...
	if (_global.__gamepad_allowed && (_global.__frame > __INPUT_GAMEPADS_TICK_PREDELAY)) {
		...
		if (_global.__allow_gamepad_enumerate) {
			if (
				_global.__current_time - _global.__enumeration_time
				> INPUT_ANDROID_GAMEPAD_ENUMERATION_INTERVAL
			) {
		...
		if (__INPUT_ON_LINUX || __INPUT_ON_ANDROID) {
			...
			repeat (_change) {
				...
				array_push(
					INPUT_GAMEPAD,
					new __input_class_source(
						__INPUT_SOURCE.GAMEPAD,
						array_length(INPUT_GAMEPAD)
					)
				);

				if (
					(_global.__source_mode == INPUT_SOURCE_MODE.MIXED)
					|| (_global.__source_mode == INPUT_SOURCE_MODE.MULTIDEVICE)
				) {
					_global
						.__players[0]
						.__source_add(INPUT_GAMEPAD[array_length(INPUT_GAMEPAD) - 1]);
				}
			...
			if (is_struct(_gamepad)) {
				if (_connected) {
					with (_gamepad) {
						if (
							(os_type == os_switch)
							&& (__description != gamepad_get_description(_g))
						) {
				...
				if (_global.__gamepad_connections_native[_g]) {
					...
					if (!__INPUT_SILENT) {
						__input_trace(
							"New gamepad = \"",
							gamepad_get_description(_g),
							"\", GUID=\"",
							gamepad_get_guid(_g),
							"\", buttons = ",
							gamepad_button_count(_g),
							", axes = ",
							gamepad_axis_count(_g),
							", hats = ",
							gamepad_hat_count(_g)
						);
					}
	...
	repeat (INPUT_MAX_PLAYERS) {
		...
		if (_global.__players[_p].__connected) {
			if (
				(_old_status == INPUT_STATUS.NEWLY_DISCONNECTED)
				|| (_old_status == INPUT_STATUS.DISCONNECTED)
			) {
			...
			if (
				(_old_status == INPUT_STATUS.NEWLY_CONNECTED)
				|| (_old_status == INPUT_STATUS.CONNECTED)
			) {
				_any_players_changed = true;
