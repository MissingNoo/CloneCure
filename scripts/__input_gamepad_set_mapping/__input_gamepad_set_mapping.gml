...
function __input_gamepad_set_mapping() {
	...
	if (__xinput) {
		...
		var _mapping_lt = __set_mapping(
			gp_shoulderlb,
			__XINPUT_AXIS_LT,
			__INPUT_MAPPING.AXIS,
			"lefttrigger"
		);
		var _mapping_rt = __set_mapping(
			gp_shoulderrb,
			__XINPUT_AXIS_RT,
			__INPUT_MAPPING.AXIS,
			"righttrigger"
	...
	switch (__raw_type) {
				...
				case "63257505":
					if (__INPUT_ON_LINUX || __INPUT_ON_WINDOWS) {
						...
						_mapping = __set_mapping(
							gp_axisrh,
							undefined,
							__INPUT_MAPPING.BUTTON_TO_AXIS,
							"rightx"
						);
		...
		case "CommunityNeoGeoMini":
			...
			if (
				(_vendor_and_product == "63257505")
				&& !__guessed_type
				&& (__INPUT_ON_LINUX || __INPUT_ON_WINDOWS)
			) {
				if (!__INPUT_SILENT) {
	...
	switch (os_type) {
		...
		case os_windows:
			...
			if (
				(_vendor_and_product == "5e04e002") || (_vendor_and_product == "5e04fd02") //XbOne Wireless Revision 2 //XbOne Wireless Revision 1
		...
		case os_linux:
			...
			if (
				__description == "Generic X-Box pad"
				|| __description == "X360 Controller"
			) {
				if (!__INPUT_SILENT) {
			...
			if (
				(__guid == "0500000049190000020400001b010000")
				&& (gamepad_get_description(__index) == "xbox Wireless Controller")
			) {
				__set_mapping(gp_face1, 0, __INPUT_MAPPING.BUTTON, "a");
				__set_mapping(gp_face2, 1, __INPUT_MAPPING.BUTTON, "b");
				__set_mapping(gp_face3, 2, __INPUT_MAPPING.BUTTON, "x");
		...
		if (is_array(__sdl2_definition)) {
			...
			repeat (array_length(__sdl2_definition) - 2) {
				...
				if (_gm_constant == undefined) {
					...
					if (string_char_at(_entry_1, string_length(_entry_1)) == "~") {
						...
						if (
							(_gm_constant == gp_axislh)
							|| (_gm_constant == gp_axislv)
							|| (_gm_constant == gp_axisrh)
							|| (_gm_constant == gp_axisrv)
						) {
							_input_reverse = true;
						} else {
							_input_invert = true;
					...
					if (__INPUT_DEBUG) {
						__input_trace(_entry_name, " = ", _raw_type, _entry_1);
					}
					...
					if (
						...
						if (__INPUT_ON_LINUX && _is_directional) {
							if (__INPUT_DEBUG) {
								__input_trace("  (Limited axis range)");
							}
							_mapping.__limited_range = true;
						} else if (
							!__INPUT_ON_LINUX
							&& !_is_directional
							&& (gamepad_axis_count(__index) >= _input_slot)
						) {
							//Nondirectional axes (triggers) use full axis range (excepting Linux remappings and XInput)
							if (__INPUT_DEBUG) {
								__input_trace("  (Extended axis range)");
							}
							_mapping.__extended_range = true;
						}
					}
			...
			if (__INPUT_ON_ANDROID && (__hat_count > 0) && (_vendor_and_product == "")) {
				...
				repeat (array_length(_dpad_array)) {
					...
					if (!is_struct(_mapping) || (_mapping[$ "__raw"] != 11 + _matched)) {
						break;
					}
					++_matched;
				}
				...
				if (_matched == 4) {
					...
					if (__INPUT_DEBUG) {
						__input_trace("  (Remapping dpad buttons to hat)");
					}
					__set_dpad_hat_mapping();
				}
			}
			...
			if (
				...
				&& (__INPUT_ON_WINDOWS || __INPUT_ON_LINUX)
			) {
				...
				if (__INPUT_DEBUG) {
					__input_trace("  (Remapping guide button)");
				}
			...
			if (
				(__simple_type == "xbox one")
				&& __input_string_contains(__description, "Elite")
				&& is_struct(__mapping_gm_to_raw[$ string(gp_paddle2)])
				&& is_struct(__mapping_gm_to_raw[$ string(gp_paddle3)])
			) {
				if (__INPUT_DEBUG) {
					__input_trace("  (Swapping Elite P2 and P3)");
				}
				var _p2_mapping = __mapping_gm_to_raw[$ string(gp_paddle2)].__raw;
				__set_mapping(
					gp_paddle2,
					__mapping_gm_to_raw[$ string(gp_paddle3)].__raw,
					__INPUT_MAPPING.BUTTON,
					"paddle2"
				);
