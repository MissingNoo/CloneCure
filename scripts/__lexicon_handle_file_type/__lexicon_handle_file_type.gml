...
function __lexicon_handle_file_type(_lang, _fileStruct) {
	...
	try {
		if (is_struct(_fileStruct) && _fileStruct[$ "fileType"] != undefined) {
			...
			_string = string_lower(
				string_copy(_filePath, _dotPos, string_length(_filePath) - _dotPos + 1)
			);
		}
		...
		if (_fileTypes[$ _string] != undefined) {
			...
			var _async;
			if (is_struct(_fileStruct)) {
				_async = _fileStruct[$ "isAsync"] != undefined
					? _fileStruct.isAsync
					: false;
			} else {
				_async = false;
		...
		__lexicon_throw(
			"an unintentional error has occurred. Please let the developer know. "
				+ string(_ex.message)
		);
	}
}
