...
function __input_class_source(_source, _gamepad = undefined) constructor {
	...
	switch (__source) {
		case __INPUT_SOURCE.KEYBOARD:
			__name = INPUT_ASSIGN_KEYBOARD_AND_MOUSE_TOGETHER
				? "keyboard and mouse"
				: "keyboard";
			break;
		...
		case __INPUT_SOURCE.GAMEPAD:
			__name = "gamepad " + string(_gamepad);
			break;
	...
	static __equal_to = function(_other_source) {
		switch (_other_source.__source) {
			case __INPUT_SOURCE.KEYBOARD:
				return __source == __INPUT_SOURCE.KEYBOARD;
				break;
			case __INPUT_SOURCE.MOUSE:
				return __source == __INPUT_SOURCE.MOUSE;
				break;
			case __INPUT_SOURCE.TOUCH:
				return __source == __INPUT_SOURCE.TOUCH;
				break;
			case __INPUT_SOURCE.GAMEPAD:
				return (__source == __INPUT_SOURCE.GAMEPAD)
					&& (__gamepad == _other_source.__gamepad);
				break;
		}
	...
	static __scan_for_binding = function(
		_player_index,
		_return_boolean,
		_ignore_struct,
		_allow_struct
		...
		if (__source == __INPUT_SOURCE.TOUCH) {
			return undefined;
		}
		...
		if (
			...
			&& (
				(__source == __INPUT_SOURCE.KEYBOARD)
					|| (__source == __INPUT_SOURCE.MOUSE)
			)
		) {
			var _binding = __input_source_scan_for_binding(
			...
			return __input_source_scan_for_binding(
				__INPUT_SOURCE.MOUSE,
				__gamepad,
				_player_index,
				_return_boolean,
				_ignore_struct,
	...
	static __validate_binding = function(_binding) {
		...
		if (
			(_type == __INPUT_BINDING_GAMEPAD_BUTTON)
			|| (_type == __INPUT_BINDING_GAMEPAD_AXIS)
		) {
			...
			if (__source == __INPUT_SOURCE.GAMEPAD) {
				...
				if (
					!is_struct(_gamepad)
					|| (_gamepad.__mapping_gm_to_raw[$ _value] == undefined)
				) {
				...
				var _raw = (_mapping.__raw == undefined)
					? _mapping.__raw_negative
					: _mapping.__raw;
				if (_raw == undefined) {
					//Raw value invalid
					return false;
				...
				if (
					_gamepad.__xinput
					&& ((_raw == __XINPUT_AXIS_LT) || (_raw == __XINPUT_AXIS_RT))
				) {
				...
				if (_raw == 0) {
					var _hat_mask = (_mapping.__hat_mask == undefined)
						? _mapping.__hat_mask_negative
						: _mapping.__hat_mask;
		...
		switch (_type) {
			...
			case __INPUT_BINDING_KEY:
				...
				if (__INPUT_ON_ANDROID) {
					...
					if (
						(_lowercase != undefined)
						&& !__input_key_is_ignored(_lowercase)
						&& (_lowercase >= __INPUT_KEYCODE_MIN)
						&& (_lowercase <= __INPUT_KEYCODE_MAX)
					) {
				...
				return !__input_key_is_ignored(_value)
					&& (_value >= __INPUT_KEYCODE_MIN)
					&& (_value <= __INPUT_KEYCODE_MAX);
				break;
					...
					case mb_side2: //Invalid on console, OperaGX, mobile, Firefox or Mac browsers
						return !(
							INPUT_ON_CONSOLE
								|| __INPUT_ON_OPERAGX
								|| INPUT_ON_MOBILE
								|| (os_browser == browser_firefox)
								|| (INPUT_ON_WEB && __INPUT_ON_MACOS)
						);
						break;
...
function __input_source_scan_for_binding(
	_source,
	_gamepad,
	_player_index,
	_return_boolean,
	...
	var _filter_func = function(_input, _ignore_struct, _allow_struct) {
		//Returns <false> if the binding failed to pass the filter
		if (
			is_struct(_ignore_struct)
			&& variable_struct_exists(_ignore_struct, string(_input))
		) {
			return false;
		}
		if (
			is_struct(_allow_struct)
			&& !variable_struct_exists(_allow_struct, string(_input))
		) {
			return false;
		}
		return true;
	...
	switch (_source) {
		case __INPUT_SOURCE.KEYBOARD:
			...
			if (
				__global.__any_keyboard_binding_defined
				&& (_keyboard_key >= __INPUT_KEYCODE_MIN)
				&& (_keyboard_key <= __INPUT_KEYCODE_MAX)
		...
		case __INPUT_SOURCE.MOUSE:
			if (INPUT_MOUSE_ALLOW_SCANNING) {
				...
				if (
					__global.__mouse_allowed
					&& !__global.__window_focus_block_mouse
					...
					mouse_wheel_up()
					&& _filter_func(
						__INPUT_BINDING_MOUSE_WHEEL_UP,
						_ignore_struct,
						_allow_struct
					)
				) {
					...
					mouse_wheel_down()
					&& _filter_func(
						__INPUT_BINDING_MOUSE_WHEEL_DOWN,
						_ignore_struct,
						_allow_struct
					)
				) {
				...
				repeat (array_length(_check_array)) {
					...
					if (input_gamepad_is_axis(_gamepad, _check)) {
						...
						if (
							...
							) && _filter_func(_check, _ignore_struct, _allow_struct)
						) {
							if (_return_boolean) {
								return true;
							}
							var _binding = input_binding_gamepad_axis(
								_check,
								(_value < 0)
							);
							if (__global.__source_mode == INPUT_SOURCE_MODE.MULTIDEVICE) {
								_binding.__gamepad_set(_gamepad);
							}
							return _binding;
						}
					} else {
						if (
							input_gamepad_check(_gamepad, _check)
							&& _filter_func(_check, _ignore_struct, _allow_struct)
						) {
							if (_return_boolean) {
								return true;
