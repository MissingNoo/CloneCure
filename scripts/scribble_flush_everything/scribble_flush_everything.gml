...
function scribble_flush_everything() {
	if (__SCRIBBLE_DEBUG) {
		__scribble_trace("Flushing everything");
	}
	...
	with (__scribble_initialize().__cache_state) {
		...
		repeat (array_length(__gc_vbuff_ids)) {
			if (__SCRIBBLE_DEBUG) {
				__scribble_trace("Deleting vertex buffer ", __gc_vbuff_ids[_i]);
			}
			vertex_delete_buffer(__gc_vbuff_ids[_i]);
			++_i;
		}
		if (__SCRIBBLE_DEBUG) {
			__scribble_trace("Clearing vertex buffer cache");
		}
		...
		repeat (array_length(__gc_grid_ids)) {
			if (__SCRIBBLE_DEBUG) {
				__scribble_trace("Deleting glyph grid ", __gc_grid_ids[_i]);
			}
		...
		if (__SCRIBBLE_DEBUG) {
			__scribble_trace("Clearing glyph grid cache");
		}
