...
function __scribble_initialize() {
	...
	with (_system) {
		__scribble_trace(
			"Welcome to Scribble Deluxe by Juju Adams! This is version "
				+ SCRIBBLE_VERSION
				+ ", "
				+ SCRIBBLE_DATE
		);
		...
		if (!shader_is_compiled(__shd_scribble)) {
			__scribble_error(
				"Shader failed to compile. Please check your version of GameMaker is compatible\nPlease report this error if it persists"
			);
		}

		if (!font_exists(asset_get_index("scribble_fallback_font"))) {
			__scribble_error(
				"Fallback font was not found. This may indicate that unused assets have been stripped from the project\nPlease untick \"Automatically remove unused assets when compiling\" in Game Options"
			);
		}
		...
		if (_fontInfo[$ "sdfEnabled"] == undefined) {
			__scribble_error(
				"Versions of GameMaker without SDF font support are not supported (versions pre-2023.1, including LTS 2022)"
			);
		}
		...
		__gmMightRemoveUnusedAssets =
			(__gmVersionMajor >= 2025)
			|| (
				(__gmVersionMajor == 2024)
					&& ((__gmVersionMinor >= 1100) || (__gmVersionMinor == 11))
			);
	...
	scribble_anim_reset();
	if (SCRIBBLE_LOAD_FONTS_ON_BOOT) {
