...
function __input_gamepad_find_in_sdl2_database() {
	...
	if (!__INPUT_SDL2_SUPPORT || !INPUT_SDL2_REMAPPING || __blacklisted || __xinput) {
		return;
	}
	...
	var _description_array = _global
		.__sdl2_database
		.by_description[$ string_copy(__guid, 1, 20)];
	if (is_array(_description_array)) {
		if (array_length(_description_array) > 0) {
			//Get the first binding for this description and OS
		...
		if (is_array(_vp_array)) {
			if (array_length(_vp_array) > 0) {
				//Get the first binding for this vendor+product and OS
