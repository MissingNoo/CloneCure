...
function __scribble_gen_6_build_lines() {
	...
	repeat (max(1, SCRIBBLE_FIT_TO_BOX_ITERATIONS)) {
		...
		if (_word_count > 0) {
			...
			var _control_delta =
				_glyph_grid[# 0, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT] - _control_index;
			repeat (_control_delta) {
				if (
					_control_grid[# _control_index, __SCRIBBLE_GEN_CONTROL.__TYPE]
					== __SCRIBBLE_GEN_CONTROL_TYPE.__HALIGN
				) {
					_state_halign = _control_grid[#
						_control_index,
			...
			repeat (_word_count) {
				...
				var _word_start_glyph = _word_grid[#
					_i,
					__SCRIBBLE_GEN_WORD.__GLYPH_START
				];

				//Find any horizontal alignment changes
				var _control_delta =
					_glyph_grid[# _word_start_glyph, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT]
					- _control_index;
				repeat (_control_delta) {
					switch (
						_control_grid[# _control_index, __SCRIBBLE_GEN_CONTROL.__TYPE]
				...
				if (_word_x + _word_width > _simulated_model_max_width) {
					...
					if (_word_width >= _simulated_model_max_width) {
						...
						if (
							_word_grid[# _i, __SCRIBBLE_GEN_WORD.__BIDI]
							>= __SCRIBBLE_BIDI.R2L
						) {
							...
							ds_grid_set_grid_region(
								...
								0,
								_word_count,
								__SCRIBBLE_GEN_WORD.__SIZE - 1,
								0,
								0
							);

							var _original_word_bidi_raw = _word_grid[#
								_i,
								__SCRIBBLE_GEN_WORD.__BIDI_RAW
							];
							var _original_word_bidi = _word_grid[#
								_i,
								__SCRIBBLE_GEN_WORD.__BIDI
							];
							var _original_word_glyph_start = _word_grid[#
								_i,
								__SCRIBBLE_GEN_WORD.__GLYPH_START
							];
							var _original_word_glyph_end = _word_grid[#
								_i,
								__SCRIBBLE_GEN_WORD.__GLYPH_END
							];
							//var _original_word_width       = _word_grid[# _i, __SCRIBBLE_GEN_WORD.__WIDTH      ]; //Unused
							var _original_word_height = _word_grid[#
								_i,
								__SCRIBBLE_GEN_WORD.__HEIGHT
							];
							...
							if (
								...
								var _left_correction = _glyph_grid[#
									_original_word_glyph_start,
							...
							var _glyph_width = _glyph_grid[#
								_j,
								__SCRIBBLE_GEN_GLYPH.__SEPARATION
							];
							if (
								(_word_x + _glyph_width >= _simulated_model_max_width)
								&& (_i > _line_word_start)
							) {
							...
							repeat (1 + _original_word_glyph_end - _j) {
								...
								if (_word_x + _glyph_width >= _simulated_model_max_width) {
									_word_grid[# _i, __SCRIBBLE_GEN_WORD.__BIDI_RAW] =
										_original_word_bidi_raw;
									_word_grid[# _i, __SCRIBBLE_GEN_WORD.__BIDI] =
										_original_word_bidi;
									_word_grid[# _i, __SCRIBBLE_GEN_WORD.__GLYPH_START] =
										_new_word_glyph_start;
									_word_grid[# _i, __SCRIBBLE_GEN_WORD.__GLYPH_END] =
										_j - 1;
									_word_grid[# _i, __SCRIBBLE_GEN_WORD.__WIDTH] =
										_word_x - _new_word_start_x;
									_word_grid[# _i, __SCRIBBLE_GEN_WORD.__HEIGHT] =
										_original_word_height;
					...
					var _glyph_start_ord = _glyph_grid[#
						_word_start_glyph,
					...
					if (_glyph_start_ord == 0x0A) {
						...
						_line_grid[#
							_line_count - 1,
							__SCRIBBLE_GEN_LINE.__DISABLE_JUSTIFY
						] = true;
					} else if (_glyph_start_ord == 0x00) {
						//Null, indicates a new page
						...
						if (_i < _word_count - 1) {
							_line_grid[#
								_line_count,
								__SCRIBBLE_GEN_LINE.__STARTS_MANUAL_PAGE
							] = true;
						}
					}
				}
			...
			if (_line_word_end >= _line_word_start) {
				//Only generate a new line if we actually have glyphs on the final line
				__SCRIBBLE_GEN_LINE_END;
				_line_y += _line_height;
			}
		}
		...
		if (!_wrap_no_pages || (SCRIBBLE_FIT_TO_BOX_ITERATIONS <= 1)) {
			break;
		}
	...
	if (SCRIBBLE_NEWLINES_PAD_LEFT_SPACE || SCRIBBLE_NEWLINES_TRIM_LEFT_SPACE) {
		...
		repeat (_line_count) {
			...
			if (
				_word_grid[# _line_word_start, __SCRIBBLE_GEN_WORD.__BIDI]
				< __SCRIBBLE_BIDI.R2L
			) {
				var _word_glyph_start = _word_grid[#
					_line_word_start,
					__SCRIBBLE_GEN_WORD.__GLYPH_START
				];
				var _word_glyph_end = _word_grid[#
					_line_word_start,
					__SCRIBBLE_GEN_WORD.__GLYPH_END
				];
				var _left_correction = _glyph_grid[#
				...
				if (
					((_left_correction > 0) && SCRIBBLE_NEWLINES_PAD_LEFT_SPACE)
					|| ((_left_correction < 0) && SCRIBBLE_NEWLINES_TRIM_LEFT_SPACE)
				) {
					ds_grid_add_region(
						_glyph_grid,
						_word_glyph_start,
	...
	if (SCRIBBLE_FLEXIBLE_WHITESPACE_WIDTH && _wrap_apply) {
		...
		repeat (_line_count) {
			...
			if (
				_word_grid[# _line_end_word, __SCRIBBLE_GEN_WORD.__BIDI_RAW]
				== __SCRIBBLE_BIDI.WHITESPACE
			) {
				//Only adjust whitespace words
				var _line_width = _line_grid[# _line, __SCRIBBLE_GEN_LINE.__WIDTH];
				if (_line_width > _simulated_model_max_width) {
					//Only adjust lines that actually exceed the maximum size
					var _delta = _simulated_model_max_width - _line_width;

					_line_grid[# _line, __SCRIBBLE_GEN_LINE.__WIDTH] =
						_simulated_model_max_width;
					...
					_glyph_grid[# _word_start_glyph, __SCRIBBLE_GEN_GLYPH.__WIDTH] +=
						_delta;
					_glyph_grid[# _word_start_glyph, __SCRIBBLE_GEN_GLYPH.__SEPARATION] +=
						_delta;
				}
			}
	...
	__width = ds_grid_get_max(
		_line_grid,
		0,
		__SCRIBBLE_GEN_LINE.__WIDTH,
		_line_count - 1,
		__SCRIBBLE_GEN_LINE.__WIDTH
	);
