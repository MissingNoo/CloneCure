...
function __input_class_verb_state() constructor {
	...
	static __tick = function(_verb_group_state_dict, _player_active) {
		...
		if (
			__global.__toggle_momentary_state
			&& (__type == __INPUT_VERB_TYPE.__BASIC)
			&& variable_struct_exists(__global.__toggle_momentary_dict, __name)
		) {
		...
		if (
			__global.__cooldown_state
			&& (__type == __INPUT_VERB_TYPE.__BASIC)
			&& variable_struct_exists(__global.__cooldown_dict, __name)
		) {
			if (
				_time
				< __release_time
					+ (INPUT_TIMER_MILLISECONDS
						? __INPUT_RATE_LIMIT_DURATION
						: (
							(__INPUT_RATE_LIMIT_DURATION / 1000)
