...
function input_xy(
	...
	_verb_d,
	_player_index = 0,
	_most_recent = INPUT_DEFAULT_2D_MOST_RECENT
) {
	static _result = {x: 0, y: 0};
	...
	if (
		((_value_l > 0.0) && !_verb_struct_l.__raw_analogue)
	...
	if (_most_recent) {
		if ((_value_l > 0.0) && (_value_r > 0.0)) {
			_dx = (_verb_struct_l.__press_time > _verb_struct_r.__press_time)
				? -_value_l
				: _value_r;
		}
		if ((_value_u > 0.0) && (_value_d > 0.0)) {
			_dy = (_verb_struct_u.__press_time > _verb_struct_d.__press_time)
				? -_value_u
				: _value_d;
		}
	}
	...
	var _coeff = clamp(
		(_d - _min_threshold) / (_max_threshold - _min_threshold),
		0.0,
		1.0
	);
	_dx *= _coeff;
	...
	if (INPUT_2D_XY_AXIS_BIAS > 0) {
		...
		var _dir_mod = (_direction % _divisor) / _divisor;
		_dir_mod = clamp(
			(_dir_mod - 0.5 * 0.999 * INPUT_2D_XY_AXIS_BIAS)
				/ (1 - 0.999 * INPUT_2D_XY_AXIS_BIAS),
			0,
			1
		);
