...
function input_binding_get_icon(_binding, _player_index = 0) {
	...
	if (_fallback_icon_struct == undefined) {
		switch (os_type) {
			case os_ps3:
				_fallback_category_name = INPUT_GAMEPAD_TYPE_PSX;
				break;
			case os_ps4:
				_fallback_category_name = INPUT_GAMEPAD_TYPE_PS4;
				break;
			case os_ps5:
				_fallback_category_name = INPUT_GAMEPAD_TYPE_PS5;
				break;
			case os_switch:
				_fallback_category_name = INPUT_GAMEPAD_TYPE_SWITCH;
				break;
			case os_xboxone:
				_fallback_category_name = INPUT_GAMEPAD_TYPE_XBOX_ONE;
				break;
	...
	switch (_type) {
		...
		case __INPUT_BINDING_GAMEPAD_AXIS:
			var _category =
				_global.__players[_player_index].__gamepad_type_override
				?? input_player_get_gamepad_type(_player_index, _binding);
			break;
	...
	var _icon_struct = is_struct(_category_data)
		? _category_data.__dictionary
		: _fallback_icon_struct;
