...
function lexicon_text(_textEntry) {
	...
	static _recursionStack = (__LEXICON_ENTRY_RECURSION_DETECTION)
		? ds_list_create()
		: undefined;
	var _str = _textEntry;

	if (argument_count > array_length(_staticArray)) {
		array_resize(_staticArray, argument_count);
	}
	...
	if (
		(string_count(_replaceChrStart, _str) == 0)
		|| (string_count(_replaceChrEnd, _str) == 0)
	) {
	...
	if (__LEXICON_ENTRY_RECURSION_DETECTION) {
		...
		repeat (ds_list_size(_recursionStack)) {
			if (_recursionStack[| _i] == _textEntry) {
				var _errorStr =
					"Found recursion within text entries!\nEnsure that localisation files do not contain duplicated reference of "
					+ string(_textEntry)
					+ "\nEntries traversed:";
				_i = 0;
				repeat (ds_list_size(_recursionStack)) {
	...
	if (ds_map_exists(_cacheMap, _cacheStr)) {
		...
		ds_list_add(
			_global.cacheList,
			{cacheStr: _cacheStr, ref: weak_ref_create(_cacheStruct)}
		);
	}
	#endregion
	var _newStr;
