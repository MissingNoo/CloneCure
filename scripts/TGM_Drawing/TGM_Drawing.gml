...
function draw_nineslice_stretched_ext(
	_sprite,
	_subimg,
	_x,
	_y,
	...
	draw_sprite_ext(
		...
		_x,
		_y,
		(_width / sprite_get_width(_sprite)) * _xscale,
		(_height / sprite_get_height(_sprite)) * _yscale,
		_rot,
		_col,
		_alpha
	);
...
function draw_sprite_centered(_sprite, _subimg, _x, _y) {
	draw_sprite(
		_sprite,
		_subimg,
		_x - sprite_get_width(_sprite) / 2,
		_y - sprite_get_height(_sprite) / 2
	);
}
...
function draw_sprite_centered_ext(
	_sprite,
	_subimg,
	_x,
	_y,
	...
	draw_sprite_ext(
		_sprite,
		_subimg,
		_x - (sprite_get_width(_sprite) / 2) * _xscale,
		_y - (sprite_get_height(_sprite) / 2) * _yscale,
		_xscale,
		_yscale,
		_rot,
		_color,
...
function draw_surface_centered(_surfaceId, _x, _y) {
	draw_surface(
		_surfaceId,
		_x - (surface_get_width(_surfaceId) / 2),
		_y - (surface_get_height(_surfaceId) / 2)
	);
}
...
function draw_surface_centered_ext(
	_surfaceId,
	_x,
	_y,
	_xscale,
	...
	draw_surface(
		_surfaceId,
		-surface_get_width(_surfaceId) / 2,
		-surface_get_height(_surfaceId) / 2
	);
...
function draw_sprite_pos_ext(
	...
	matrix_set(
		matrix_world,
		matrix_build(_x, _y, 0, 0, 0, _rotation, _xscale, _yscale, 1)
	);
...
function gpu_set_blendmode_test(index, _debugInfo = false) {
	...
	*/
	...
	if (index == 0) {
		...
		_type = string_join_ext(
			", ",
			[__type1(_bm1), __type1(_bm2), __type1(_bm3), __type1(_bm4)]
		);
...
function color_gradient(_colorsArray, _progress) {
	...
	return merge_color(
		_colorsArray[floor(_prog)],
		_colorsArray[ceil(_prog)],
		frac(_prog)
	);
}
...
function draw_text_outline_gradient(
	_x,
	_y,
	_str,
	_outlineColor1 = c_black,
	_outlineColor2 = c_black,
	_outlineColor3 = c_black,
	_outlineColor4 = c_black,
	_outlineAlpha = 1,
	_outlineSize = 1,
	_fidelity = 4
) {
	...
	for (var i = 0; i < Tau; i += _fidelityIncrement) {
		draw_text_color(
			_x + cos(i) * _outlineSize,
			_y - sin(i) * _outlineSize,
			_str,
			_outlineColor1,
			_outlineColor2,
			_outlineColor3,
...
function draw_text_transformed_shadow(
	_x,
	_y,
	_str,
	_xscale,
	...
	draw_text_transformed(
		_x + (_shadowDistX * _xscale),
		_y + (_shadowDistY * _yscale),
		_str,
		_xscale,
		_yscale,
		_angle
...
function draw_text_wave(_x, _y, _str, _strWidth, _waveAmplitude = 3, _waveSpeed = 0.01) {
	...
	repeat (_isize) {
		...
		draw_text(
			_xx - _wf,
			_yy + sin(i + current_time * _waveSpeed) * _waveAmplitude,
			_char
		);
		_xx += _ww;
		i++;
...
function draw_text_wave_colorful(
	_x,
	_y,
	_str,
	_strWidth,
...
function draw_text_wave_rainbow(
	_x,
	_y,
	_str,
	_strWidth,
	...
	repeat (_isize) {
		...
		var _wf = (_strWidth == 0) ? 0 : _strWidth / 2;
		draw_set_color(
			make_color_hsv((current_time * _colorSpeed + i * 10) % 255, 255, 255)
		);
		draw_text(
			_xx - _wf,
			_yy + sin(i + current_time * _waveSpeed) * _waveAmplitude,
...
function draw_text_shake(_x, _y, _str, _strWidth, _dist = 1) {
	...
	repeat (_isize) {
		...
		var _wf = (_strWidth == 0) ? 0 : _strWidth / 2;
		draw_text(
			_xx - _wf + random_range(-_dist, _dist),
			_yy + random_range(-_dist, _dist),
			_char
		);
		_xx += _ww;
		i++;
