...
function scribble_glyph_set(_font, _character, _property, _value, _relative = false) {
	...
	if ((_character == all) || (_character == "all")) {
		...
		if (_property == SCRIBBLE_GLYPH.SEPARATION) {
			_grid[# _glyph_index, SCRIBBLE_GLYPH.WIDTH] = _grid[#
				_glyph_index,
				SCRIBBLE_GLYPH.SEPARATION
			];
		}
		if (_property == SCRIBBLE_GLYPH.WIDTH) {
			_grid[# _glyph_index, SCRIBBLE_GLYPH.SEPARATION] = _grid[#
				_glyph_index,
				SCRIBBLE_GLYPH.WIDTH
			];
		}

		//Changing the height of the space character also changes its font height
		if (_property == SCRIBBLE_GLYPH.HEIGHT) {
			_grid[# _glyph_index, SCRIBBLE_GLYPH.FONT_HEIGHT] = _grid[#
				_glyph_index,
				SCRIBBLE_GLYPH.HEIGHT
			];
		}
		if (_property == SCRIBBLE_GLYPH.FONT_HEIGHT) {
			_grid[# _glyph_index, SCRIBBLE_GLYPH.HEIGHT] = _grid[#
				_glyph_index,
				SCRIBBLE_GLYPH.FONT_HEIGHT
			];
		}

		//Ensure that a change to the height of a space character also sets the font height for the whole font
		if (
			(_property == SCRIBBLE_GLYPH.HEIGHT)
			|| (_property == SCRIBBLE_GLYPH.FONT_HEIGHT)
		) {
			ds_grid_set_region(
				_grid,
				0,
				...
				ds_grid_width(_grid) - 1,
				SCRIBBLE_GLYPH.FONT_HEIGHT,
				_grid[# _glyph_index, SCRIBBLE_GLYPH.FONT_HEIGHT]
			);
		...
		if (_unicode == 0x20) {
			...
			//Changing the width of the space character also changes the separation
			if (_property == SCRIBBLE_GLYPH.SEPARATION) {
				_grid[# _glyph_index, SCRIBBLE_GLYPH.WIDTH] = _new_value;
			}
			if (_property == SCRIBBLE_GLYPH.WIDTH) {
				_grid[# _glyph_index, SCRIBBLE_GLYPH.SEPARATION] = _new_value;
			}

			//Changing the height of the space character also changes its font height
			if (_property == SCRIBBLE_GLYPH.HEIGHT) {
				_grid[# _glyph_index, SCRIBBLE_GLYPH.FONT_HEIGHT] = _new_value;
			}
			if (_property == SCRIBBLE_GLYPH.FONT_HEIGHT) {
				_grid[# _glyph_index, SCRIBBLE_GLYPH.HEIGHT] = _new_value;
			}
