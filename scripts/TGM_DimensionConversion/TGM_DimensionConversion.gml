...
function room_to_gui_dimension(x1, y1, camera, gui_width, gui_height, normalize) {
	var _px =
			...
			* (gui_width / camera_get_view_width(camera)),
		_py =
			(y1 - camera_get_view_y(camera))
			* (gui_height / camera_get_view_height(camera));
	return normalize
		? new Vector2(_px / gui_width, _py / gui_height)
		: new Vector2(_px, _py);
}
...
/// @param {bool} normalize If enabled, the value will be returned from 0 to 1 - useful for shaders and so on
/// @returns {struct}
function room_to_gui_dimension_ext(
	x1,
	y1,
	camera,
	angle,
...
/// @param {bool} normalize If enabled, the value will be returned from 0 to 1 - useful for shaders and so on.
/// @returns {struct}
function gui_to_room_dimension_ext(
	x1,
	y1,
	camera,
	angle,
...
/// @returns {struct}
function screen_to_world_dimension(
	_viewMatrix,
	_projectionMatrix,
	_x,
	_y,
	...
	var _camX = -(
		_viewMatrix[12] * _viewMatrix[0]
			+ _viewMatrix[13] * _viewMatrix[1]
			+ _viewMatrix[14] * _viewMatrix[2]
	);
	var _camY = -(
		_viewMatrix[12] * _viewMatrix[4]
			+ _viewMatrix[13] * _viewMatrix[5]
			+ _viewMatrix[14] * _viewMatrix[6]
	);
	var _camZ = -(
		_viewMatrix[12] * _viewMatrix[8]
			+ _viewMatrix[13] * _viewMatrix[9]
			+ _viewMatrix[14] * _viewMatrix[10]
	);
...
/// @returns {struct}
function world_to_screen_dimension(
	_viewMatrix,
	_projectionMatrix,
	_x,
	_y,
	...
	return new Vector2(
		(_cx * 0.5 + 0.5) * _canvasWidth,
		(_cy * 0.5 + 0.5) * _canvasHeight
	);
}
