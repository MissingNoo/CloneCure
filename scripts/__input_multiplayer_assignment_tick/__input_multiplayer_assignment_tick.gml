...
function __input_multiplayer_assignment_tick() {
	...
	if (
		(_global.__join_leave_verb != undefined)
		&& !variable_struct_exists(_global.__basic_verb_dict, _global.__join_leave_verb)
	) {
		__input_error(
			"Multiplayer leave verb \"",
			_global.__join_leave_verb,
	...
	repeat (_global.__join_player_max) {
		if (
			(_global.__join_leave_verb != undefined)
			&& input_check_pressed(_global.__join_leave_verb, _p)
		) {
	...
	if (_abort && (_global.__join_abort_callback != undefined)) {
		__input_trace(
			...
			_global.__previous_source_mode
		);
		...
		if (is_method(_global.__join_abort_callback)) {
			...
			is_numeric(_global.__join_abort_callback)
			&& script_exists(_global.__join_abort_callback)
		) {
			script_execute(_global.__join_abort_callback);
		} else {
			__input_error(
				"Multiplayer abort callback set to an illegal value (typeof=",
				typeof(_global.__join_abort_callback),
				")"
			);
		}
	}
