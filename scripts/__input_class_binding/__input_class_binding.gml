...
function __input_class_binding() constructor {
	...
	static __threshold_get = function() {
		return {mini: __threshold_min, maxi: __threshold_max};
	};
	...
	static __import = function(_binding_shell) {
		var _type_var = variable_struct_exists(_binding_shell, "type")
			? "type"
			: "__type";
		var _value_var = variable_struct_exists(_binding_shell, "value")
			? "value"
			: "__value";
		var _axis_negative_var = variable_struct_exists(_binding_shell, "axis_negative")
			? "axis_negative"
			: "__axis_negative";
		var _gamepad_description_var = variable_struct_exists(
			_binding_shell,
			"gamepad_description"
		)
			? "gamepad_description"
			: "__gamepad_description";
		var _threshold_min_var = variable_struct_exists(_binding_shell, "threshold_min")
			? "threshold_min"
			: "__threshold_min";
		var _threshold_max_var = variable_struct_exists(_binding_shell, "threshold_max")
			? "threshold_max"
			: "__threshold_max";
		...
		if (
			!variable_struct_exists(_binding_shell, _value_var)
		...
		if (
			...
			&& !variable_struct_exists(_binding_shell, _axis_negative_var)
		) {
		...
		if (
			...
			switch (_value) {
				case __INPUT_LEGACY_GP_GUIDE:
				case __INPUT_LEGACY_GP_ALT_GUIDE:
					_value = gp_guide;
					__input_trace(
					...
					break;
				case __INPUT_LEGACY_GP_MISC1:
				case __INPUT_LEGACY_GP_ALT_MISC1:
					_value = gp_misc1;
					__input_trace(
						"Warning! Legacy gamepad constant found, updating value (= ",
						_value,
					...
					break;
				case __INPUT_LEGACY_GP_TOUCHPAD:
				case __INPUT_LEGACY_GP_ALT_TOUCHPAD:
					_value = gp_touchpad;
					__input_trace(
						"Warning! Legacy gamepad constant found, updating value (= ",
						_value,
					...
					break;
				case __INPUT_LEGACY_GP_PADDLE1:
				case __INPUT_LEGACY_GP_ALT_PADDLE1:
					_value = gp_paddle1;
					__input_trace(
						"Warning! Legacy gamepad constant found, updating value (= ",
						_value,
					...
					break;
				case __INPUT_LEGACY_GP_PADDLE2:
				case __INPUT_LEGACY_GP_ALT_PADDLE2:
					_value = gp_paddle2;
					__input_trace(
						"Warning! Legacy gamepad constant found, updating value (= ",
						_value,
					...
					break;
				case __INPUT_LEGACY_GP_PADDLE3:
				case __INPUT_LEGACY_GP_ALT_PADDLE3:
					_value = gp_paddle3;
					__input_trace(
						"Warning! Legacy gamepad constant found, updating value (= ",
						_value,
					...
					break;
				case __INPUT_LEGACY_GP_PADDLE4:
				case __INPUT_LEGACY_GP_ALT_PADDLE4:
					_value = gp_paddle4;
					__input_trace(
						"Warning! Legacy gamepad constant found, updating value (= ",
						_value,
		...
		if (__gamepad_description != undefined) {
			...
			repeat (array_length(__global.__gamepads)) {
				...
				if (
					is_struct(_gamepad) && (_gamepad.description == __gamepad_description)
				) {
					__gamepad_index = _g;
					break;
				}
	...
	static __set_android_lowercase = function() {
		...
		if (__INPUT_ON_ANDROID && (__type == __INPUT_BINDING_KEY)) {
			...
			__android_lowercase = (_android_lowercase != __value)
				? _android_lowercase
				: undefined;
	...
	static __set_key = function(_key, _player_set) {
		...
		if (_player_set) {
			...
			if (!__INPUT_KEYBOARD_NORMATIVE) {
				...
				if (_keycode == vk_comma) {
					_keycode = 0x2C;
				} else if (_keycode == vk_hyphen) {
					_keycode = 0x2D;
				} else if (_keycode == vk_period) {
					_keycode = 0x2E;
				} else if (_keycode == vk_fslash) {
					_keycode = 0x2F;
				} else if (_keycode == vk_semicolon) {
					_keycode = 0x3B;
				} else if (_keycode == vk_equals) {
					_keycode = 0x3D;
				} else if (_keycode == vk_lbracket) {
					_keycode = 0x5B;
				} else if (_keycode == vk_bslash) {
					_keycode = 0x5C;
				} else if (_keycode == vk_rbracket) {
					_keycode = 0x5D;
				} else if (_keycode == vk_backtick) {
					_keycode = 0x60;
				}
			}
		}
	...
	static __get_source_type = function() {
		switch (__type) {
			...
			case __INPUT_BINDING_MOUSE_WHEEL_DOWN:
				...
				break;
			case __INPUT_BINDING_VIRTUAL_BUTTON:
				return __INPUT_SOURCE.TOUCH;
				break;
			case __INPUT_BINDING_GAMEPAD_BUTTON:
				return __INPUT_SOURCE.GAMEPAD;
				break;
			case __INPUT_BINDING_GAMEPAD_AXIS:
				return __INPUT_SOURCE.GAMEPAD;
				break;
