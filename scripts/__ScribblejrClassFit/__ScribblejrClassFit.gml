...
function __ScribblejrClassFit(
	...
	if (is_infinity(_maxWidth)) {
		...
		if (_system.__perCharacterWrap) {
			...
			if (_fitsAlready) {
				...
				//Doesn't fit, let's do some layout!
				...
				repeat (SCRIBBLEJR_FIT_ITERATIONS) {
					...
					repeat (array_length(_charArray)) {
						...
						if (_char == "\n") {
							...
							if (_cursorX + _spaceWidth > _adjustedWidth) {
								...
								if (_lastIteration && (_stretchStart == _i)) {
									_stretchStart++;
								}
					...
					if (_height > _adjustedHeight) {
						...
						if (_iteration == 0) {
							_iteration = SCRIBBLEJR_FIT_ITERATIONS - 2;
						}
					}
			...
			var _fitsAlready = true;
			var _height =
				_fontScale * string_height_ext(_string, -1, _maxWidth / _fontScale - 1);
			if (_height > _maxHeight) {
				_fitsAlready = false;
			} else {
				// _height <= _maxHeight
			...
			if (_fitsAlready) {
				...
				__width =
					__scale
					* ((_width > _adjustedWidth)
						? string_width_ext(_string, -1, __wrapWidth - 1)
						: _width);
				__height =
					__scale
					* ((_height > _adjustedHeight)
						? string_height_ext(_string, -1, __wrapWidth - 1)
						: _height);
				Draw = __DrawFit;
			}
	...
	static __DrawStretches = function(
		_x,
		_y,
		_colour = c_white,
		_alpha = 1,
	...
	static __DrawVertexBuffer = function(_x, _y, _colour = c_white, _alpha = 1) {
		static _shdScribblejr_u_vPositionAlphaScale = shader_get_uniform(
			__shdScribblejr,
			"u_vPositionAlphaScale"
		);
		static _shdScribblejr_u_iColour = shader_get_uniform(
			__shdScribblejr,
			"u_iColour"
		);
		...
		shader_set_uniform_f(
			_shdScribblejr_u_vPositionAlphaScale,
			_x,
			_y,
			_alpha,
			__scale
	...
	static __DrawVertexBufferSDF = function(
		...
		static _shdScribblejrSDF_u_vPositionAlphaScale = shader_get_uniform(
			__shdScribblejrSDF,
			"u_vPositionAlphaScale"
		);
		static _shdScribblejrSDF_u_iColour = shader_get_uniform(
			__shdScribblejrSDF,
			"u_iColour"
		);

		static _shdScribblejrSDFShadow_u_vPositionAlphaScale = shader_get_uniform(
			__shdScribblejrSDFShadow,
			"u_vPositionAlphaScale"
		);
		static _shdScribblejrSDFShadow_u_vColorSoftness = shader_get_uniform(
			__shdScribblejrSDFShadow,
			"u_vColorSoftness"
		);
		...
		with (_sdfEffects) {
			if (struct_get_from_hash(_sdfEffects, _dropShadowEnableHash)) {
				...
				shader_set_uniform_f(
					_shdScribblejrSDFShadow_u_vPositionAlphaScale,
					_x + _scale * dropShadowOffsetX,
					_y + _scale * dropShadowOffsetY,
					dropShadowAlpha * _alpha,
					_scale
				);
				shader_set_uniform_f(
					_shdScribblejrSDFShadow_u_vColorSoftness,
		...
		shader_set_uniform_f(
			_shdScribblejrSDF_u_vPositionAlphaScale,
			_x,
			_y,
			_alpha,
			__scale
