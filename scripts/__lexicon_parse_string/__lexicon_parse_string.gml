...
function __lexicon_parse_string(_lexiconTextCache) {
	...
	_lexiconTextCache.timeStamp = _global.framen;
	...
	if (!_lexiconTextCache.isCompiled) {
		...
		while (true) {
			if (
				!(
					string_count(_replaceChrStart + string(_ii) + _replaceChrEnd, _newStr)
						> 0
				)
			) {
				break;
			...
			_newStr = string_replace_all(
				_newStr,
				_replaceChrStart + string(_ii) + _replaceChrEnd,
				argument[_i] ?? "__lexicon_null" + string(_ii)
			);
			++_i;
			++_ii;
			//}
		...
		if (
			(string_count(_replaceChrStart, _newStr) > 0)
			&& (string_count(_replaceChrEnd, _newStr) > 0)
		) {
			var _remaining =
				string_count(_replaceChrStart, _newStr)
				+ string_count(_replaceChrEnd, _newStr);
			var _posStart = 1;
			var _posEnd = 1;
			...
			for (_i = 0; _i < _remaining; _i += 2) {
				...
				var _strKey = string_copy(
					_newStr,
					_posStart + _leftReplaceChrLen,
					_posEnd - _posStart - _leftReplaceChrLen
				);
				...
				if (string_count(",", _strKey) > 0) {
					var _argStr = string_copy(
						_strKey,
						string_pos(",", _strKey) + 1,
						string_length(_strKey)
					);
					...
					repeat (_len - 1) {
						...
						if (string_count("__lexicon_null", _args[_ii]) > 0) {
							...
							_strKey = string_replace(
								_strKey,
								"__lexicon_null" + _numStr,
								_replaceChrStart + _numStr + _replaceChrEnd
							);
							_args[_ii] =
								string_replace(
									_args[_ii],
					...
					if (string_count("__lexicon_null", _args[_ii]) > 0) {
						...
						_args[_ii] =
							string_replace(_args[_ii], "__lexicon_null", _replaceChrStart)
							+ _replaceChrEnd;
					}
					//_args[_ii] = string_copy(_argStr, _sepPos, _argStrLen);
					_strKeyStripped = string_copy(
						_strKey,
						1,
						string_pos(",", _strKey) - _sepLen
					);
				}
				...
				if (variable_struct_exists(_global.dynamicMap, _strKeyStripped)) {
					...
					if (_callback[1]) {
						...
						array_push(
							_dynamicArray,
							[_strKeyStripped, undefined, _strKey, _args]
						);
					} else {
						_newStr = string_replace_all(
							_newStr,
							_replaceChrStart + _strKey + _replaceChrEnd,
							__lexicon_resolve_dynamic(_strKeyStripped, _args)
						);
					}
					...
					if (
						__LEXICON_ENTRY_RECURSION_DETECTION
						&& (
							string_count(
								_strKeyStripped,
								_global.textEntries[$ _strKeyStripped]
							) > 0
						)
					) {
						__lexicon_throw(
							"Recursive entry found in "
								+ _strKeyStripped
					...
					repeat (argument_count - 1) {
						...
						if (
							(_target == undefined)
							&& variable_struct_exists(argument[_ii], _strKeyStripped)
						) {
							_lexiconTextCache.isDynamic = true;
							var _iii = 0;
							...
							repeat (array_length(_dynamicArray)) {
								...
								if (
									_entry != undefined
									&& weak_ref_alive(_entry)
									&& _entry.ref == _currentStruct
								) {
									_weakref = _entry;
									break;
								}
							...
							if (_weakref == undefined) {
								_weakref = weak_ref_create(argument[_ii]);
							}
							array_push(
								_dynamicArray,
								[_strKeyStripped, _weakref, _strKey, /*_hashAvailable ? variable_get_hash(_strKey) :*/ _args]
							);
							++_ii;
							break;
						}
						...
						if (variable_struct_exists(self, _strKeyStripped)) {
							...
							if (_weakref == undefined) {
								_weakref = weak_ref_create(self);
							}
							array_push(
								_dynamicArray,
								[_strKeyStripped, _weakref, _strKey, /*_hashAvailable ? variable_get_hash(_strKey) : undefined*/ _args]
							);
							++_ii;
							break;
						}
		...
		repeat (string_count("__lexicon_null", _newStr)) {
			if (string_count("__lexicon_null", _newStr) == 0) {
				break;
			}
			...
			_newStr = string_replace_all(
				_newStr,
				"__lexicon_null" + string(_i),
				_replaceChrStart + string(_i) + _replaceChrEnd
			);
			++_i;
		}
		...
		repeat (array_length(_dynamicArray)) {
			...
			if (_id == undefined) {
				_newStr = string_replace_all(
					_newStr,
					_replaceChrStart + _strKey + _replaceChrEnd,
					__lexicon_resolve_dynamic(_strKeyStripped, _entry[3])
				);
				...
				if (variable_struct_exists(_id.ref, _strKeyStripped)) {
					_newStr = string_replace_all(
						_newStr,
						_replaceChrStart + _strKey + _replaceChrEnd,
						/*_hashAvailable ? struct_get_from_hash(_id.ref, _entry[2]) :*/ _id.ref[$
							_strKeyStripped
						]
					);
				}
