...
function input_radial_sector(
	...
	if ((_distance <= max(0, _magnitude_min)) || (_distance > _magnitude_max)) {
		return false;
	}

	//Angle bound
	if (angle_difference(_angle_min, _angle_max) == 0.0) {
		return true;
	}

	//Angle difference
	_angle_min = _angle_min % 360;
	if (_angle_min < 0.0) {
		_angle_min += 360;
	}

	_angle_max = _angle_max % 360 - _angle_min;
	if (_angle_max < 0.0) {
		_angle_max += 360;
	}
