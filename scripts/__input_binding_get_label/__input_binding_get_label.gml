...
function __input_binding_get_label(_type, _value, _axis_negative) {
	if (!INPUT_ON_PC) {
		...
		if (__INPUT_ON_PS) {
			if (
				((_type == __INPUT_BINDING_GAMEPAD_BUTTON) && (_value == gp_select))
		...
		case __INPUT_BINDING_GAMEPAD_AXIS:
			switch (_value) {
				case gp_face1:
					...
					break; //Xbox A, Nintendo B, PlayStation Cross
				case gp_face2:
					return "gamepad face east";
					break; //Xbox B, Nintendo A, PlayStation Circle
				case gp_face3:
					return "gamepad face west";
					break; //Xbox X, Nintendo Y, PlayStation Square
				case gp_face4:
					return "gamepad face north";
					break; //Xbox Y, Nintendo X, PlayStation Triangle
				case gp_shoulderl:
				...
				case gp_select:
					return __INPUT_ON_PS ? "gamepad touchpad click" : "gamepad select";
					break;
