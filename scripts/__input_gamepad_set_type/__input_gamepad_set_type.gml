	...
	switch (os_type) {
		case os_switch:
			switch (__description) {
				case "Handheld":
					...
					break; //JoyCon L+R railed or Lite (Slot 0 only)
				case "Pro Controller":
		...
		default:
			...
			if (__xinput) {
				...
				variable_struct_exists(_global.__raw_type_dictionary, _vendor_and_product)
			) {
				...
				if (__input_string_contains(_desc, "neogeo")) {
					...
					if (
						__input_string_contains(_desc, "xbox", "ultimate", "zero", "2c")
						&& !__input_string_contains(_desc, "zero 2")
					) {
						__raw_type = "CommunityLikeXBox";
					...
					__INPUT_ON_IOS
					|| __input_string_contains(_desc, "nimbus", "horipad ultimate", "mfi")
				) {
					__raw_type = "AppleController";
				} else if (__input_string_contains(_desc, "throttle", "flight quadrant")) {
					__raw_type = "SDLThrottle";
				} else if (
					__input_string_contains(_desc, "flightstick", "hotas")
					|| (
						__input_string_contains(_desc, "flight", "sim", "eclipse")
							&& __input_string_contains(_desc, "stick", "yoke", "rudder")
					)
				) {
					...
					|| (
						__input_string_contains(_desc, "wheel")
							&& __input_string_contains(
								_desc,
								"racing",
								"steering",
				...
				if (!INPUT_SDL2_ALLOW_NONGAMEPAD_JOYSTICKS) {
					...
					if (__input_string_contains(_desc, "drum")) {
						...
						__input_string_contains(_desc, "ddr", "dance")
						&& !__input_string_contains(_desc, "hyperkin")
					) {
						__raw_type += "Dancepad";
					}
				}
			...
			switch (_vendor_and_product) {
				...
				case "3412adbe":
					if (
						((__button_count == 12) || (__button_count == 23))
						&& (__axis_count == 6)
						&& (__hat_count == 0)
						&& __INPUT_ON_WINDOWS
					) {
				...
				case "63257505":
					if (__INPUT_ON_WINDOWS || __INPUT_ON_LINUX) {
						...
						if ((__button_count == 17) && (__hat_count == 0)) {
							...
							if (
								(_desc == "USB ")
								|| (_desc == "SWITCH CO.,LTD. Retro-bit Controller")
							) {
								...
								__input_string_contains(_desc, "Controller (Dinput)")
							) {
					...
					if (
						...
						|| (
							(__INPUT_ON_WINDOWS || __INPUT_ON_LINUX)
								&& (__button_count == 25)
								&& (__axis_count == 6)
								&& (__hat_count == 0) //Windows and Linux identity
								&& (
									(
										__input_string_contains(
											gamepad_get_guid(__index + 1),
											"8f0e"
										)
											&& __input_string_contains(
												gamepad_get_guid(__index + 1),
									...
									) //Port comes in pairs, look ahead
										|| (
											__input_string_contains(
												gamepad_get_guid(__index - 1),
												"8f0e"
						...
						//Port comes in pairs, look behind
						if (!__INPUT_SILENT) {
							__input_trace(
								"Overriding controller ",
								__index,
				...
				case "00000000":
					if (
						(__description == "Wireless Gamepad")
						&& (__button_count == 16)
						&& (__axis_count == 4)
						&& (__hat_count == 1)
						&& __INPUT_ON_WINDOWS
								...
								case "HIDWiiClassic":
									...
									repeat (__index) {
										...
										if (gamepad_is_connected(_g)) {
											...
											if (
												(
													_global.__gamepads[@ _g].__raw_type
														== "HIDWiiRemote"
												)
												|| (
													_global.__gamepads[@ _g].__raw_type
								...
								case "HIDWiiRemote":
									...
									if (
										...
										if (
											...
											&& (
												_global.__gamepads[@ _g].__button_count
													== 0
											)
