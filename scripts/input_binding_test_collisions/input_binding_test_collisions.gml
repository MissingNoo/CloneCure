...
function input_binding_test_collisions(
	_verb_name,
	_src_binding,
	_player_index = 0,
	_profile_name = undefined
) {
		...
		repeat (array_length(_global.__basic_verb_array)) {
			...
			if (_group_matches) {
				...
				repeat (INPUT_MAX_ALTERNATE_BINDINGS) {
					...
					var _extant_binding = __binding_get(
						_profile_name,
						_verb,
						_alternate_index,
						false
					...
					if (is_struct(_extant_binding)) {
						if (
							(_extant_binding.__label == _src_binding.__label)
							&& (
								(_global.__source_mode != INPUT_SOURCE_MODE.MULTIDEVICE)
									|| (
										_extant_binding.__gamepad_index
											== _src_binding.__gamepad_index
									)
									|| (_extant_binding.__gamepad_index == undefined)
									|| (_src_binding.__gamepad_index == undefined)
							)
						) {
							array_push(
								_output_array,
								{__verb: _verb, __alternate: _alternate_index}
							);
						}
					}
