...
function scribble_super_glyph_copy(_target, _source, _overwrite) {
	...
	repeat (array_length(_work_array)) {
		...
		repeat (1 + _glyph_range_array[1] - _unicode) {
			__scribble_glyph_duplicate(
				_source_glyphs_map,
				_source_glyphs_data_grid,
				_target_glyphs_map,
				_target_glyph_data_grid,
				_unicode,
				_overwrite
			);
			++_unicode;
	...
	ds_grid_set_region(
		...
		ds_grid_width(_target_glyph_data_grid),
		SCRIBBLE_GLYPH.FONT_HEIGHT,
...
function __scribble_super_glyph_copy_common(_target_font_data, _source_font_data) {
	if (_source_font_data.__sdf == undefined) {
		__scribble_error(
			"Cannot determine if the source font is an SDF font. Please add glyphs to it"
		);
		...
		if (_target_font_data.__sdf == false) {
			__scribble_error(
				"Cannot mix standard/sprite fonts with SDF fonts (target is not an SDF font)"
			);
		}

		if (_source_font_data.__sdf == false) {
			__scribble_error(
				"Cannot mix standard/sprite fonts with SDF fonts (source is not an SDF font)"
			);
		}

		if (_source_font_data.__sdf_pxrange == undefined) {
			__scribble_error(
				"Source font's SDF pxrange must be defined before copying glyphs"
			);
		}

		if (
			(_target_font_data.__sdf_pxrange != undefined)
			&& (_target_font_data.__sdf_pxrange != _source_font_data.__sdf_pxrange)
		) {
			__scribble_error(
				"SDF font pxrange must match (target = ",
				_target_font_data.__sdf_pxrange,
				" vs. source = ",
				_source_font_data.__sdf_pxrange,
				")"
			);
		}
	}
...
function __scribble_glyph_duplicate(
	_source_map,
	_source_grid,
	_target_map,
	_target_grid,
