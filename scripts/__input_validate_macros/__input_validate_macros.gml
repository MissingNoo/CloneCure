...
function __input_validate_macros() {
	...
	if (
		(INPUT_STARTING_SOURCE_MODE != INPUT_SOURCE_MODE.FIXED)
		...
		__input_error(
			"INPUT_STARTING_SOURCE_MODE must be a member of the INPUT_SOURCE_MODE enum"
		);
	}
	...
	if (
		!is_numeric(INPUT_MAX_ALTERNATE_BINDINGS)
		|| (floor(INPUT_MAX_ALTERNATE_BINDINGS) != INPUT_MAX_ALTERNATE_BINDINGS)
		|| (INPUT_MAX_ALTERNATE_BINDINGS < 1)
	) {
		__input_error(
	...
	if ((INPUT_2D_XY_AXIS_BIAS < 0) || (INPUT_2D_XY_AXIS_BIAS > 1)) {
		__input_error(
			"INPUT_2D_XY_AXIS_BIAS should be between 0.0 and 1.0 (was ",
	...
	if (
		(INPUT_FALLBACK_PROFILE_BEHAVIOR != 0)
	...
	if (
		!is_string(INPUT_AUTO_PROFILE_FOR_KEYBOARD)
		&& !is_undefined(INPUT_AUTO_PROFILE_FOR_KEYBOARD)
	) {
		__input_error(
	...
	if (
		!is_string(INPUT_AUTO_PROFILE_FOR_MOUSE)
		&& !is_undefined(INPUT_AUTO_PROFILE_FOR_MOUSE)
	) {
		__input_error(
			"INPUT_AUTO_PROFILE_FOR_KEYBOARD must be a string (or <undefined> if the profile is to be unavailable)"
		);
	...
	if (
		!is_string(INPUT_AUTO_PROFILE_FOR_GAMEPAD)
		&& !is_undefined(INPUT_AUTO_PROFILE_FOR_GAMEPAD)
	) {
		__input_error(
	...
	if (
		!is_string(INPUT_AUTO_PROFILE_FOR_TOUCH)
		&& !is_undefined(INPUT_AUTO_PROFILE_FOR_TOUCH)
	) {
		__input_error(
	...
	if (
		!is_string(INPUT_AUTO_PROFILE_FOR_MIXED)
		&& !is_undefined(INPUT_AUTO_PROFILE_FOR_MIXED)
	) {
		__input_error(
	...
	if (
		!is_string(INPUT_AUTO_PROFILE_FOR_MULTIDEVICE)
		&& !is_undefined(INPUT_AUTO_PROFILE_FOR_MULTIDEVICE)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_MAX_PLAYERS)
		|| (floor(INPUT_MAX_PLAYERS) != INPUT_MAX_PLAYERS)
		|| (INPUT_MAX_PLAYERS < 1)
	) {
		__input_error(
	...
	if (
		!is_string(INPUT_CONTROLLER_TYPE_PATH)
		&& !is_undefined(INPUT_CONTROLLER_TYPE_PATH)
	) {
	...
	if (
		(INPUT_IGNORE_RESERVED_KEYS_LEVEL != 0)
	...
	if (
		!is_numeric(INPUT_MAX_TOUCHPOINTS)
		|| (floor(INPUT_MAX_TOUCHPOINTS) != INPUT_MAX_TOUCHPOINTS)
		|| (INPUT_MAX_TOUCHPOINTS < 1)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_TOUCH_HISTORY_FRAMES)
		|| (floor(INPUT_TOUCH_HISTORY_FRAMES) != INPUT_TOUCH_HISTORY_FRAMES)
		|| (INPUT_TOUCH_HISTORY_FRAMES < 1)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_VIRTUAL_BUTTON_MIN_THRESHOLD)
		|| (INPUT_VIRTUAL_BUTTON_MIN_THRESHOLD < 0)
		|| (INPUT_VIRTUAL_BUTTON_MIN_THRESHOLD >= INPUT_VIRTUAL_BUTTON_MAX_THRESHOLD)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_VIRTUAL_BUTTON_MAX_THRESHOLD)
		|| (INPUT_VIRTUAL_BUTTON_MAX_THRESHOLD < 0)
		|| (INPUT_VIRTUAL_BUTTON_MAX_THRESHOLD < INPUT_VIRTUAL_BUTTON_MIN_THRESHOLD)
	) {
		__input_error(
	...
	if (
		(INPUT_VIRTUAL_KEYBOARD_AUTOCAPITALIZATION_TYPE != kbv_autocapitalize_none)
		...
		__input_error(
			"INPUT_VIRTUAL_KEYBOARD_AUTOCAPITALIZATION_TYPE must be a virtual keyboard autocapitalization type constant"
		);
	}
	...
	if (
		!is_numeric(INPUT_DEFAULT_AXIS_MIN_THRESHOLD)
		|| (INPUT_DEFAULT_AXIS_MIN_THRESHOLD < 0)
		|| (INPUT_DEFAULT_AXIS_MIN_THRESHOLD >= INPUT_DEFAULT_AXIS_MAX_THRESHOLD)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_DEFAULT_AXIS_MAX_THRESHOLD)
		|| (INPUT_DEFAULT_AXIS_MAX_THRESHOLD > 1)
		|| (INPUT_DEFAULT_AXIS_MAX_THRESHOLD < INPUT_DEFAULT_AXIS_MIN_THRESHOLD)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_DEFAULT_TRIGGER_MIN_THRESHOLD)
		|| (INPUT_DEFAULT_TRIGGER_MIN_THRESHOLD < 0)
		|| (INPUT_DEFAULT_TRIGGER_MIN_THRESHOLD >= INPUT_DEFAULT_TRIGGER_MAX_THRESHOLD)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_DEFAULT_TRIGGER_MAX_THRESHOLD)
		|| (INPUT_DEFAULT_TRIGGER_MAX_THRESHOLD > 1)
		|| (INPUT_DEFAULT_TRIGGER_MAX_THRESHOLD < INPUT_DEFAULT_TRIGGER_MIN_THRESHOLD)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_ANDROID_GAMEPAD_ENUMERATION_INTERVAL)
		|| (INPUT_ANDROID_GAMEPAD_ENUMERATION_INTERVAL < 1)
	) {
		__input_error(
	...
	if (
		(INPUT_GYRO_DEFAULT_AXIS_X != INPUT_GYRO.AXIS_PITCH)
		&& (INPUT_GYRO_DEFAULT_AXIS_X != INPUT_GYRO.AXIS_YAW)
		&& (INPUT_GYRO_DEFAULT_AXIS_X != INPUT_GYRO.AXIS_ROLL)
	) {
		__input_error(
			"INPUT_GYRO_DEFAULT_AXIS_X must be either INPUT_GYRO.AXIS_PITCH, INPUT_GYRO.AXIS_YAW, or INPUT_GYRO.AXIS_ROLL"
		);
	}

	if (
		(INPUT_GYRO_DEFAULT_AXIS_Y != INPUT_GYRO.AXIS_PITCH)
		&& (INPUT_GYRO_DEFAULT_AXIS_Y != INPUT_GYRO.AXIS_YAW)
		&& (INPUT_GYRO_DEFAULT_AXIS_Y != INPUT_GYRO.AXIS_ROLL)
	) {
		__input_error(
			"INPUT_GYRO_DEFAULT_AXIS_Y must be either INPUT_GYRO.AXIS_PITCH, INPUT_GYRO.AXIS_YAW, or INPUT_GYRO.AXIS_ROLL"
		);
	}
	...
	if (
		!is_numeric(INPUT_VIBRATION_DEFAULT_STRENGTH)
		|| (INPUT_VIBRATION_DEFAULT_STRENGTH < 0)
		|| (INPUT_VIBRATION_DEFAULT_STRENGTH > 1.0)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_VIBRATION_JOYCON_STRENGTH)
		|| (INPUT_VIBRATION_JOYCON_STRENGTH < 0)
		|| (INPUT_VIBRATION_JOYCON_STRENGTH > 1.0)
	) {
		__input_error(
	...
	if (
		!is_numeric(INPUT_TRIGGER_EFFECT_DEFAULT_STRENGTH)
		|| (INPUT_TRIGGER_EFFECT_DEFAULT_STRENGTH < 0)
		|| (INPUT_TRIGGER_EFFECT_DEFAULT_STRENGTH > 1.0)
	) {
		__input_error(
