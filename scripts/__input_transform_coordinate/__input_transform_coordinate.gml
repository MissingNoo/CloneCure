...
function __input_transform_coordinate(
	_x,
	_y,
	_input_system,
	_output_system,
	...
	if (_input_system != _output_system) {
		//Only do MATHS if the output system is different
		...
		if ((_input_system == 0) || (_output_system == 0)) {
			...
			if ((_camera == undefined) && view_enabled && view_visible[0]) {
		...
		if (_input_system == 0) {
			//Input coordinate system is room-space
			if (_viewA == 0) {
				//Skip expensive rotation step if we can
		...
		} else if (_input_system == 1) {
			//Input coordinate system is GUI-space
		...
		} else if (_input_system == 2) {
			//Input coordinate system is device-space
