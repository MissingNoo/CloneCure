...
function ScribblejrReprocess() {
	...
	repeat (array_length(_elementsArray)) {
		//We add elements to the cache during this sweep - make sure we don't process new elements
		var _element = _elementsArray[_i];

		if (
			is_instanceof(_element, __ScribblejrClassExt)
				...
				if ((_wrapperRef != undefined) && weak_ref_alive(_wrapperRef)) {
					...
					if (_newString != _element.__string) {
						...
						if (is_instanceof(self, __ScribblejrClassExt)) {
							var _newElement = new __ScribblejrClassExt(
								__key,
								__stringOriginal,
								__hAlign,
								__vAlign,
							...
							var _newElement = new __ScribblejrClassExtFit(
								__key,
								__stringOriginal,
								__hAlign,
								__vAlign,
							...
							var _newElement = new __ScribblejrClassExtShrink(
								__key,
								__stringOriginal,
								__hAlign,
								__vAlign,
							...
							__ScribblejrError(
								$"Could not recreate text element, unrecognised instanceof ({
									instanceof(self)
								})"
							);
						}
