...
function __scribble_class_model(_element, _model_cache_name) constructor {
	...
	static __mcache_name_array = __scribble_initialize()
		.__cache_state
		.__mcache_name_array;
	...
	if (__SCRIBBLE_DEBUG) {
		__scribble_trace("Caching model \"", __cache_name, "\"");
	}
	...
	static __submit = function(_page, _double_draw) {
		...
		__pages_array[_page].__submit(
			(SCRIBBLE_ALWAYS_DOUBLE_DRAW || __has_arabic || __has_thai) && _double_draw
		);
	};
	...
	static __flush = function() {
		...
		if (__SCRIBBLE_DEBUG) {
			__scribble_trace("Flushing model \"" + string(__cache_name) + "\"");
		}
	...
	static __reset = function() {
		if (__SCRIBBLE_DEBUG) {
			__scribble_trace("Resetting model \"" + string(__cache_name) + "\"");
		}
	...
	/// @param endCharacter
	static __get_bbox_revealed = function(
		_page,
		_in_start,
		_in_end,
		_padding_l,
		...
		if (_end < 0) {
			...
			var _top = ds_grid_get_min(
				_glyph_grid,
				_start,
				__SCRIBBLE_GLYPH_LAYOUT.__TOP,
				_end,
				__SCRIBBLE_GLYPH_LAYOUT.__TOP
			);
			var _right = ds_grid_get_max(
				_glyph_grid,
				_start,
				__SCRIBBLE_GLYPH_LAYOUT.__RIGHT,
				_end,
				__SCRIBBLE_GLYPH_LAYOUT.__RIGHT
			);
			var _bottom = ds_grid_get_max(
				_glyph_grid,
				_start,
				__SCRIBBLE_GLYPH_LAYOUT.__BOTTOM,
				_end,
				__SCRIBBLE_GLYPH_LAYOUT.__BOTTOM
			);
		}
