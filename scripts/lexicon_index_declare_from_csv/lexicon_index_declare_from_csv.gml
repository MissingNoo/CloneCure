...
// Feather ignore all
function lexicon_index_declare_from_csv(
	_filePath,
	_cellDelimiter = ",",
	_stringDelimiter = "\""
) {
	...
	for (var _j = 1; _j < _height; ++_j) {
		...
		if (is_array(_locale)) {
			...
			repeat (array_length(_locale)) {
				/* Feather ignore once GM1061 */
				if (
					!variable_struct_exists(__LEXICON_STRUCT.localeMap, _locale[_i])
					&& variable_struct_exists(__LEXICON_STRUCT.languageMap, _language)
				) {
				...
				_file = new __lexicon_file(
					_language,
					_locale[0],
					_fileTypes.csv.parser,
					_filePath,
					false,
					[_cellDelimiter, _stringDelimiter]
				);
				_langStruct.files[array_length(_langStruct.files)] = _file;
			}
		} else {
			if (
				!(is_undefined(_language) || is_undefined(_locale))
				&& (
					variable_struct_exists(__LEXICON_STRUCT.languageMap, _language)
