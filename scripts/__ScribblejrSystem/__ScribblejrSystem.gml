...
function __ScribblejrSystem() {
	...
	__ScribblejrTrace(
		"Welcome to Scribble Junior by Juju Adams! This is version ",
		SCRIBBLEJR_VERSION,
		", ",
		SCRIBBLEJR_DATE
	);
	...
	if (GM_build_type == "run") {
		global.scribblejrSystem = _system;
	}
			...
			function() {
				...
				with (_system) {
					...
					repeat (sqrt(array_length(_array))) {
						_index = (_index + 1) % array_length(_array);

						var _element = _array[_index];
						if (
							(!weak_ref_alive(_element.__wrapper))
							&& (current_time > _element.__lastDraw + __SCRIBBLEJR_TIMEOUT)
						) {
							if (SCRIBBLEJR_VERBOSE) {
								__ScribblejrTrace("Freeing ", _element.__key);
							}
