function __ScribblejrClassBase() constructor {
	...
	static __Bake = function() {
		if (__fontIsDynamic) {
			return;
		} //Don't bake dynamic fonts
		while (!__BakeVertexBuffer()) {
			__BakeVertexBuffer();
		}
	};
	...
	static __BakeVertexBufferTimed = function() {
		...
		if (_system.__budgetUsed >= _system.__budget) {
			return;
		} //Don't exceed the baking budget
	...
	static __BakeVertexBuffer = function() {
		...
		if (__vertexBaker.__tickMethod()) {
			if (SCRIBBLEJR_VERBOSE) {
				__ScribblejrTrace("Compiled ", self);
			}
			__vertexBuffer = __vertexBaker.__vertexBuffer;
			Draw = ScribblejrCacheFontInfo(__font).sdfEnabled
				? __DrawVertexBufferSDF
				: __DrawVertexBuffer;
