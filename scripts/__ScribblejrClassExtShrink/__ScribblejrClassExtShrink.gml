...
function __ScribblejrClassExtShrink(
	_key,
	_string,
	_hAlign,
	_vAlign,
	...
	if (string_count("[", _string) <= 0) {
		...
		array_push(
			__fragmentArray,
			{__colour: -1, __string: __string, __x: _xOffset, __y: _yOffset}
		);
	} else {
		...
		repeat (_lineCount) {
			...
			var _width = __ScribblejrStringFragment(
				_lineArray[_lineIndex],
				_lineHeight,
				(SCRIBBLEJR_SCALE_SPRITES ? 1 : (1 / _fontScale))
					/ SCRIBBLEJR_GLOBAL_FONT_SCALE, //Sprite scale
				_colour,
				_fragmentArray,
				_spriteArray
			);
	...
	static __DrawSimple = function(
		...
		if (_sdfEffects != undefined) {
			if (
				...
				__DrawSprites(
					_x + _sdfEffects.dropShadowOffsetX,
	...
	static __DrawSimpleScaled = function(
		_x,
		_y,
		_colour = c_white,
		_alpha = 1,
		...
		if (_sdfEffects != undefined) {
			if (
				...
				gpu_set_fog(true, _sdfEffects.dropShadowColour, 0, 0);
				__DrawSprites(
					_x + __scale * _sdfEffects.dropShadowOffsetX,
