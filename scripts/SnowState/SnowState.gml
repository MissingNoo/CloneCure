...
function SnowState(_initState, _execEnter = true) constructor {
	...
	__defaultEvents = {
		...
		enter: {exists: SNOWSTATE_EVENT.NOT_DEFINED, func: function() {}},
		leave: {exists: SNOWSTATE_EVENT.NOT_DEFINED, func: function() {}},
	};
	__invalidStateNames = [
		// It is what it is
	...
	__add_event_method = function(_event) {
		var _temp = {exec: __execute, event: _event};
	...
	__assert_state_name_valid = function(_state, _error = true) {
		...
		if (!is_string(_state) || (_state == "")) {
			if (_func != undefined) {
				_func("State name should be a non-empty string.");
			}
			return false;
		}
		...
		var _name, _i;
		_i = 0;
		repeat (array_length(__invalidStateNames)) {
			_name = __invalidStateNames[_i];
			++_i;
			if (_state == _name) {
				if (_func != undefined) {
					_func("State name can not be \"", _name, "\".");
				}
				return false;
			}
		}
	...
	__assert_transition_name_valid = function(_state, _error = true) {
		...
		if (!is_string(_state) || (_state == "")) {
			if (_func != undefined) {
				_func("Transition name should be a non-empty string.");
			}
			return false;
		}
	...
	__func_exec = function(_func, _args = undefined) {
		...
		switch (array_length(_args)) {
			...
			default:
				__snowstate_error("Can't use more than 16 arguments.");
				break;
		}
	...
	__set_default_event = function(_event, _method, _defined) {
		__defaultEvents[$ _event] = {exists: _defined, func: _method};
	...
	__transition_exists = function(_transitionName, _from) {
		if (_from == SNOWSTATE_WILDCARD_TRANSITION_NAME) {
			...
			// Default
			if (
				variable_struct_exists(__transitions, _from)
				&& variable_struct_exists(__transitions[$ _from], _transitionName)
			) {
				return SNOWSTATE_TRIGGER.DEFINED;
			}
	...
	__trigger = function(_transitionName, _data) {
		if (!__assert_transition_name_valid(_transitionName)) {
			return false;
		}
		...
		if (__transition_exists(_transitionName, _source) == SNOWSTATE_TRIGGER.DEFINED) {
			if (
				__try_triggers(
					__transitions[$ _source][$ _transitionName],
					_currState,
					_transitionName,
					_data
				)
		...
		// Wild triggers
		if (
			__transition_exists(_transitionName, SNOWSTATE_WILDCARD_TRANSITION_NAME)
			== SNOWSTATE_TRIGGER.DEFINED
		) {
			if (
				__try_triggers(
					__wildTransitions[$ _transitionName],
		...
		while (variable_struct_exists(__parent, _source)) {
			...
			if (
				__transition_exists(_transitionName, _source) == SNOWSTATE_TRIGGER.DEFINED
			) {
				if (
					__try_triggers(
						__transitions[$ _source][$ _transitionName],
	...
	__try_triggers = function(_transitions, _source, _trigger, _data) {
		...
		_i = 0;
		repeat (array_length(_transitions)) {
			_transition = _transitions[_i];
			++_i;

			// For reflexive wildcard transitions, change to source
			_dest = _transition.to;
			if (_dest == SNOWSTATE_REFLEXIVE_TRANSITION_NAME) {
				_dest = _source;
			}
	...
	__update_events_from_parent = function(_name) {
		...
		repeat (array_length(_events)) {
			...
			switch (_parEvent.exists) {
				case SNOWSTATE_EVENT.DEFINED:
					_exists = SNOWSTATE_EVENT.INHERITED;
					break;
				case SNOWSTATE_EVENT.INHERITED:
					_exists = SNOWSTATE_EVENT.INHERITED;
					break;
				case SNOWSTATE_EVENT.DEFAULT:
					_exists = SNOWSTATE_EVENT.DEFAULT;
					break;
				default:
					break;
			}

			_state[$ _event] = {exists: _exists, func: _parEvent.func};
			++_i;
		}
	...
	add = function(_name, _struct = {}) {
		if (!__assert_state_name_valid(_name)) {
			return undefined;
		}
		...
		if (SNOWSTATE_DEBUG_WARNING && __is_state_defined(_name)) {
			__snowstate_trace(
				"State \"",
				_name,
				"\" has been defined already. Replacing the previous definition."
			);
		}
	...
	change = function(_state, _leave = undefined, _enter = undefined, _data = undefined) {
		if ((_leave != undefined) && !__is_really_a_method(_leave)) {
			__snowstate_error(
				"Invalid value for \"leave_func\" in change(). Should be a function."
			);
			return undefined;
		}
		...
		if ((_enter != undefined) && !__is_really_a_method(_enter)) {
			__snowstate_error(
				"Invalid value for \"enter_func\" in change(). Should be a function."
			);
			return undefined;
		}
	...
	state_is = function(_target, _source = get_current_state()) {
		...
		if (!__assert_state_name_valid(_target)) {
			return false;
		}
		if (!__assert_state_name_valid(_source)) {
			return false;
		}
		...
		while (_state != undefined) {
			...
			_state = variable_struct_exists(__parent, _state)
				? __parent[$ _state]
				: undefined;
		}
	...
	set_time = function(_time, _us = true) {
		...
		__stateStartTime =
			get_timer()
			- (_us ? _time : (_time * 1 / game_get_speed(gamespeed_fps) * 1000000));
	...
	add_child = function(_parent, _name, _struct = {}) {
		...
		if (!__assert_state_name_valid(_parent)) {
			return undefined;
		}
	...
	inherit = function() {
		...
		if (SNOWSTATE_CIRCULAR_INHERITANCE_ERROR) {
			...
			if (_i < _len) {
				...
				_str += " -> " + string(_state);
				__snowstate_error(
					"Circular inheritance found. Inheritance chain: ",
		...
		if (array_length(__childQueue) > 0) {
			__history[@ 0] = array_pop(__childQueue);
		}
	...
	event_set_default_function = function(_event, _function) {
		if (SNOWSTATE_DEBUG_WARNING && (variable_struct_names_count(__states) > 0)) {
			__snowstate_trace(
				"event_set_default_function() should be called before defining any state."
			);
		}
	...
	event_exists = function(_event) {
		...
		if (!variable_struct_exists(__states[$ _state], _event)) {
			return SNOWSTATE_EVENT.NOT_DEFINED;
		}
	...
	add_transition = function(
		...
		if (_dest == SNOWSTATE_WILDCARD_TRANSITION_NAME) {
			__snowstate_error(
				"Destination state name can not be the same as SNOWSTATE_WILDCARD_TRANSITION_NAME."
			);
			return undefined;
		}
		...
		if (!__is_really_a_method(_condition)) {
			__snowstate_error(
				"Invalid value for \"condition\" in add_transition(). Should be a function."
			);
			return undefined;
		}
		if (!__is_really_a_method(_leave)) {
			__snowstate_error(
				"Invalid value for \"leave_func\" in add_transition(). Should be a function."
			);
			return undefined;
		}
		if (!__is_really_a_method(_enter)) {
			__snowstate_error(
				"Invalid value for \"enter_func\" in add_transition(). Should be a function."
			);
			return undefined;
		}
		...
		repeat (array_length(_source)) {
			...
			if (!is_string(_from) || (_from == "")) {
				if (SNOWSTATE_DEBUG_WARNING) {
					__snowstate_trace(
						"State name should be a non-empty string. Transition not added."
					);
				}
			} else if (_from == SNOWSTATE_REFLEXIVE_TRANSITION_NAME) {
				if (SNOWSTATE_DEBUG_WARNING) {
					__snowstate_trace(
						"Source state name can not be the same as SNOWSTATE_REFLEXIVE_TRANSITION_NAME. Transition not added."
					);
				}
			} else {
				__add_transition(
	...
	/// @param {function} [enter_func]
	/// @returns {SnowState} self
	add_wildcard_transition = function(
		_transitionName,
		_dest,
		_condition = function() {
			return true;
	...
	/// @param {function} [enter_func]
	/// @returns {SnowState} self
	add_reflexive_transition = function(
		_transitionName,
		_source,
		_condition = function() {
			return true;
	...
	transition_exists = function(_transitionName, _source) {
		if (!is_string(_transitionName)) {
			return false;
		}
		if (!is_string(_source)) {
			return false;
		}
		if (_source == SNOWSTATE_WILDCARD_TRANSITION_NAME) {
			return true;
		}

		if (!__assert_transition_name_valid(_transitionName, false)) {
			return false;
		}
	...
	trigger = function(_transition, _data = undefined) {
		if (is_array(_transition)) {
			var _i = 0;
			repeat (array_length(_transition)) {
				if (__trigger(_transition[_i], _data)) {
					return true;
				}
				++_i;
			}
	...
	history_set_max_size = function(_size) {
		...
		if (_size < 0) {
			if (SNOWSTATE_DEBUG_WARNING) {
				__snowstate_trace(
					"History size should non-negative. Setting the size to 0 instead of ",
					_size,
					"."
				);
			}
	...
	history_get = function() {
		...
		if (get_previous_state() == undefined) {
			return [__get_current_state()];
		}
...
// Startup errors
if (
	!is_string(SNOWSTATE_WILDCARD_TRANSITION_NAME)
...
if (
	!is_string(SNOWSTATE_REFLEXIVE_TRANSITION_NAME)
	|| (string_length(SNOWSTATE_REFLEXIVE_TRANSITION_NAME) != 1)
) {
	var _str = "[SnowState]\n";
...
show_debug_message(
	"[SnowState] You are using SnowState by @sohomsahaun (Version: "
		+ string(SNOWSTATE_VERSION)
		+ " | Date: "
		+ string(SNOWSTATE_DATE)
		+ ")"
);
