...
function __input_initialize() {
	...
	if (__INPUT_SILENT) {
		__input_trace(
			"Warning! Per __INPUT_SILENT mode, most logging is suppressed. This is NOT recommended"
		);
	}
	...
	try {
		var _split = array_equals(
			string_split("Juju\nwaz\nere", "\n", true),
			["Juju", "waz", "ere"]
		);
		var _trim =
			string_trim("         you can't catch me          ") == "you can't catch me";
	...
	if (INPUT_ON_WEB) {
		...
		try {
			var _instance_of = is_instanceof(
				input_binding_empty(),
	...
	try {
		_global.__use_gp_extended =
			is_numeric(gp_home)
			&& is_numeric(gp_extra1)
			&& is_numeric(gp_paddler)
			&& is_numeric(gp_paddlel)
			&& is_numeric(gp_paddlerb)
			&& is_numeric(gp_paddlelb)
			&& is_numeric(gp_touchpadbutton);
	} catch (_error) {
		_global.__use_gp_extended = false;
	}
	...
	if (!__INPUT_SILENT) {
		__input_trace(
			_global.__use_debug_overlay_status
	...
	if (!__INPUT_SILENT) {
		__input_trace(
			_global.__allow_gamepad_tester
	...
	if (__INPUT_ON_ANDROID) {
		...
		if (!__INPUT_SILENT) {
			__input_trace(
				_global.__allow_gamepad_enumerate
					? "Using gamepad enumeration"
					: "Gamepad enumeration is unavailable"
			);
		}
	}
	...
	_global.__time_source = time_source_create(
		time_source_global,
		1,
		...
		function() {
			...
			if (!instance_exists(input_controller_object)) {
				...
				if (instance_exists(input_controller_object)) {
					...
					instance_create_depth(
						0,
						-__INPUT_CONTROLLER_OBJECT_DEPTH,
						__INPUT_CONTROLLER_OBJECT_DEPTH,
			...
			if (input_controller_object.depth != __INPUT_CONTROLLER_OBJECT_DEPTH) {
				if (GM_build_type == "run") {
					//Be nasty when running from the IDE >:(
					__input_error(
						"input_controller_object depth has been changed (expected ",
						__INPUT_CONTROLLER_OBJECT_DEPTH,
						", got ",
						input_controller_object.depth,
	...
	if (
		(string_pos("127.0.0.1", parameter_string(0)) > 0)
		|| (string_pos("localhost", parameter_string(0)) > 0)
	) {
		show_message(
	...
	_global.__gamepad_allowed =
		(INPUT_ON_PC && INPUT_PC_GAMEPAD)
		|| (INPUT_ON_MOBILE && INPUT_MOBILE_GAMEPAD)
		|| INPUT_ON_CONSOLE;
	_global.__keyboard_allowed =
		(INPUT_ON_PC && INPUT_PC_KEYBOARD)
		|| (__INPUT_ON_SWITCH && INPUT_SWITCH_KEYBOARD)
		|| (INPUT_ON_MOBILE && INPUT_MOBILE_WEB_KEYBOARD && INPUT_ON_WEB)
		|| (__INPUT_ON_ANDROID && INPUT_ANDROID_KEYBOARD);
	_global.__mouse_allowed =
		(INPUT_ON_PC && INPUT_PC_MOUSE)
		|| (__INPUT_ON_SWITCH && INPUT_SWITCH_MOUSE)
		|| (INPUT_ON_MOBILE && INPUT_MOBILE_MOUSE);
	_global.__touch_allowed =
		INPUT_ON_MOBILE || (__INPUT_ON_SWITCH && INPUT_SWITCH_TOUCH);

	//No gamepad, no vibration
	_global.__vibration_allowed = false;
	if (_global.__gamepad_allowed) {
		_global.__vibration_allowed =
			(__INPUT_ON_WINDOWS && INPUT_WINDOWS_VIBRATION)
			|| (__INPUT_ON_SWITCH && INPUT_SWITCH_VIBRATION)
			|| (__INPUT_ON_XBOX && INPUT_XBOX_VIBRATION)
			|| ((os_type == os_ps4) && INPUT_PS4_VIBRATION)
			|| ((os_type == os_ps5) && INPUT_PS5_VIBRATION);
	}

	//Mouse overrides touch
	if (_global.__mouse_allowed && _global.__touch_allowed) {
		_global.__touch_allowed = false;
		if (!INPUT_MOUSE_ALLOW_VIRTUAL_BUTTONS) {
			if (!__INPUT_SILENT) {
				__input_trace(
					"Warning! Mouse configuration overrides touch. Virtual buttons may not work as expected."
				);
			}
		}
	}
	...
	_global.__sdl2_database = {by_guid: {}, by_vendor_product: {}, by_description: {}};
	...
	if (!__INPUT_SDL2_SUPPORT || !INPUT_SDL2_REMAPPING) {
		...
		if (INPUT_SDL2_ALLOW_EXTERNAL) {
			...
			if (_external_string != "") {
				if (
					!extension_exists("Steamworks")
					&& (GM_build_type == "run")
					&& (environment_get_variable("SteamEnv") == "1")
					&& (environment_get_variable("EnableConfiguratorSupport") != "0")
	...
	_dict[$ INPUT_GAMEPAD_TYPE_SWITCH] = [
		//Switch
	...
	_dict[$ INPUT_GAMEPAD_TYPE_XBOX_360] = [
		//Xbox 360
		[true, false, false, false], //P1: X---
	...
	if (__INPUT_ON_WINDOWS || INPUT_ON_WEB) {
		...
		for (var _i = 0x0D; _i <= 0x20; _i++) {
			__input_key_name_set(_i + vk_f1 - 1, "f" + string(_i));
		}
	...
	if (
		__INPUT_STEAMWORKS_SUPPORT
		&& INPUT_ALLOW_STEAMWORKS
		&& extension_exists("Steamworks")
	) {
		try {
	...
	if (!_global.__on_steam_deck) {
		...
		if (_deck_envar != "") {
			...
			if (ds_exists(_map, ds_type_map)) {
				...
				if (__INPUT_ON_LINUX) {
					_identifier = _map[? "gl_renderer_string"];
				}
				if (__INPUT_ON_WINDOWS) {
					_identifier = _map[? "video_adapter_description"];
				}

				//Steam Deck GPU identifier
				if (
					(_identifier != undefined)
					&& __input_string_contains(_identifier, "AMD Custom GPU 0")
				) {
	...
	if (_global.__using_steamworks) {
		...
		_global.__steam_trigger_mode[$ string(__INPUT_TRIGGER_EFFECT.__TYPE_OFF)] =
			steam_input_sce_pad_trigger_effect_mode_off;
		_global.__steam_trigger_mode[$ string(__INPUT_TRIGGER_EFFECT.__TYPE_FEEDBACK)] =
			steam_input_sce_pad_trigger_effect_mode_feedback;
		_global.__steam_trigger_mode[$ string(__INPUT_TRIGGER_EFFECT.__TYPE_WEAPON)] =
			steam_input_sce_pad_trigger_effect_mode_weapon;
		_global.__steam_trigger_mode[$ string(__INPUT_TRIGGER_EFFECT.__TYPE_VIBRATION)] =
			steam_input_sce_pad_trigger_effect_mode_vibration;
	}

	if (__INPUT_ON_MACOS || __INPUT_ON_LINUX) {
		//Define the virtual controller's identity
		var _os = __INPUT_ON_MACOS ? "macos" : "linux";
		var _id = __INPUT_ON_MACOS
			? "030000005e0400008e02000001000000"
			: "03000000de280000ff11000001000000";

		//Access the blacklist
		var _blacklist_os = is_struct(_global.__blacklist_dictionary)
			? _global.__blacklist_dictionary[$ _os]
			: undefined;
		var _blacklist_id = is_struct(_blacklist_os)
			? _blacklist_os[$ "guid"]
			: undefined;
		...
		if (is_struct(_blacklist_os) && (_blacklist_id == undefined)) {
			...
			_blacklist_os[$ "guid"] = {};
			_blacklist_id = is_struct(_blacklist_os)
				? _blacklist_os[$ "guid"]
				: undefined;
		}
		...
		if (
			...
			&& (
				(_steam_environ != "")
					&& (_steam_environ == "1") || _global.__using_steamworks
			)
			&& (_steam_configs != "")
			...
			if (
				_global.__using_steamworks
				|| (environment_get_variable("SDL_GAMECONTROLLER_IGNORE_DEVICES") == "")
			) {
			...
			if (
				!_steam_generic
				&& !_steam_ps
				&& (!_steam_switch || _global.__steam_switch_labels)
			) {
				...
				_global.__simple_type_lookup[$ "UnknownNonSteamController"] =
					INPUT_GAMEPAD_TYPE_XBOX_ONE;
				if (!__INPUT_SILENT) {
					__input_trace(
						"Steam Input configuration indicates Xbox-like identity for virtual controllers"
					);
				}
			}
		}
	...
	if (INPUT_ON_CONSOLE || (INPUT_ON_WEB && !INPUT_ON_PC)) {
		...
		if (__INPUT_ON_ANDROID) {
			...
			if (ds_exists(_map, ds_type_map)) {
				...
				if (
					(string_pos("_cheets", _device) > 1)
					|| (
						(string_pos("cheets_", _device) > 0)
							&& (
								string_pos("cheets_", _device)
									< (string_length(_device) - 6)
							)
					)
				) {
	...
	if (
		__input_global().__on_steam_deck
		|| __INPUT_ON_SWITCH
		|| INPUT_ON_MOBILE
		|| (__INPUT_ON_WINDOWS && _global.__touch_allowed)
	) {
	...
	_global.__gamepad_motion_support =
		__INPUT_ON_PS
		|| __INPUT_ON_SWITCH
		|| _global.__using_steamworks;
	...
	_global.__source_mouse = INPUT_ASSIGN_KEYBOARD_AND_MOUSE_TOGETHER
		? INPUT_KEYBOARD
		: (new __input_class_source(__INPUT_SOURCE.MOUSE));
	...
	repeat (_max_gamepads) {
		_global.__source_gamepad[@ _g] = new __input_class_source(
			__INPUT_SOURCE.GAMEPAD,
			_g
		);
		++_g;
	}
	...
	if (INPUT_STARTING_SOURCE_MODE == INPUT_SOURCE_MODE.MIXED) {
		if (!variable_struct_exists(_global.__profile_dict, INPUT_AUTO_PROFILE_FOR_MIXED)) {
			__input_error(
				"Default profile for mixed \"",
				INPUT_AUTO_PROFILE_FOR_MIXED,
		...
		if (
			!variable_struct_exists(
				...
				INPUT_AUTO_PROFILE_FOR_MULTIDEVICE
			)
		) {
			__input_error(
				"Default profile for multidevice \"",
				INPUT_AUTO_PROFILE_FOR_MULTIDEVICE,
				"\" has not been defined in __input_config_verbs()"
			);
		}
