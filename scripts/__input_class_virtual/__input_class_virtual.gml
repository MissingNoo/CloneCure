...
function __input_class_virtual() constructor {
	...
	static get_threshold = function() {
		static _struct = {__mini: undefined, __maxi: undefined};
		...
		if (__record_history != is_array(__history_array)) {
			if (__record_history) {
				...
				repeat (INPUT_TOUCH_HISTORY_FRAMES + 1) {
					__history_array[@ _i] = {x: undefined, y: undefined};
	...
	static __set_as_background = function() {
		rectangle(
			0,
			0,
			max(display_get_width(), display_get_height()),
			max(display_get_width(), display_get_height())
		);
		__priority = -infinity;
		__background = true;
	...
	static __capture_touchpoint = function(_device) {
		...
		if (__first_touch_only && (_device > 0)) {
			return false;
		}
	...
	static __tick = function() {
		...
		if (!__held) {
			...
			var _sustain = __momentary
				? (__capture_frame == __global.__frame)
				: device_mouse_check_button(__touch_device, mb_left);
			...
			if (!_sustain) {
				...
				var _reference = (__type == INPUT_VIRTUAL_TYPE.TOUCHPAD)
				...
				if (_length <= 0) {
					...
					var _threshold_factor =
						clamp(
							(_length - __threshold_min)
								/ (__threshold_max - __threshold_min),
							0,
							1
						) / _length;
					...
					switch (__type) {
						...
						case INPUT_VIRTUAL_TYPE.DPAD_HORIZONTAL:
							if (
								...
								_player.__verb_set_from_virtual(
									__verb_right,
									1,
									1,
									false
						...
						case INPUT_VIRTUAL_TYPE.TOUCHPAD:
							...
							_player.__verb_set_from_virtual(
								__verb_left,
								max(0, -__normalized_x),
								max(0, -__normalized_x),
								true
							);
							_player.__verb_set_from_virtual(
								__verb_up,
								max(0, -__normalized_y),
								max(0, -__normalized_y),
								true
							);
							_player.__verb_set_from_virtual(
