...
function __scribble_gen_2_parser() {
	...
	_element_text =
		(_element.__preprocessorFunc ?? _system.__defaultPreprocessorFunc)(_element_text)
		?? _element_text;
	...
	if (ds_grid_width(_glyph_grid) < _element_expected_text_length) {
		ds_grid_resize(
			_glyph_grid,
			_element_expected_text_length,
			__SCRIBBLE_GEN_GLYPH.__SIZE
		);
	}
	...
	_control_grid[# _control_count, __SCRIBBLE_GEN_CONTROL.__TYPE] =
		__SCRIBBLE_GEN_CONTROL_TYPE.__HALIGN;
	_control_grid[# _control_count, __SCRIBBLE_GEN_CONTROL.__DATA] = _state_halign;
	++_control_count;

	_control_grid[# _control_count, __SCRIBBLE_GEN_CONTROL.__TYPE] =
		__SCRIBBLE_GEN_CONTROL_TYPE.__COLOUR;
	...
	while (true) {
		...
		if ((_glyph_ord & $E0) == $C0) {
			...
			_glyph_ord =
				((_glyph_ord & $1F) << 6)
				| (buffer_read(_string_buffer, buffer_u8) & $3F);
		} else if ((_glyph_ord & $F0) == $E0) {
			...
			if (
				(_glyph_ord == 0x5C)
				&& (
					buffer_peek(_string_buffer, buffer_tell(_string_buffer), buffer_u8)
						== 0x6E
				)
			) {
		...
		if (_tag_start != undefined) {
			...
			if (_glyph_ord == SCRIBBLE_COMMAND_TAG_CLOSE) {
				//If we've hit a command tag close character (usually ])
				_tag_open_count--;
				...
				if (_tag_open_count <= 0) {
					...
					repeat (_tag_parameter_count) {
						array_push(
							_tag_parameters,
							buffer_read(_string_buffer, buffer_string)
						);
					}
					...
					switch (_command_tag_lookup_accelerator_map[? _tag_command_name]) {
						...
						case 0:
							...
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__TYPE
							] = __SCRIBBLE_GEN_CONTROL_TYPE.__EFFECT;
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__DATA
							] = 0;
							++_control_count;
						...
						case 6:
							...
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__UNICODE
							] = 0x00;
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__BIDI] =
								__SCRIBBLE_BIDI.ISOLATED;
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__X] = 0;
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__Y] = 0;
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__WIDTH] = 0;
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__HEIGHT] =
								_font_line_height;
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__FONT_HEIGHT
							] = _font_line_height;
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__SEPARATION
							] = 0;
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__LEFT_OFFSET
							] = 0;
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT
							] = _control_count;
						...
						// [offset,dX,dY]
						case 38:
							var _offset_dx = (_tag_parameter_count > 1)
								? real(_tag_parameters[1])
								: 0;
							var _offset_dy = (_tag_parameter_count > 2)
								? real(_tag_parameters[2])
								: 0;
						...
						case 39:
							if (
								(_glyph_count > 0)
								&& (array_length(_offset_data_array) >= 3)
							) {
						...
						case 10:
							_state_colour =
								(floor(255 * clamp(real(_tag_parameters[1]), 0, 1)) << 24)
								| (_state_colour & 0x00FFFFFF);
						...
						// [&nbsp;]
						case 21:
							repeat ((array_length(_tag_parameters) == 2)
								? real(_tag_parameters[1])
								: 1) {
								_glyph_grid[#
									_glyph_count,
									__SCRIBBLE_GEN_GLYPH.__UNICODE
								] = 0xA0; //Non-breaking space (dec = 160)
								_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__BIDI] =
									__SCRIBBLE_BIDI.SYMBOL;
								_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__X] = 0;
								_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__Y] = 0;
								_glyph_grid[#
									_glyph_count,
									__SCRIBBLE_GEN_GLYPH.__WIDTH
								] = _font_space_width;
								_glyph_grid[#
									_glyph_count,
									__SCRIBBLE_GEN_GLYPH.__HEIGHT
								] = _font_line_height;
								_glyph_grid[#
									_glyph_count,
									__SCRIBBLE_GEN_GLYPH.__FONT_HEIGHT
						...
						// [cycle]
						case 22:
							var _cycle_r = (_tag_parameter_count > 1)
								? max(1, real(_tag_parameters[1]))
								: 0;
							var _cycle_g = (_tag_parameter_count > 2)
								? max(1, real(_tag_parameters[2]))
								: 0;
							var _cycle_b = (_tag_parameter_count > 3)
								? max(1, real(_tag_parameters[3]))
								: 0;
							var _cycle_a = (_tag_parameter_count > 4)
								? max(1, real(_tag_parameters[4]))
								: 0;

							_state_effect_flags =
								_state_effect_flags | (1 << _effects_map[? "cycle"]);
							...
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__TYPE
							] = __SCRIBBLE_GEN_CONTROL_TYPE.__CYCLE;
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__DATA
							] =
						...
						case 28:
							...
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__UNICODE] =
								__SCRIBBLE_GLYPH_SURFACE;
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__BIDI] =
								__SCRIBBLE_BIDI.SYMBOL;

							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__X] =
								_state_halign_offset;
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__Y] =
								_state_valign_offset;
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__WIDTH] =
								_surface_w;
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__HEIGHT] =
								_surface_h;
							_glyph_grid[#
							...
							_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__TEXTURE] =
								surface_get_texture(_surface);
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__QUAD_U0
							] = 0;
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__QUAD_V0
							] = 0;
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__QUAD_U1
							] = 1;
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__QUAD_V1
							] = 1;
							_glyph_grid[#
								_glyph_count,
								__SCRIBBLE_GEN_GLYPH.__FONT_NAME
							] = undefined;
						...
						case 29:
							...
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__TYPE
							] = __SCRIBBLE_GEN_CONTROL_TYPE.__REGION;
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__DATA
							] = _tag_parameters[1];
						...
						case 32:
							if (array_length(_tag_parameters) != 5) {
								__scribble_error(
									"[typistSound] tags must use the same number of arguments as .sound()"
								);
							} else {
								_control_grid[#
									_control_count,
									__SCRIBBLE_GEN_CONTROL.__TYPE
								] = __SCRIBBLE_GEN_CONTROL_TYPE.__EVENT;
								_control_grid[#
									_control_count,
									__SCRIBBLE_GEN_CONTROL.__DATA
								] = new __scribble_class_event(
									__SCRIBBLE_TYPIST_SOUND_COMMAND_TAG,
									_tag_parameters
								);
								++_control_count;
							}
						...
						case 33:
							if (
								(array_length(_tag_parameters) != 4)
								&& (array_length(_tag_parameters) != 5)
							) {
								__scribble_error(
									"[typistSoundPerChar] tags must use the same number of arguments as .sound_per_char()"
								);
							} else {
								_control_grid[#
									_control_count,
								...
								_control_grid[#
									...
									__SCRIBBLE_TYPIST_SOUND_PER_CHAR_COMMAND_TAG,
									_tag_parameters
								);
								++_control_count;
							}
						...
						case 36: // [indent]
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__TYPE
							] = __SCRIBBLE_GEN_CONTROL_TYPE.__INDENT_START;
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__DATA
							] = undefined;
						...
						case 37: // [/indent]
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__TYPE
							] = __SCRIBBLE_GEN_CONTROL_TYPE.__INDENT_STOP;
							_control_grid[#
								_control_count,
								__SCRIBBLE_GEN_CONTROL.__DATA
							] = undefined;
						...
						default:
							...
							if (ds_map_exists(_effects_map, _tag_command_name)) {
								...
								_state_effect_flags =
									_state_effect_flags
									| (1 << _effects_map[? _tag_command_name]);
								...
								ds_map_exists(_effects_slash_map, _tag_command_name)
							) {
								//Check if this is a effect name, but with a forward slash at the front
								_state_effect_flags = ~(
									(~_state_effect_flags)
										| (1 << _effects_slash_map[? _tag_command_name])
								);
								...
								variable_struct_exists(_colors_struct, _tag_command_name)
							) {
								//Set a pre-defined colour
								_state_colour =
									(_state_colour & 0xFF000000)
									| (_colors_struct[$ _tag_command_name] & 0x00FFFFFF);
								...
								ds_map_exists(_typewriter_events_map, _tag_command_name)
							) {
								//Events
								...
								var _copy_size =
									_buffer_length - buffer_tell(_string_buffer);

								_buffer_length =
									string_byte_length(_macro_result) + _copy_size;
								if (
									_buffer_length > buffer_get_size(_other_string_buffer)
								) {
								...
								buffer_copy(
									...
									_copy_size,
									_other_string_buffer,
									buffer_tell(_other_string_buffer)
								);
								buffer_seek(_other_string_buffer, buffer_seek_start, 0);
								...
								if (
									(!SCRIBBLE_USE_SPRITE_WHITELIST)
									|| (_sprite_whitelist_map[? _sprite_index] ?? false)
								) {
									...
									if (!SCRIBBLE_LEGACY_ANIMATION_SPEED) {
										_image_speed *= __scribble_image_speed_get(
											_sprite_index
										);
									}
									...
									if (
										(_image_speed != 0)
										&& (sprite_get_number(_sprite_index) > 1)
									) {
										__has_animation = true;
									}
									...
									_glyph_grid[#
										_glyph_count,
										__SCRIBBLE_GEN_GLYPH.__UNICODE
									] = __SCRIBBLE_GLYPH_SPRITE;
									_glyph_grid[#
										_glyph_count,
										__SCRIBBLE_GEN_GLYPH.__BIDI
									] = __SCRIBBLE_BIDI.SYMBOL;
									...
									_glyph_grid[#
										_glyph_count,
										__SCRIBBLE_GEN_GLYPH.__SPRITE_INDEX
									] = _sprite_index;
									_glyph_grid[#
										_glyph_count,
										__SCRIBBLE_GEN_GLYPH.__IMAGE_INDEX
									] = _image_index;
									_glyph_grid[#
										_glyph_count,
										__SCRIBBLE_GEN_GLYPH.__IMAGE_SPEED
									] = _image_speed;
								...
								_control_grid[#
									...
									__SCRIBBLE_AUDIO_COMMAND_TAG,
									_tag_parameters
								);
								++_control_count;
							} else if (
								ds_map_exists(_external_sound_map, _tag_command_name)
							) {
								...
								_control_grid[#
									...
									__SCRIBBLE_AUDIO_COMMAND_TAG,
									[_external_sound_map[? _tag_command_name]]
								);
								++_control_count;
							} else {
								var _first_char = string_copy(_tag_command_name, 1, 1);
								if (
									(string_length(_tag_command_name) <= 7)
									&& ((_first_char == "$") || (_first_char == "#"))
								) {
									...
									if (!__SCRIBBLE_ON_WEB) {
										...
										try {
											...
											if (!SCRIBBLE_BGR_COLOR_HEX_CODES) {
												_decoded_colour = scribble_rgb_to_bgr(
													_decoded_colour
												);
											}
										} catch (_error) {
											...
											__scribble_trace(
												"Error! \"",
												string_delete(_tag_command_name, 1, 2),
										...
										repeat (string_length(_tag_command_name) - 1) {
											...
											switch (
												ord(string_char_at(_tag_command_name, _i))
											) {
												case ord("1"):
													_decoded_colour |= 1;
													break;
									...
									var _second_char = string_copy(
										_tag_command_name,
										2,
										1
									);
									if (
										((_first_char == "d") || (_first_char == "D"))
										&& (
					...
					if ((_new_halign != undefined) && (_new_halign != _state_halign)) {
						...
						_control_grid[# _control_count, __SCRIBBLE_GEN_CONTROL.__TYPE] =
							__SCRIBBLE_GEN_CONTROL_TYPE.__HALIGN;
						_control_grid[# _control_count, __SCRIBBLE_GEN_CONTROL.__DATA] =
							_state_halign;
						++_control_count;
			...
			} else if (_glyph_ord == SCRIBBLE_COMMAND_TAG_ARGUMENT) {
				//If we've hit a command tag argument delimiter character (usually ,)
			...
			if (
				(_glyph_ord == SCRIBBLE_COMMAND_TAG_OPEN)
				&& !_ignore_commands
				&& (
					_state_command_tag_flipflop
						|| (
							__scribble_buffer_peek_unicode(
								_string_buffer,
								buffer_tell(_string_buffer)
				...
				(_glyph_ord == 0x0A) || (SCRIBBLE_HASH_NEWLINE && (_glyph_ord == 0x23)) //If we've hit a newline (\n)
			) {
				//If we've hit a hash, and hash newlines are on
				__SCRIBBLE_PARSER_WRITE_NEWLINE;
			} else if (_glyph_ord == 0x09) {
				...
				_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__Y] = 0;
				_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__WIDTH] =
					SCRIBBLE_TAB_WIDTH * _font_space_width;
				_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__HEIGHT] =
					_font_line_height;
				_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__FONT_HEIGHT] =
					_font_line_height;
				_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__SEPARATION] =
					SCRIBBLE_TAB_WIDTH * _font_space_width;
				_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__LEFT_OFFSET] = 0;
				_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT] =
					_control_count;
				...
				//Zero-width space, or a Thai grave accent
				...
				#endregion
			} else if (SCRIBBLE_UNDO_UNICODE_SUBSTITUTIONS && (_glyph_ord == 0x2026)) {
				//Replace ellipsis … with three full stops (periods)
				//Figure out how much we need to copy and if we need to resize the target buffer
				...
				//Only write glyphs that aren't system control characters
				if (SCRIBBLE_UNDO_UNICODE_SUBSTITUTIONS) {
					if (
						(_glyph_ord == 0x2013) //En dash –
						...
						(_glyph_ord == 0x2018) || (_glyph_ord == 0x2019) //Start single quote ‘
					) {
						//End single quote ’
						//Replace with single quote '
						_glyph_ord = 0x0027;
					} else if (
						(_glyph_ord == 0x201C) //Start double quote “
				...
				if ((_glyph_write >= 0x0600) && (_glyph_write <= 0x06FF)) {
					...
					var _glyph_next = __scribble_buffer_peek_unicode(
						_string_buffer,
						_buffer_offset
					);
					...
					if (_glyph_write == 0x0644) {
						...
						switch (_glyph_next) {
							case 0x0622:
								var _glyph_replacement = 0xFEF5;
								break; //Lam with Alef with madda above
							case 0x0623:
								var _glyph_replacement = 0xFEF7;
								break; //Lam with Alef with hamza above
							case 0x0625:
								var _glyph_replacement = 0xFEF9;
								break; //Lam with Alef with madda below
							case 0x0627:
								var _glyph_replacement = 0xFEFB;
								break; //Lam with Alef with hamza below
						}
						...
						if (_glyph_replacement != undefined) {
							...
							_glyph_next = __scribble_buffer_peek_unicode(
								_string_buffer,
								_buffer_offset
							);
						}
					...
					while ((_glyph_next >= 0x064B) && (_glyph_next <= 0x0652)) {
						// Tashkil range
						_buffer_offset += 2;
						_glyph_next = __scribble_buffer_peek_unicode(
							_string_buffer,
					...
					if (_glyph_prev_arabic_join_next) {
						// Does the previous glyph allow joining to us?
						if (_arabic_join_prev_map[? _glyph_next]) {
							// Does the next glyph allow joining to us?
					...
					if ((_glyph_ord < 0x064B) || (_glyph_ord > 0x0652)) {
						_glyph_prev_arabic_join_next = _arabic_join_next_map[?
							_glyph_joiner
						];
					}
					...
					if (
						...
						_glyph_grid[# _glyph_count - 1, __SCRIBBLE_GEN_GLYPH.__Y] -=
							0.17
							* _glyph_grid[#
								_glyph_count - 1,
								__SCRIBBLE_GEN_GLYPH.__FONT_HEIGHT
					...
					if ((_glyph_write >= 0x0900) && (_glyph_write <= 0x097F)) {
						...
						if ((_glyph_write >= 0x0E00) && (_glyph_write <= 0x0E7F)) {
							...
							if (_thai_top_map[? _glyph_write] && (_glyph_count >= 1)) {
								...
								if (_thai_base_map[? _base]) {
									...
									if (_thai_base_ascender_map[? _base]) {
										if (_followingNikhahit) {
											...
											if (
												(_glyph_count >= 2)
												&& _thai_upper_map[? _glyph_prev]
												&& _thai_base_ascender_map[? _glyph_prev]
											) {
												_glyph_write += 0xf713 - 0x0e48;
											}
										}
								...
								_thai_lower_map[? _glyph_write]
								&& (_glyph_count > 0)
								&& _thai_base_descender_map[? _glyph_prev]
							) {
