...
function input_check_opposing_repeat(
	_verb_negative,
	_verb_positive,
	_player_index = 0,
	_most_recent = INPUT_DEFAULT_OPPOSING_MOST_RECENT,
	...
	//Both held
	if (_held_n && _held_p && !_most_recent) {
		return 0;
	}
	...
	if (!_held_p) {
		return (
			_repeat_n && !input_check(_verb_positive, _player_index, _delay + _predelay)
		)
			? -1
			: 0;
	}
	if (!_held_n) {
		return (
			_repeat_p && !input_check(_verb_negative, _player_index, _delay + _predelay)
		)
			? 1
			: 0;
	}
