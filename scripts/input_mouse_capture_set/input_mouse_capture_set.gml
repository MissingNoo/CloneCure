...
function input_mouse_capture_set(_state, _sensitivity = 1) {
	...
	if (!_state || window_get_fullscreen()) {
		return;
	}
	...
	switch (os_type) {
		case os_windows: //Window area intersecting the border on any monitor invalidates mouse capture position
			if (_monitor_update_frame < _global.__frame) {
				...
				_monitor_coords = window_get_visible_rects(
					_window_x,
					_window_y,
					_window_x + _window_width,
					_window_y + _window_height
				);
			}
			...
			repeat (array_length(_monitor_coords) div 8) {
				if (
					(__input_wrap_underflow(_monitor_coords[_i * 8 + 2]) != 0) //W=0 is identity for "not on this monitor"
					&& (__input_wrap_underflow(_monitor_coords[_i * 8 + 3]) != 0) //H=0 is identity for "not on this monitor"
					&& (
						(
							(
								__input_wrap_underflow(_monitor_coords[_i * 8 + 2])
									- __input_wrap_underflow(_monitor_coords[_i * 8])
							) != _window_width
						) //Intersects a horizontal edge
							|| (
								__input_wrap_underflow(_monitor_coords[_i * 8 + 3])
									- __input_wrap_underflow(_monitor_coords[_i * 8 + 1])
									!= _window_height
							)
					)
				) {
					//Intersects a vertical edge
					if (__INPUT_DEBUG_CAPTURE) {
						__input_trace(
							"Window exceeds display bounds on monitor ",
							_i + 1,
							", mouse capture blocked"
						);
					}
		...
		case os_macosx: //Window area bordering or outside the primary monitor invalidates mouse capture position
			...
			if (
				((_monitor_coords[2] - _monitor_coords[0]) != _window_width) //Intersects a horizontal edge
				|| (_monitor_coords[3] - _monitor_coords[1] != _window_height)
			) {
				//Intersects a vertical edge
				if (__INPUT_DEBUG_CAPTURE) {
					__input_trace(
						"Window exceeds display bounds on primary monitor, mouse capture blocked"
					);
				}
				_global.__mouse_capture_blocked = true;
				return;
		...
		case os_linux: //No window_get_visible_rect() values but we can test top left and primary monitor intersection
			if (
				((_window_x < 0) || (_window_y < 0)) //Negative values are always out of bounds
				|| (
					(_window_x < display_get_width())
						&& (_window_x + _window_width > display_get_width())
				) //Primary horizontal edge
				|| (
					(_window_y < display_get_height())
						&& (_window_y + _window_height > display_get_height())
				)
			) {
