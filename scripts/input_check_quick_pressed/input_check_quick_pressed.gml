...
function input_check_quick_pressed(_verb, _player_index = 0, _buffer_duration = 0) {
	...
	if (is_array(_verb)) {
		...
		repeat (array_length(_verb)) {
			if (input_check_quick_pressed(_verb[_i], _player_index, _buffer_duration)) {
				return true;
			}
			++_i;
		}
	...
	if (_buffer_duration <= 0) {
		...
		return (_verb_struct.__quick_press_time >= 0)
			&& (
				(__input_get_time() - _verb_struct.__quick_press_time) <= _buffer_duration
			);
	}
}
