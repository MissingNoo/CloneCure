...
function __input_gamepad_set_blacklist() {
	...
	var _description_lower = string_replace_all(
		string_lower(gamepad_get_description(__index)),
		" ",
		""
	);
	...
	switch (os_type) {
		case os_windows:
			...
			if (
				((__vendor == "4c05") && (__product == "6802")) //PS3 controller
				&& (
					((__axis_count == 4) && (__button_count == 19)) //Bad driver
						|| ((__axis_count == 8) && (__button_count == 0))
				)
			) {
				//DsHidMini gyro
		...
		case os_linux:
			...
			if (
				(__button_count == 0)
				&& (__axis_count == _joycon_imu_axis_count)
				&& (__hat_count == 0)
			) {
				var _i = 0;
			...
			if (
				(__raw_type == "HIDWiiMotionPlus")
				|| (__raw_type == "HIDWiiRemoteNunchuk")
	...
	if (
		(__vendor != "de28")
		&& variable_struct_exists(_global.__ignore_gamepad_types, __simple_type)
	) {
	...
	var _os_guid_dict = is_struct(_os_filter_dict)
		? _os_filter_dict[$ "guid"]
		: undefined;
	var _os_vidpid_dict = is_struct(_os_filter_dict)
		? _os_filter_dict[$ "vid+pid"]
		: undefined;
	var _os_desc_array = is_struct(_os_filter_dict)
		? _os_filter_dict[$ "description contains"]
		: undefined;
	...
	if (is_struct(_os_guid_dict) && variable_struct_exists(_os_guid_dict, __guid)) {
		...
		is_struct(_os_vidpid_dict)
		&& variable_struct_exists(_os_vidpid_dict, string(__vendor) + string(__product))
	) {
		if (!__INPUT_SILENT) {
			__input_trace(
