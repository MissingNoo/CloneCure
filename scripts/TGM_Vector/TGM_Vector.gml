...
function Vector2(_x = 0, _y = _x) constructor {
	...
	static Normalized = function() {
		...
		return new Vector2(x * _normalized, y * _normalized);
	};
	...
	static Lerp = function(_vectorB, _amount) {
		gml_pragma("forceinline");
		return new Vector2(lerp(x, _vectorB.x, _amount), lerp(y, _vectorB.y, _amount));
	};
...
function Vector3(_x = 0, _y = _x, _z = _x) constructor {
	...
	static Normalized = function() {
		...
		return new Vector3(x * _normalized, y * _normalized, z * _normalized);
	};
	...
	static PitchTo = function(_vectorB) {
		return radtodeg(
			arctan2(_vectorB.z - z, point_distance(x, y, _vectorB.x, _vectorB.y))
		);
	};
	...
	static Max = function(_vectorB) {
		gml_pragma("forceinline");
		return new Vector3(max(x, _vectorB.x), max(y, _vectorB.y), max(z, _vectorB.z));
	};

	/// @desc Returns a vector that is made from the smallest components of two vectors.
	static Min = function(_vectorB) {
		gml_pragma("forceinline");
		return new Vector3(min(x, _vectorB.x), min(y, _vectorB.y), min(z, _vectorB.z));
	};
