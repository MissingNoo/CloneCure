...
function __scribble_gen_9_write_vbuffs() {
	...
	ds_grid_set_grid_region(
		...
		0,
		__SCRIBBLE_GEN_VBUFF_POS.__QUAD_L
	);
	ds_grid_set_grid_region(
		_vbuff_pos_grid,
		_glyph_grid,
		...
		__SCRIBBLE_GEN_GLYPH.__Y,
		0,
		__SCRIBBLE_GEN_VBUFF_POS.__QUAD_R
	);
	...
	ds_grid_add_grid_region(
		...
		__SCRIBBLE_GEN_GLYPH.__HEIGHT,
		0,
		__SCRIBBLE_GEN_VBUFF_POS.__QUAD_B
	);
	...
	repeat (__pages) {
		...
		if (SCRIBBLE_ALLOW_GLYPH_DATA_GETTER) {
			with (_page_data) {
				...
				ds_grid_set_grid_region(
					...
					0,
					__glyph_end,
					__SCRIBBLE_GEN_VBUFF_POS.__SIZE - 1,
					0,
					__SCRIBBLE_GLYPH_LAYOUT.__LEFT
				);
			}
		}
		...
		repeat (_page_data.__glyph_count) {
			var _packed_indexes = _glyph_grid[#
				_i,
				__SCRIBBLE_GEN_GLYPH.__ANIMATION_INDEX
			];
			...
			repeat (_control_delta) {
				switch (_control_grid[# _control_index, __SCRIBBLE_GEN_CONTROL.__TYPE]) {
					case __SCRIBBLE_GEN_CONTROL_TYPE.__COLOUR:
						_glyph_colour = _control_grid[#
							_control_index,
							__SCRIBBLE_GEN_CONTROL.__DATA
						];
						var _write_colour = __SCRIBBLE_FIX_ARGB
							? scribble_rgb_to_bgr(_glyph_colour)
							: _glyph_colour; //Fix for bug in vertex_argb() on OpenGL targets (2021-11-24  runtime 2.3.5.458)
						break;

					case __SCRIBBLE_GEN_CONTROL_TYPE.__EFFECT:
						_glyph_effect_flags = _control_grid[#
							_control_index,
							__SCRIBBLE_GEN_CONTROL.__DATA
						];
						break;
					...
					case __SCRIBBLE_GEN_CONTROL_TYPE.__CYCLE:
						_glyph_cycle = _control_grid[#
							_control_index,
							__SCRIBBLE_GEN_CONTROL.__DATA
						];

						if (_glyph_cycle == undefined) {
							_write_colour = __SCRIBBLE_FIX_ARGB
								? scribble_rgb_to_bgr(_glyph_colour)
								: _glyph_colour; //Fix for bug in vertex_argb() on OpenGL targets (2021-11-24  runtime 2.3.5.458)
						} else {
							_write_colour = __SCRIBBLE_FIX_ARGB
								? scribble_rgb_to_bgr(_glyph_cycle)
								: _glyph_cycle; //Fix for bug in vertex_argb() on OpenGL targets (2021-11-24  runtime 2.3.5.458)
						}
						break;
					...
					case __SCRIBBLE_GEN_CONTROL_TYPE.__EVENT:
						...
						var _line_index =
							(_packed_indexes % __SCRIBBLE_MAX_LINES)
							+ ((_character_index > 0) ? 1 : 0);

						var _event = _control_grid[#
							_control_index,
							__SCRIBBLE_GEN_CONTROL.__DATA
						];
					...
					case __SCRIBBLE_GEN_CONTROL_TYPE.__REGION:
						if (_region_name != undefined) {
							...
							if (_region_start <= _region_end) {
								...
								array_push(
									_region_bbox_array,
									{
										...
										y1: ds_grid_get_min(
											_vbuff_pos_grid,
											_region_bbox_start,
											__SCRIBBLE_GEN_VBUFF_POS.__QUAD_T,
											_region_end,
											__SCRIBBLE_GEN_VBUFF_POS.__QUAD_T
										),
										x2: ds_grid_get_max(
								...
								//N.B. This array is exposed to the end-user via .region_get_bboxes()
								array_push(
									_page_data.__region_array,
									{
										name: _region_name,
										bbox_array: _region_bbox_array,
										start_glyph: _region_start
											- _page_data.__glyph_start,
			...
			if (_glyph_ord == __SCRIBBLE_GLYPH_SPRITE) {
				...
				if (_image_speed > 0) {
					_glyph_effect_flags |= 0x01;
				} //Set the sprite flag bit

				if (SCRIBBLE_ADD_SPRITE_ORIGINS) {
					_glyph_x +=
						(_glyph_width div 2)
						- _glyph_xscale * sprite_get_xoffset(_sprite_index);
					_glyph_y +=
						(_glyph_height div 2)
						- _glyph_yscale * sprite_get_yoffset(_sprite_index);
				}

				var _sprite_number = sprite_get_number(_sprite_index);
				if (_sprite_number > 127) {
					__scribble_trace(
						"In-line sprites cannot have more than 127 frames (",
						sprite_get_name(_sprite_index),
						")"
					);
					_sprite_number = 127;
				}
				...
				if (_image_speed >= 2) {
					__scribble_trace(
						"Image speed cannot be more than 2.0 ("
							+ string(_image_speed)
							+ ")"
					);
					_image_speed = 2;
				...
				var _glyph_sprite_data =
					16384 * floor(256 * _image_speed)
					+ 128 * _sprite_number
					+ _image_index;

				var _j = _image_index;
				repeat ((_image_speed > 0) ? _sprite_number : 1) {
					//Only draw one image if we have an image speed of 0 since we're not animating
					...
					if (!__SCRIBBLE_ON_WEB) {
						...
						var _crop_height = _html5_sprite_height_workaround_dict[$
							string(_sprite_index) + ":" + string(_j)
						];
						if (_crop_height == undefined) {
							var _sprite_data = sprite_get_info(_sprite_index);
							_crop_height = _sprite_data.frames[_j].crop_height;

							_html5_sprite_height_workaround_dict[$
								string(_sprite_index) + ":" + string(_j)
							] = _crop_height;
						}
	...
	var _control_delta =
		_glyph_grid[# _i - 1, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT] - _control_index;
	repeat (_control_delta) {
		if (
