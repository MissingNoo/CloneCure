...
function __input_player_tick_sources(_player) {
	...
	var _current_profile_dict = is_string(__profile_name)
		? __profiles_dict[$ __profile_name]
		: undefined;

	var _is_multidevice_player =
		(_global.__source_mode == INPUT_SOURCE_MODE.MULTIDEVICE) && (__index == 0);
				...
				switch (_source_type) {
							...
							switch (_binding.__type) {
								case __INPUT_BINDING_KEY:
									...
									if (__INPUT_ON_ANDROID) {
										if (
											(_binding.__android_lowercase != undefined)
											&& keyboard_check(
												_binding.__android_lowercase
											)
										) {
											_value = 1.0;
											_raw = 1.0;
					...
					case __INPUT_SOURCE.GAMEPAD:
						...
						repeat (INPUT_MAX_ALTERNATE_BINDINGS) {
							...
							if (
								_is_multidevice_player
								&& (_binding.__gamepad_index != undefined)
								&& (_source_gamepad != _binding.__gamepad_index)
							) {
								continue;
							}
							...
							switch (_binding.__type) {
								case __INPUT_BINDING_GAMEPAD_BUTTON:
									if (
										_global
											.__gamepads[_source_gamepad]
											.__get_held(_binding.__value)
									) {
										_value = 1.0;
										_raw = 1.0;
										_analogue = false;
								...
								case __INPUT_BINDING_GAMEPAD_AXIS:
									...
									var _found_raw = _global
										.__gamepads[_source_gamepad]
										.__get_value(_binding.__value);

									var _binding_threshold_min = _binding.__threshold_min;
									var _binding_threshold_max = _binding.__threshold_max;

									if (
										(_binding_threshold_min == undefined)
										|| (_binding_threshold_max == undefined)
									) {
										var _threshold_struct = __axis_threshold_get(
											_binding.__value
										);
									...
									_found_value =
										(_found_value - _binding_threshold_min)
										/ (
											_binding_threshold_max
												- _binding_threshold_min
										);
