...
/// @ignore
/// feather ignore all
function __lexicon_localization_class(
	_langName,
	_region,
	_locale = "",
	_format
	...
	static __GetDateString = function(
		...
		switch (_format ?? _global.dateLength) {
			...
			case lexicon_length.MEDIUM:
				...
				_string = string_replace_all(
					_string,
					"MMM",
					__format.monthNames.short[_month - 1]
				);
				_string = string_replace_all(_string, "D", string(_day));
				break;
			...
			case lexicon_length.LONG:
				...
				_string = string_replace_all(
					_string,
					"MMMM",
					__format.monthNames.long[_month - 1]
				);
				_string = string_replace_all(_string, "D", string(_day));
				break;
			...
			case lexicon_length.FULL:
				...
				_string = string_replace_all(
					_string,
					"EEEE",
					__format.dayNames.long[date_get_weekday(_date)]
				);
				_string = string_replace_all(
					_string,
					"MMMM",
	...
	static __GetTimeString = function(
		_date = date_current_datetime(),
		_format = undefined
	) {
		static _localeCache = "";
		...
		_string = string_replace_all(
			_string,
			" a",
			(!_useMilitaryTime ? " " + string(__format.amPM[_isPM]) : "")
		);
		_currentHour = _hour;
		_currentMinute = _minute;
		_currentSecond = _second;
	...
	static __GetDateTimeString = function(
		_date = date_current_datetime(),
		_formatA = lexicon_length.SHORT,
		_formatB = lexicon_length.SHORT
	) {
		static _localeCache = "";
		static _results = ["", ""];
		static _currentString = "";
		...
		if (
			...
			&& (
				(_year == _currentYear)
					...
					&& (_day == _currentDay)
					&& (_hour == _currentHour)
					&& (_minute == _currentMinute)
					&& (_second == _currentSecond)
			)
		) {
			return _currentString;
		}
