...
function input_vibrate_curve(
	...
	if (!animcurve_exists(_curve)) {
		__input_error("Animation curve doesn't exist (", _curve, ")");
	}
	_pan = clamp(_pan, -1, 1);
	_duration = max(_duration, 0);

	_global
		.__players[_player_index]
		.__vibration_add_event(
			new __input_class_vibration_curve(
				_strength,
				animcurve_get(_curve),
				_pan,
				_duration,
				_force
			)
