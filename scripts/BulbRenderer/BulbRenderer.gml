...
function BulbRenderer(_camera) constructor {
	...
	Update = function() {
		...
		if ((!_hdr) || (hdrBloomIterations != __oldHDRBloomIterations)) {
			__FreeBloomSurfaces();
		}

		//Free up memory if the normal map state has changed
		if ((!normalMap) && __oldNormalMap) {
	...
	DrawLitSurface = function(
		_surface,
		_x,
		_y,
		_width,
		_height,
		_textureFiltering = undefined,
		_alphaBlend = undefined
	) {
		...
		if ((__lightSurface != undefined) && surface_exists(__lightSurface)) {
			if (
				hdr
				&& _system.__hdrAvailable
				&& (hdrBloomIntensity > 0)
				&& (hdrBloomIterations >= 1)
				&& (__lightSurface != undefined)
				&& surface_exists(__lightSurface)
			) {
	...
	__KawaseBloom = function(_surface, _thresholdCoeff) {
		...
		static _u_vThreshold = shader_get_uniform(
		...
		if (array_length(_bloomSurfaceArray) < hdrBloomIterations - 1) {
			...
			repeat (hdrBloomIterations) {
				...
				_bloomSurfaceArray[_i] = _surface_create(
					_bloomWidth,
					_bloomHeight,
					surface_rgba16float
				);
		...
		shader_set_uniform_f(
			_u_vThreshold,
			hdrBloomThresholdMin * _thresholdCoeff,
			hdrBloomThresholdMax * _thresholdCoeff
		);
		shader_set_uniform_f(
			shader_get_uniform(__shdBulbKawaseDownWithThreshold, "u_vTexel"),
			texture_get_texel_width(surface_get_texture(_surface)),
			texture_get_texel_height(surface_get_texture(_surface))
		);
		draw_surface_stretched(
			_surface,
			0,
			0,
			surface_get_width(_bloomSurfaceArray[0]),
			surface_get_height(_bloomSurfaceArray[0])
		);
		shader_reset();
		surface_reset_target();
		...
		if (hdrBloomIterations >= 2) {
			...
			repeat (hdrBloomIterations - 1) {
				...
				shader_set_uniform_f(
					shader_get_uniform(__shdBulbKawaseDown, "u_vTexel"),
					texture_get_texel_width(
						surface_get_texture(_bloomSurfaceArray[_i - 1])
					),
				...
				draw_surface_stretched(
					_bloomSurfaceArray[_i - 1],
					0,
					0,
					surface_get_width(_bloomSurfaceArray[_i]),
					surface_get_height(_bloomSurfaceArray[_i])
				);
				surface_reset_target();
			...
			repeat (hdrBloomIterations - 1) {
				...
				shader_set_uniform_f(
					shader_get_uniform(__shdBulbKawaseUp, "u_vTexel"),
					texture_get_texel_width(surface_get_texture(_bloomSurfaceArray[_i])),
					texture_get_texel_height(surface_get_texture(_bloomSurfaceArray[_i]))
				);
				draw_surface_stretched(
					_bloomSurfaceArray[_i],
					0,
					0,
					surface_get_width(_bloomSurfaceArray[_i - 1]),
					surface_get_height(_bloomSurfaceArray[_i - 1])
				);
				surface_reset_target();
		...
		if (GetTonemap() == BULB_TONEMAP_BAD_GAMMA) {
			if (ambientInGammaSpace) {
				...
				return make_color_rgb(
					255 * power(color_get_red(ambientColor) / 255, 1 / BULB_GAMMA),
			...
			if (ambientInGammaSpace) {
				return make_color_rgb(
					255 * power(color_get_red(ambientColor) / 255, BULB_GAMMA),
	...
	__BulbRendererDefineOverlayUnderlay();
	__BulbRendererDefineAccumulateSoft();
	if (_system.__hasStencil) {
		__BulbRendererDefineAccumulateHard();
	} else {
		__BulbRendererDefineAccumulateHardNoStencil();
	}
	__BulbRendererDefineVertexBuffers();
	__BulbRendererDefineLight();
