...
function __ScribblejrClassExtFit(
	_key,
	_string,
	_hAlign,
	_vAlign,
	...
	_maxWidth,
	_maxHeight
) : __ScribblejrClassBase() constructor {
	...
	var _spriteScale =
		(SCRIBBLEJR_SCALE_SPRITES ? 1 : (1 / _fontScale)) / SCRIBBLEJR_GLOBAL_FONT_SCALE;
	...
	if (_system.__perCharacterWrap) {
		...
		array_push(_layoutArray, {__whitespace: true, __joinsRight: false});

		var _ignoreNext = false;
		var _cursorX = 0;
		var _i = 0;
		repeat (array_length(_unicodeArray) - 1) {
			//Don't include the final space
					...
					if (_foundColour != undefined) {
						...
						if (sprite_exists(_sprite)) {
							...
							var _fragment = {
								...
								__width: _spriteScale
									* _spriteLocalScale
									* sprite_get_width(_sprite),
								__shift: _spriteScale
									* _spriteLocalScale
									* sprite_get_width(_sprite),
								__whitespace: false,
								__joinsRight: true,
		...
		while (_iterations < SCRIBBLEJR_FIT_ITERATIONS) {
			...
			repeat (array_length(_layoutArray)) {
				...
				if (_fragment.__whitespace) {
					...
					if (_fragment.__joinsRight) {
						...
						_cursorX +=
							_stretchWidth + (_fragment.__shift - _fragment.__width);
						_stretchWidth = 0;
						_stretchStart = _i + 1;
					}
				}
			...
			if (_longWord || (_overallHeight > _adjustedHeight)) {
				...
				if (_iterations == 0) {
					_iterations = SCRIBBLEJR_FIT_ITERATIONS - 2;
				}
			}
		...
		var _newlineLayoutCountArray = array_create(
			array_length(_manualLineArray) + 1,
			infinity
		);
	...
	if (SCRIBBLEJR_VERBOSE) {
		__ScribblejrTrace("Created ", self);
	}
	...
	static __DrawNative = function(
		_x,
		_y,
		_colour = c_white,
		_alpha = 1,
		_sdfEffects = undefined
	) {
		...
		if (_sdfEffects != undefined) {
			if (
				SCRIBBLEJR_SHADOW_SPRITES
				&& __fontIsSDF
				&& struct_get_from_hash(_sdfEffects, _dropShadowEnableHash)
			) {
				gpu_set_fog(true, _sdfEffects.dropShadowColour, 0, 0);
				__DrawSprites(
					_x + _scale * _sdfEffects.dropShadowOffsetX,
		...
		if (SCRIBBLEJR_AUTO_BAKE) {
			__BakeVertexBufferTimed();
		}
		if (SCRIBBLEJR_AUTO_RESET_DRAW_STATE) {
	...
	static __DrawSprites = function(_x, _y, _alpha) {
		...
		var _spriteScale =
			(SCRIBBLEJR_SCALE_SPRITES ? _textScale : __scale)
			/ SCRIBBLEJR_GLOBAL_FONT_SCALE;
	...
	static __DrawVertexBuffer = function(_x, _y, _colour = c_white, _alpha = 1) {
		static _shdScribblejrExt_u_vPositionAlphaScale = shader_get_uniform(
			__shdScribblejrColor,
			"u_vPositionAlphaScale"
		);
		static _shdScribblejrExt_u_iColour = shader_get_uniform(
			__shdScribblejrColor,
			"u_iColour"
		);
		...
		shader_set_uniform_f(
			_shdScribblejrExt_u_vPositionAlphaScale,
			_x,
			_y,
			_alpha,
			__scale * __fontScale
	...
	static __DrawVertexBufferSDF = function(
		_x,
		_y,
		_colour = c_white,
		_alpha = 1,
		...
		static _shdScribblejrExt_SDF_u_vPositionAlphaScale = shader_get_uniform(
			__shdScribblejrColorSDF,
			"u_vPositionAlphaScale"
		);
		static _shdScribblejrExt_SDF_u_iColour = shader_get_uniform(
			__shdScribblejrColorSDF,
			"u_iColour"
		);

		static _shdScribblejrColorSDFShadow_u_vPositionAlphaScale = shader_get_uniform(
			__shdScribblejrColorSDFShadow,
			"u_vPositionAlphaScale"
		);
		static _shdScribblejrColorSDFShadow_u_vColorSoftness = shader_get_uniform(
			__shdScribblejrColorSDFShadow,
			"u_vColorSoftness"
		);
		...
		with (_sdfEffects) {
			if (struct_get_from_hash(_sdfEffects, other._dropShadowEnableHash)) {
				...
				shader_set_uniform_f(
					_shdScribblejrColorSDFShadow_u_vPositionAlphaScale,
					_xShadow,
					_yShadow,
				...
				shader_set_uniform_f(
					_shdScribblejrColorSDFShadow_u_vColorSoftness,
		...
		shader_set_uniform_f(
			_shdScribblejrExt_SDF_u_vPositionAlphaScale,
			_x,
			_y,
			_alpha,
			_scale
