...
function __input_steam_apply_trigger_effect(_steam_handle, _effect, _trigger, _strength) {
	...
	if (!__INPUT_ON_WINDOWS || _global.__on_wine || !is_numeric(_steam_handle)) {
		return false;
	}
	...
	if (!is_struct(variable_struct_get(_effect, "__params"))) {
		return false;
	}
	...
	_command_array[_trigger][$ _key_data][$ string(_effect.__mode_name) + "_param"] =
		_effect.__params;
	...
	_command_array[_trigger][$ _key_mode] = _global.__steam_trigger_mode[$
		_effect.__mode
	];

	//Apply player strength to effect params
	if (_effect.__params[$ __INPUT_STEAMWORKS_KEY_AMPLITUDE] != undefined) {
		_effect.__params[$ __INPUT_STEAMWORKS_KEY_AMPLITUDE] *= _strength;
	}
	if (_effect.__params[$ __INPUT_STEAMWORKS_KEY_STRENGTH] != undefined) {
		_effect.__params[$ __INPUT_STEAMWORKS_KEY_STRENGTH] *= _strength;
	}
	...
	if (_trigger == _left) {
		_steam_trigger_params[$ _key_trigger_mask] =
			steam_input_sce_pad_trigger_effect_trigger_mask_l2;
	}
	if (_trigger == _right) {
		_steam_trigger_params[$ _key_trigger_mask] =
			steam_input_sce_pad_trigger_effect_trigger_mask_r2;
	}
