...
function __scribble_class_typist(_per_line) constructor {
	...
	/// @param [gain=1]
	static sound_per_char = function(
		_in_sound_array,
		_pitch_min,
		_pitch_max,
		_exception_string,
		...
		if (is_string(_exception_string)) {
			...
			repeat (string_length(_exception_string)) {
				__sound_per_char_exception_dict[$
					ord(string_char_at(_exception_string, _i))
				] = true;
				++_i;
			}
		} else {
	...
	static unpause = function() {
		if (__paused) {
			...
			__window_index = (__window_index + 2) % (2 * __SCRIBBLE_WINDOW_COUNT);
	...
	static character_delay_add = function(_character, _delay) {
		...
		if (is_string(_character)) {
			...
			if (string_length(_character) >= 2) {
				_char_2 = ord(string_char_at(_character, 2));
			}
		}
	...
	static get_state = function() {
		if (
			(__last_element == undefined)
			|| (__last_page == undefined)
			|| (__last_character == undefined)
		) {
			return 0.0;
		}
		if (__in == undefined) {
		...
		if (!weak_ref_alive(__last_element)) {
			return 2.0;
		} //If there's no element then report that the element is totally faded out

		var _model = __last_element.ref.__get_model(true);
		if (!is_struct(_model)) {
			return 2.0;
		} //If there's no model then report that the element is totally faded out
		...
		var _t = clamp(
			(
				__window_array[__window_index]
					+ max(0, __window_array[__window_index + 1] + __smoothness - _max)
			) / (_max + __smoothness),
			0,
			1
		);
	...
	static __associate = function(_text_element) {
		var _carry_skip =
			__skip && ((__last_element == undefined) || !__drawn_since_skip);

		if (
			(__last_element == undefined)
			|| !weak_ref_alive(__last_element)
			|| (__last_element.ref != _text_element)
		) {
			//We didn't have an element defined, or we swapped to a different element
			reset();
			__last_element = weak_ref_create(_text_element);
		} else if (!weak_ref_alive(__last_element)) {
			//Our associated element got GC'd for some reason and we didn't
			__scribble_trace(
	...
	static __process_event_stack = function(
		...
		repeat (array_length(__event_stack)) {
			...
			var _event_position = __per_line
				? _event_struct.line_index
				: _event_struct.character_index;
			...
			switch (_event_name) {
				...
				case "pause":
					if ((!__skip && !__sync_started) || (!__skip_paused)) {
						if (
							...
							&& (__last_character >= _character_count)
							&& (array_length(__event_stack) <= 0)
						) {
							__scribble_trace(
				...
				case "delay":
					if (!__skip && !__ignore_delay && !__sync_started) {
						var _duration = (array_length(_event_data) >= 1)
							? real(_event_data[0])
				...
				case __SCRIBBLE_TYPIST_SOUND_COMMAND_TAG: //TODO - Add warning when adding a conflicting custom event
					sound(
						__scribble_parse_sound_array_string(_event_data[1]),
						real(_event_data[2]),
						real(_event_data[3]),
						real(_event_data[4])
					);
				...
				default:
					...
					if (is_method(_function)) {
						with (_function_scope) {
							_function(_target_element, _event_data, _event_position);
						}
	...
	static __play_sound = function(_head_pos, _character) {
		...
		if (is_array(_sound_array) && (array_length(_sound_array) > 0)) {
			...
			if (_play_sound) {
				...
				var _audio_asset = _sound_array[
					floor(__scribble_random() * array_length(_sound_array))
				];
				...
				if (_audio_asset != undefined) {
					var _inst = __scribble_play_sound(
						_audio_asset,
						__sound_gain,
						lerp(__sound_pitch_min, __sound_pitch_max, __scribble_random())
					);
					__sound_finish_time =
						current_time
						+ 1000 * audio_sound_length(_inst)
						- __sound_overlap;
				}
			}
		}
	...
	static __execute_function_per_character = function(_function_scope) {
		...
		if (is_method(__function_per_char)) {
			...
			(__function_per_char != undefined) && script_exists(__function_per_char)
		) {
			script_execute(
				__function_per_char,
				_function_scope,
	...
	static __execute_function_on_complete = function(_function_scope) {
		...
		if (is_method(__function_on_complete)) {
			...
			(__function_on_complete != undefined) && script_exists(__function_on_complete)
		) {
	...
	static __tick = function(_target_element, _in_function_scope) {
		var _function_scope = (__function_scope != undefined)
			? __function_scope
			: _in_function_scope;
		...
		var _page_data = _pages_array[__last_page];
		var _page_character_count = __per_line
			? _page_data.__line_count
			: _page_data.__character_count;

		if (!__in) {
			if (__skip) {
				__window_array[@ __window_index] = _page_character_count;
			} else {
				__window_array[@ __window_index] = min(
					_page_character_count,
					_head_pos + _speed
				);
			}
		} else {
			...
			if (__paused) {
				...
				if (audio_is_paused(__sync_instance)) {
					...
					if (
						audio_sound_get_track_position(__sync_instance) > __sync_pause_end
					) {
						...
						//Increment the window index
						__window_index =
							(__window_index + 2) % (2 * __SCRIBBLE_WINDOW_COUNT);
						__window_array[@ __window_index] = _head_pos;
						__window_array[@ __window_index + 1] = _head_pos - __smoothness;
					} else {
						_paused = true;
			...
			if (!_paused) {
				...
				while (_remaining > 0) {
					...
					if (_head_pos >= __last_character) {
						...
						var _found_events = __last_element
							.ref
							.get_events(__last_character, undefined, __per_line);
						...
						if (
								...
								(
									__last_character
										< (SCRIBBLE_DELAY_LAST_CHARACTER
											? _page_character_count
											: (_page_character_count - 1))
								) || (_found_size > 0)
							)
						) {
							var _glyph_ord = _page_data.__glyph_grid[#
								__last_character - 1,
								__SCRIBBLE_GLYPH_LAYOUT.__UNICODE
							];
							...
							if (__last_character > 1) {
								_glyph_ord =
									(_glyph_ord << 32)
									| _page_data.__glyph_grid[#
										__last_character - 2,
										__SCRIBBLE_GLYPH_LAYOUT.__UNICODE
									];
								var _double_char_delay = __character_delay_dict[$
									_glyph_ord
								];
								_double_char_delay = (_double_char_delay == undefined)
									? 0
									: _double_char_delay;
							...
							if (_delay > 0) {
								array_push(
									__event_stack,
									new __scribble_class_event("delay", [_delay])
								);
							}
						}
		...
		if (__skip) {
			...
			var _i = 0;
			repeat (__SCRIBBLE_WINDOW_COUNT) {
				__window_array[@ _i + 1] = min(
					__window_array[_i + 1] + _speed,
					__window_array[_i]
				);
				_i += 2;
			}
