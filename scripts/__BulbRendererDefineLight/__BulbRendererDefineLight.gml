...
function __BulbRendererDefineLight() {
	...
	GetLightSurface = function() {
		...
		if (
			(__lightSurface != undefined)
			&& (
				(surface_get_width(__lightSurface) != _surfaceWidth)
					|| (surface_get_height(__lightSurface) != _surfaceHeight)
			)
		) {
		...
		if ((__lightSurface == undefined) || !surface_exists(__lightSurface)) {
			...
			if (_system.__hdrAvailable) {
				...
				__lightSurface = _surface_create(
					_surfaceWidth,
					_surfaceHeight,
					hdr ? surface_rgba16float : surface_rgba8unorm
				);
			} else {
	...
	GetLightValue = function(_worldX, _worldY) {
		...
		var _vector = matrix_transform_vertex(
			matrix_multiply(_viewMatrix, _projMatrix),
			_worldX,
			_worldY,
			0
		);
		...
		if (_tonemap == BULB_TONEMAP_BAD_GAMMA) {
			...
			_resultR =
				(_resultR * (6.2 * _resultR + 0.5))
				/ (_resultR * (6.2 * _resultR + 1.7) + 0.06);
			_resultG =
				(_resultG * (6.2 * _resultG + 0.5))
				/ (_resultG * (6.2 * _resultG + 1.7) + 0.06);
			_resultB =
				(_resultB * (6.2 * _resultB + 0.5))
				/ (_resultB * (6.2 * _resultB + 1.7) + 0.06);
			...
			if ((_tonemap == BULB_TONEMAP_NONE) || (_tonemap == BULB_TONEMAP_CLAMP)) {
				...
				_resultR =
					(
						(
							_resultR * (_uc2_A * _resultR + _uc2_C * _uc2_B)
								+ _uc2_D * _uc2_E
						) / (_resultR * (_uc2_A * _resultR + _uc2_B) + _uc2_D * _uc2_F)
					)
					- _uc2_E / _uc2_F;
				_resultG =
					(
						(
							_resultG * (_uc2_A * _resultG + _uc2_C * _uc2_B)
								+ _uc2_D * _uc2_E
						) / (_resultG * (_uc2_A * _resultG + _uc2_B) + _uc2_D * _uc2_F)
					)
					- _uc2_E / _uc2_F;
				_resultB =
					(
						(
							_resultB * (_uc2_A * _resultB + _uc2_C * _uc2_B)
								+ _uc2_D * _uc2_E
						) / (_resultB * (_uc2_A * _resultB + _uc2_B) + _uc2_D * _uc2_F)
					)
					- _uc2_E / _uc2_F;
			} else if (_tonemap == BULB_TONEMAP_UNREAL3) {
				_resultR = max(0, _resultR - 0.004);
				...
				_resultR =
					(_resultR * (2.51 * _resultR + 0.03))
					/ (_resultR * (2.43 * _resultR + 0.59) + 0.14);
				_resultG =
					(_resultG * (2.51 * _resultG + 0.03))
					/ (_resultG * (2.43 * _resultG + 0.59) + 0.14);
				_resultB =
					(_resultB * (2.51 * _resultB + 0.03))
					/ (_resultB * (2.43 * _resultB + 0.59) + 0.14);
			}
