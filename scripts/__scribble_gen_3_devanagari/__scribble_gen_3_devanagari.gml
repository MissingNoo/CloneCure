...
function __scribble_gen_3_devanagari() {
	...
	static _krutidev_matra_lookup_map =
		__scribble_initialize().__krutidev_matra_lookup_map;
	...
	repeat (_glyph_count) {
		switch (_glyph_grid[# _i, __SCRIBBLE_GEN_GLYPH.__UNICODE]) {
			...
			case ord("'"):
				...
				_glyph_grid[# _i, __SCRIBBLE_GEN_GLYPH.__UNICODE] =
					__SCRIBBLE_DEVANAGARI_OFFSET
					+ (_in_single_quote ? ord("^") : ord("*"));
				break;
	...
	repeat (_glyph_count - 1) {
		...
		if (_char == ord("ि")) {
			...
			_glyph_grid[# _j, __SCRIBBLE_GEN_GLYPH.__UNICODE] =
				__SCRIBBLE_DEVANAGARI_OFFSET + ord("f");
			_glyph_grid[# _j, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT] = _glyph_grid[#
				_j + 1,
	...
	for (var _i = 0; _i < _glyph_count; ++_i) {
		...
		if (
			...
			_glyph_grid[# _i + _copyCount, __SCRIBBLE_GEN_GLYPH.__UNICODE] =
				__SCRIBBLE_DEVANAGARI_OFFSET + ord("Z");
			_glyph_grid[# _i + _copyCount, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT] =
				_glyph_grid[# _copyCount - 1, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT];
	...
	var _threeChar =
		_twoChar | ((_glyph_grid[# 1, __SCRIBBLE_GEN_GLYPH.__UNICODE] & 0xFFFF) << 32);
	var _fourChar =
		_threeChar | ((_glyph_grid[# 2, __SCRIBBLE_GEN_GLYPH.__UNICODE] & 0xFFFF) << 48);
	...
	for (var _i = 0; _i < _glyph_count; ++_i) {
		...
		_fourChar =
			_threeChar
			| ((_glyph_grid[# _i + 3, __SCRIBBLE_GEN_GLYPH.__UNICODE] & 0xFFFF) << 48);
		...
		if (_replacementArray != undefined) {
			...
			if ((_foundLength == 1) && (_replacementLength == 1)) {
				...
				_glyph_grid[# _i, __SCRIBBLE_GEN_GLYPH.__UNICODE] =
					__SCRIBBLE_DEVANAGARI_OFFSET + _replacementArray[0];
			} else {
				...
				var _j = 0;
				repeat (_copyCount) {
					_glyph_grid[# _i + _j, __SCRIBBLE_GEN_GLYPH.__UNICODE] =
						__SCRIBBLE_DEVANAGARI_OFFSET + _replacementArray[_j];
					++_j;
				}
				...
				if (_foundLength > _replacementLength) {
					...
					ds_grid_set_grid_region(
						...
						0,
						0,
						_glyph_count - _insertPos,
						__SCRIBBLE_GEN_GLYPH.__SIZE,
						_insertPos + (_replacementLength - _foundLength),
						0
					);

					if (_replacementLength - _foundLength == 1) {
						//Adding one character
						_glyph_grid[# _insertPos, __SCRIBBLE_GEN_GLYPH.__UNICODE] =
							__SCRIBBLE_DEVANAGARI_OFFSET
							+ _replacementArray[_replacementLength - 1];
						_glyph_grid[# _insertPos, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT] =
							_glyph_grid[#
								_insertPos - 1,
								__SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT
							];
						...
						_glyph_grid[# _insertPos, __SCRIBBLE_GEN_GLYPH.__UNICODE] =
							__SCRIBBLE_DEVANAGARI_OFFSET
							+ _replacementArray[_replacementLength - 2];
						_glyph_grid[# _insertPos, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT] =
							_glyph_grid[#
								_insertPos - 1,
								__SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT
						...
						_glyph_grid[# _insertPos + 1, __SCRIBBLE_GEN_GLYPH.__UNICODE] =
							__SCRIBBLE_DEVANAGARI_OFFSET
							+ _replacementArray[_replacementLength - 1];
						_glyph_grid[#
							_insertPos + 1,
				...
				_twoChar =
					(_glyph_grid[# _i + 1, __SCRIBBLE_GEN_GLYPH.__UNICODE] & 0xFFFF)
					<< 16;
				_threeChar =
					_twoChar
					| (
						(_glyph_grid[# _i + 2, __SCRIBBLE_GEN_GLYPH.__UNICODE] & 0xFFFF)
							<< 32
					);
				_fourChar =
					_threeChar
					| (
						(_glyph_grid[# _i + 3, __SCRIBBLE_GEN_GLYPH.__UNICODE] & 0xFFFF)
							<< 48
					);
			}
		}
	...
	repeat (_glyph_count) {
		var _control_delta =
			_glyph_grid[# _i, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT] - _control_index;
		repeat (_control_delta) {
			if (
				_control_grid[# _control_index, __SCRIBBLE_GEN_CONTROL.__TYPE]
				== __SCRIBBLE_GEN_CONTROL_TYPE.__FONT
			) {
				var _font_name = _control_grid[#
					_control_index,
		...
		if (_found_glyph == 0xFFFF) {
			...
			//Don't transform sprite, surfaces, or stuff that's non-printable
			...
			if (_data_index == undefined) {
				...
				ds_grid_set_grid_region(
					_glyph_grid,
					_font_glyph_data_grid,
					_data_index,
					SCRIBBLE_GLYPH.UNICODE,
					_data_index,
					SCRIBBLE_GLYPH.FONT_NAME,
					_i,
					__SCRIBBLE_GEN_GLYPH.__UNICODE
				);
			}
	...
	_glyph_grid[# _glyph_count, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT] = (_glyph_count > 0)
		? _glyph_grid[# _glyph_count - 1, __SCRIBBLE_GEN_GLYPH.__CONTROL_COUNT]
		: 0; //Make sure we collect controls at the end of a string
...
function __scribble_debug_krutidev(_glyph_grid) {
	...
	while (true) {
		...
		if (_unicode >= __SCRIBBLE_DEVANAGARI_OFFSET) {
