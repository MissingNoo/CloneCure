...
function __input_finalize_default_profiles() {
	...
	if (!is_struct(_global.__default_profile_dict)) {
		__input_error(
			...
			typeof(_global.__default_profile_dict),
	...
	repeat (array_length(_global.__profile_array)) {
		...
		repeat (array_length(_profile_verb_names)) {
			...
			repeat (array_length(_verb_data)) {
				...
				_global
					.__default_player
					.__binding_set(_profile_name, _verb_name, _a, _binding);
	...
	if (!__INPUT_SILENT) {
		...
		if (!variable_struct_exists(_global.__profile_dict, INPUT_AUTO_PROFILE_FOR_MOUSE)) {
			__input_trace(
				"Warning! Default profile for mouse \"",
				INPUT_AUTO_PROFILE_FOR_MOUSE,
				"\" has not been defined in __input_config_verbs()"
			);
		}
		if (
			!variable_struct_exists(
				_global.__profile_dict,
				INPUT_AUTO_PROFILE_FOR_GAMEPAD
			)
		) {
			__input_trace(
	...
	if (!variable_struct_exists(_global.__basic_verb_dict, INPUT_CURSOR_VERB_UP)) {
		if (!__INPUT_SILENT) {
			__input_trace(
	...
	if (!variable_struct_exists(_global.__basic_verb_dict, INPUT_CURSOR_VERB_DOWN)) {
		if (!__INPUT_SILENT) {
			__input_trace(
	...
	if (!variable_struct_exists(_global.__basic_verb_dict, INPUT_CURSOR_VERB_LEFT)) {
		if (!__INPUT_SILENT) {
			__input_trace(
	...
	if (!variable_struct_exists(_global.__basic_verb_dict, INPUT_CURSOR_VERB_RIGHT)) {
		if (!__INPUT_SILENT) {
			__input_trace(
