...
/// @returns {String}
function string_currency_prettify(
	_cash,
	_prefix = "$",
	_decimals = 0,
	_removeZeroes = true
	...
	var _cashAbs = abs(_cash), _negativeSymbol = "";
	if (_cash < 0) {
		_negativeSymbol = "-";
	}
	if (_cashAbs < 1000) {
		...
		var _pos = floor(log10(_cashAbs) / 3),
			// number of digits divided by 3
			_dividor = power(10, _pos * 3),
			_value = _cashAbs / _dividor;
		if (_removeZeroes) {
			var _fractional = string_replace(
				string_replace_all(frac(_value), "0", ""),
				".",
				""
			);
		...
		return _prefix
			+ _negativeSymbol
			+ string_format(_value, 0, _decimals)
			+ _suffixes[_pos];
	}
}
...
function string_limit_nonmono(_str, _width) {
	if (string_width(_str) < _width) {
		return _str;
	}
	var _strFinal = "", _char = "", i = 1, isize = string_length(_str), _ww = 0;
	repeat (isize) {
...
/// @returns {string}
function string_random_letter_case(
	_str,
	_firstIsUpper = true,
	_sequence = 1,
	_skipChar = true,
...
function string_first_letter_upper_case(_str) {
	var _string = string_lower(_str), _strFinal = "", i = 1, isize = string_length(_str);
	repeat (isize) {
...
function string_reverse(_str) {
	var _strFinal = "", isize = string_length(_str), i = isize;
	repeat (isize) {
