// Feather disable all
function __input_class_vibration_curve(
	_strength,
	_curve,
	_pan,
	_duration,
	...
	static __tick = function(_time_step) {
		...
		if (_count == 1) {
			__output_left = clamp(
				animcurve_channel_evaluate(animcurve_get_channel(__curve, 0), _t),
				0,
				1
			);
			...
			var _right = animcurve_get_channel(
				__curve,
				animcurve_get_channel(__curve, 1)
			);
