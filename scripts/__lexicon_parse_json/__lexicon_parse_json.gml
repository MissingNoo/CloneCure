...
function __lexicon_parse_json(_json) {
	...
	if ((_global.language != _map.language) && (!_global.forceLoadFile)) {
		__lexicon_trace(
			"language is " + _global.language + " where it expected " + _map.language
		);
		return;
	}
	...
	repeat (array_length(_textArray)) {
		if (is_struct(_textStructPtr[$ _textArray[_k]])) {
			__lexicon_text_struct_to_string(
				_textArray[_k],
				_textStructPtr[$ _textArray[_k]]
			);
		} else if (is_array(_textStructPtr[$ _textArray[_k]])) {
			__lexicon_text_array_to_string(
				_textArray[_k],
				_textStructPtr[$ _textArray[_k]]
			);
		} else {
			if (
				...
				&& variable_struct_exists(_global.textEntries, _textArray[_k])
			) {
