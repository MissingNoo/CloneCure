...
function directory_get_contents(
	_pathSource,
	_contentsArray,
	_extension = "*.*",
	_searchFiles = true,
	_searchFolders = true,
	_searchSubdir = true,
	_dataType = DIRSCAN_DATA_TYPE.FULL_PATH,
	_getSize = false,
	_debug = true
) {
	...
	var _contents = [];
	var _file = file_find_first(
		_pathSource + "/" + _extension,
		fa_directory | fa_archive | fa_readonly
	);
	var _filesCount = 0;
	while (_file != "") {
		if (_file == ".") {
	...
	repeat (_filesCount) {
		...
		if (_debug) {
			show_debug_message(
				$"Scanning: [{_progress * 100}%] {_dirName} | {_fileName}"
			);
		}
		...
		if (_data != undefined) {
			array_push(_contentsArray, _data);
		}
		++_i;
	}
	return _contents;
