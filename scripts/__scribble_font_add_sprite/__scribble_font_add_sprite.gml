...
function __scribble_font_add_sprite_ext(_sprite, _mapstring, _proportional, _separation) {
	var _spritefont = __font_add_sprite_ext__(
		_sprite,
		_mapstring,
		_proportional,
		_separation
...
function __scribble_font_add_sprite_common(
	_sprite,
	_spritefont,
	_proportional,
	_separation
	...
	if (_scribble_state.__default_font == undefined) {
		if (SCRIBBLE_VERBOSE) {
			__scribble_trace("Setting default font to \"" + string(_sprite_name) + "\"");
		}
	...
	if (SCRIBBLE_VERBOSE) {
		__scribble_trace(
			...
			_sprite_name,
			"\" has ",
			array_length(_info_glyph_names),
			" characters"
		);
	}
		...
		if (_unicode == 32) {
			if (_proportional) {
				if (_image >= array_length(_sprite_frames)) {
					//Cases where the space character has not been added to the mapstring
					var _space_width =
						1
			...
			_font_glyph_data_grid[# _i, SCRIBBLE_GLYPH.TEXTURE] = _sprite_frames[
				0
			].texture; //Use the texture ID for the first image from the sprite
			...
			_font_glyph_data_grid[# _i, SCRIBBLE_GLYPH.X_OFFSET] =
				_x_offset - _sprite_x_offset;
			_font_glyph_data_grid[# _i, SCRIBBLE_GLYPH.Y_OFFSET] =
				_image_info.y_offset - _sprite_y_offset;
	...
	if (SCRIBBLE_VERBOSE) {
		__scribble_trace("Added \"", _sprite_name, "\" as a spritefont");
	}
