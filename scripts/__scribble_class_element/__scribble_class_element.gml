...
function __scribble_class_element(_string, _unique_id) constructor {
	...
	static __ecache_weak_array = __scribble_initialize()
		.__cache_state
		.__ecache_weak_array;
	static __ecache_name_array = __scribble_initialize()
		.__cache_state
		.__ecache_name_array;
	...
	if (__SCRIBBLE_DEBUG) {
		__scribble_trace("Caching element \"" + __cache_name + "\"");
	}
	...
	static draw = function(_x, _y, _typist = undefined) {
		...
		if (__last_drawn < __scribble_state.__frames) {
			...
			if (SCRIBBLE_SAFELY_WRAP_TIME) {
				__animation_time = __animation_time % 16383;
		...
		with (_scribble_state) {
			...
			if (
				(!__shader_anim_disabled)
				&& __blink_on_duration + __blink_off_duration > 0
			) {
				other.__animation_blink_state =
					(
						(other.__animation_time + __blink_time_offset)
							% (__blink_on_duration + __blink_off_duration)
					) < __blink_on_duration;
			} else {
		...
		_model.__submit(
			__page,
			(__sdf_outline_thickness > 0) || (__sdf_shadow_alpha > 0)
		);
	...
	static align = function(_halign = __starting_halign, _valign = __starting_valign) {
		if (_halign == "pin_left") {
			_halign = __SCRIBBLE_PIN_LEFT;
		}
		if (_halign == "pin_centre") {
			_halign = __SCRIBBLE_PIN_CENTRE;
		}
		if (_halign == "pin_center") {
			_halign = __SCRIBBLE_PIN_CENTRE;
		}
		if (_halign == "pin_right") {
			_halign = __SCRIBBLE_PIN_RIGHT;
		}
		if (_halign == "fa_justify") {
			_halign = __SCRIBBLE_FA_JUSTIFY;
		}
	...
	static blend = function(_colour, _alpha) {
		...
		if (_colour != undefined) {
			__blend_colour = _colour & 0xFFFFFF;
		}
		if (_alpha != undefined) {
	...
	static scale_to_box = function(_max_width, _max_height, _maximise = false) {
		...
		if (
			(_max_width != __scale_to_box_width)
			|| (_max_height != __scale_to_box_height)
			|| (_maximise != __scale_to_box_maximise)
		) {
	...
	static region_detect = function(_element_x, _element_y, _pointer_x, _pointer_y) {
		...
		if (__matrix_inverse == undefined) {
			__matrix_inverse = __scribble_matrix_inverse(
				matrix_multiply(_matrix, matrix_get(matrix_world))
			);
		}
		var _vector = matrix_transform_vertex(
			__matrix_inverse,
			_pointer_x,
			_pointer_y,
			0
	...
	static __update_bbox_matrix = function() {
		...
		if (__bbox_dirty) {
			...
			var _bbox = _model.__get_bbox(
				SCRIBBLE_BOUNDING_BOX_USES_PAGE ? __page : undefined,
				__padding_l,
				__padding_t,
				__padding_r,
				__padding_b
			...
			if ((_xscale == 1) && (_yscale == 1) && (__post_angle == 0)) {
				...
				//TODO - Optimize this
				__bbox_matrix = matrix_multiply(
					matrix_build(-__origin_x, -__origin_y, 0, 0, 0, 0, 1, 1, 1),
					matrix_multiply(
						matrix_build(0, 0, 0, 0, 0, 0, _xscale, _yscale, 1),
				...
				var _vertex = matrix_transform_vertex(__bbox_matrix, _r, _t, 0);
				__bbox_obb_x1 = _vertex[0];
				__bbox_obb_y1 = _vertex[1];
				var _vertex = matrix_transform_vertex(__bbox_matrix, _l, _b, 0);
				__bbox_obb_x2 = _vertex[0];
				__bbox_obb_y2 = _vertex[1];
				var _vertex = matrix_transform_vertex(__bbox_matrix, _r, _b, 0);
				__bbox_obb_x3 = _vertex[0];
				__bbox_obb_y3 = _vertex[1];

				__bbox_aabb_left = min(
					__bbox_obb_x0,
					__bbox_obb_x1,
					__bbox_obb_x2,
					__bbox_obb_x3
				...
				__bbox_aabb_right = max(
					__bbox_obb_x0,
					__bbox_obb_x1,
					__bbox_obb_x2,
					__bbox_obb_x3
				);
				__bbox_aabb_bottom = max(
					__bbox_obb_y0,
					__bbox_obb_y1,
					__bbox_obb_y2,
					__bbox_obb_y3
	...
	static get_bbox_revealed = function(_x, _y, _typist) {
		...
		if (_typist != undefined) {
			var _bbox = _model.__get_bbox_revealed(
				__page,
				0,
				_typist.__window_array[_typist.__window_index],
				__padding_l,
				__padding_t,
				__padding_r,
				__padding_b
		...
		if ((_xscale == 1) && (_yscale == 1) && (__post_angle == 0)) {
			...
			var _vertex = matrix_transform_vertex(__bbox_matrix, _l, _t, 0);
			var _x0 = _x + _vertex[0];
			var _y0 = _y + _vertex[1];
			var _vertex = matrix_transform_vertex(__bbox_matrix, _r, _t, 0);
			var _x1 = _x + _vertex[0];
			var _y1 = _y + _vertex[1];
			var _vertex = matrix_transform_vertex(__bbox_matrix, _l, _b, 0);
			var _x2 = _x + _vertex[0];
			var _y2 = _y + _vertex[1];
			var _vertex = matrix_transform_vertex(__bbox_matrix, _r, _b, 0);
			var _x3 = _x + _vertex[0];
			var _y3 = _y + _vertex[1];
	...
	static get_text = function() {
		var _page = ((argument_count > 0) && (argument[0] != undefined))
			? argument[0]
			: __page;
	...
	static msdf_shadow = function(
		...
		__scribble_error(
			".msdf_shadow(), and MSDF fonts as a whole, have been removed from Scribble\nInstead, please use GameMaker's native SDF fonts"
		);
		return self;
	};
	...
	static msdf_border = function(_colour, _thickness) {
		__scribble_error(
			".msdf_border(), and MSDF fonts as a whole, have been removed from Scribble\nInstead, please use GameMaker's native SDF fonts"
		);
		return self;
	};
	...
	static msdf_feather = function(_thickness) {
		__scribble_error(
			".msdf_feather(), and MSDF fonts as a whole, have been removed from Scribble\nInstead, please use GameMaker's native SDF fonts"
		);
		return self;
	};
	...
	static flush = function() {
		...
		if (__SCRIBBLE_DEBUG) {
			__scribble_trace("Flushing element \"" + string(__cache_name) + "\"");
		}
	...
	static preprocessor = function(_function) {
		if (_function != __preprocessorFunc) {
			if ((_function != undefined) && (!script_exists(_function))) {
				__scribble_error(
					"Preprocessor functions must be stored in scripts in global scope"
				);
			}
	...
	static template = function(_template, _on_change = true) {
		if (is_array(_template)) {
			if (
				!_on_change
				|| !is_array(__template)
				|| !array_equals(__template, _template)
			) {
				if (_on_change) {
	...
	static debug_draw_bbox = function(_x, _y) {
		...
		switch (__starting_valign) {
			case fa_top:
				break;
			case fa_middle:
				_y -= __wrap_max_height / 2;
				break;
			case fa_bottom:
				_y -= __wrap_max_height;
				break;
		}
		...
		draw_rectangle(
			_x + 1,
			_y + 1,
			_x - 1 + __wrap_max_width,
			_y - 1 + __wrap_max_height,
			true
		);
	...
	static __get_model = function(_allow_create) {
		...
		if (__flushed || (__text == "")) {
			...
			if (__model_cache_name_dirty) {
				...
				buffer_write(_buffer, buffer_u8, 0x3A); //colon
				...
				buffer_write(
					_buffer,
					buffer_text,
					string(__wrap_max_width - (__padding_l + __padding_r))
				);
				buffer_write(_buffer, buffer_u8, 0x3A);
				buffer_write(
					_buffer,
					buffer_text,
					string(__wrap_max_height - (__padding_t + __padding_b))
				);
				buffer_write(_buffer, buffer_u8, 0x3A);
				...
				buffer_write(_buffer, buffer_u8, 0x2C);
				...
				buffer_write(
					_buffer,
					buffer_text,
					string(ptr(__preprocessorFunc ?? pointer_null))
				);
				buffer_write(_buffer, buffer_u8, 0x3A);
	...
	static __set_standard_uniforms = function(_typist, _function_scope) {
		...
		static _u_iTypewriterUseLines = shader_get_uniform(
			__shd_scribble,
			"u_iTypewriterUseLines"
		);
		static _u_iTypewriterMethod = shader_get_uniform(
			__shd_scribble,
			"u_iTypewriterMethod"
		);
		static _u_iTypewriterCharMax = shader_get_uniform(
			__shd_scribble,
			"u_iTypewriterCharMax"
		);
		static _u_fTypewriterWindowArray = shader_get_uniform(
			__shd_scribble,
			"u_fTypewriterWindowArray"
		);
		static _u_fTypewriterSmoothness = shader_get_uniform(
			__shd_scribble,
			"u_fTypewriterSmoothness"
		);
		static _u_vTypewriterStartPos = shader_get_uniform(
			__shd_scribble,
			"u_vTypewriterStartPos"
		);
		static _u_vTypewriterStartScale = shader_get_uniform(
			__shd_scribble,
			"u_vTypewriterStartScale"
		);
		static _u_fTypewriterStartRotation = shader_get_uniform(
			__shd_scribble,
			"u_fTypewriterStartRotation"
		);
		static _u_fTypewriterAlphaDuration = shader_get_uniform(
			__shd_scribble,
			"u_fTypewriterAlphaDuration"
		);

		static _u_vShadowOffsetAndSoftness = shader_get_uniform(
			__shd_scribble,
		...
		static _u_fOutlineThickness = shader_get_uniform(
			__shd_scribble,
			"u_fOutlineThickness"
		);
		...
		shader_set_uniform_f(
			_u_vColourBlend,
			colour_get_red(__blend_colour) / 255,
		...
		if (
			...
			shader_set_uniform_f(
				_u_vGradient,
				colour_get_red(__gradient_colour) / 255,
			...
			shader_set_uniform_f(
				_u_vFlash,
				colour_get_red(__flash_colour) / 255,
			...
			shader_set_uniform_f(
				_u_vRegionActive,
				__region_glyph_start,
				__region_glyph_end
			);

			shader_set_uniform_f(
				_u_vRegionColour,
				colour_get_red(__region_colour) / 255,
		...
		if (_scribble_state.__shader_anim_desync) {
			with (_scribble_state) {
				...
				shader_set_uniform_f_array(
					_u_aDataFields,
					__shader_anim_disabled
						? _shader_uniforms_disabled
						: _anim_properties_array
				);
			}
		}
		...
		if (_typist != undefined) {
			...
			shader_set_uniform_f_array(
				_u_fTypewriterWindowArray,
				__tw_reveal_window_array
			);
		} else {
		...
		shader_set_uniform_f(
			_u_vShadowOffsetAndSoftness,
			__sdf_shadow_xoffset,
			__sdf_shadow_yoffset,
			__sdf_shadow_softness
		);

		shader_set_uniform_f(
			_u_vShadowColour,
			colour_get_red(__sdf_shadow_colour) / 255,
		...
		shader_set_uniform_f(
			_u_vOutlineColour,
			colour_get_red(__sdf_outline_colour) / 255,
	...
	static __update_scale_to_box_scale = function() {
		...
		if (__scale_to_box_width > 0) {
			_xscale =
				__scale_to_box_width / (_model.__get_width() + __padding_l + __padding_r);
		}
		if (__scale_to_box_height > 0) {
			_yscale =
				__scale_to_box_height
				/ (_model.__get_height() + __padding_t + __padding_b);
		}
	...
	static __update_matrix = function(_model, _x, _y) {
		...
		if (__matrix_dirty || (__matrix_x != _x) || (__matrix_y != _y)) {
			...
			if (!_model.__pad_bbox_l) {
				_x_offset += __padding_l;
			}
			if (!_model.__pad_bbox_t) {
				_y_offset += __padding_t;
			}
			if (!_model.__pad_bbox_r) {
				_x_offset -= __padding_r;
			}
			if (!_model.__pad_bbox_b) {
			...
			if ((_xscale == 1) && (_yscale == 1) && (_angle == 0)) {
				...
				//FIXME - Re-optimise
				__matrix = matrix_multiply(
					matrix_build(_x_offset, _y_offset, 0, 0, 0, 0, 1, 1, 1),
					matrix_multiply(
						matrix_build(0, 0, 0, 0, 0, 0, _xscale, _yscale, 1),
						matrix_multiply(
							matrix_build(0, 0, 0, 0, 0, __post_angle, 1, 1, 1),
