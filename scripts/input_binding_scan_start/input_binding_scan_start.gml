...
function input_binding_scan_start(
	_success_method,
	_failure_method = undefined,
	_player_index = 0
) {
	...
	if (
		!(
			is_method(_success_method)
				|| (is_numeric(_success_method) && script_exists(_success_method))
		)
	) {
		__input_error(
			"Binding scan success callback set to an illegal value (typeof=",
			typeof(_success_method),
			")"
		);
	}
	...
	if (
		!(
			is_method(_failure_method)
				|| (is_numeric(_failure_method) && script_exists(_failure_method))
				|| (_failure_method == undefined)
		)
	) {
		__input_error(
			"Binding scan failure callback set to an illegal value (typeof=",
			typeof(_failure_method),
			")"
		);
	}
