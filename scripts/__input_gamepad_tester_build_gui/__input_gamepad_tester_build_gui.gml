...
function __input_gamepad_tester_build_gui() {
	...
	with (_global.__gamepad_tester_data) {
		...
		__view_pointer = dbg_view(
			"Input Gamepad Tester",
			true,
			0.5 * (window_get_width() - _width),
			0.5 * (window_get_height() - _height),
			_width,
			_height
		);
		...
		dbg_checkbox(
			_global.__gamepad_tester_data.__block_input_ref,
			"Debug blocks input"
		);
		...
		repeat (array_length(__input_ref_array)) {
			...
			repeat (13 - string_length(_label)) {
				_label += " ";
			}
			_label += " = ";
