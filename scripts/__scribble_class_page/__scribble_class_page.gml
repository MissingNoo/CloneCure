...
function __scribble_class_page() constructor {
	...
	} //FIXME - Workaround for pointers not being stringified properly on HTML5
	...
	static __submit = function(_double_draw) {
		...
		static _u_fSDFThicknessOffset = shader_get_uniform(
			__shd_scribble,
			"u_fSDFThicknessOffset"
		);
		...
		repeat (array_length(__vertex_buffer_array)) {
			...
			if (
				_data[__SCRIBBLE_VERTEX_BUFFER.__FONT_TYPE]
				== __SCRIBBLE_FONT_TYPE.__RASTER
			) {
				shader_set_uniform_f(_u_fFontType, __SCRIBBLE_FONT_TYPE.__RASTER);
				vertex_submit(
					_data[__SCRIBBLE_VERTEX_BUFFER.__VERTEX_BUFFER],
					pr_trianglelist,
					_data[__SCRIBBLE_VERTEX_BUFFER.__TEXTURE]
				);
			} else if (
				_data[__SCRIBBLE_VERTEX_BUFFER.__FONT_TYPE]
				== __SCRIBBLE_FONT_TYPE.__RASTER_WITH_EFFECTS
			) {
				...
				_data[__SCRIBBLE_VERTEX_BUFFER.__FONT_TYPE] == __SCRIBBLE_FONT_TYPE.__SDF
			) {
				//Set shader uniforms unique to the SDF shader
				shader_set_uniform_f(_u_fFontType, __SCRIBBLE_FONT_TYPE.__SDF);
				shader_set_uniform_f(
					_u_vTexel,
					_data[__SCRIBBLE_VERTEX_BUFFER.__TEXEL_WIDTH],
					_data[__SCRIBBLE_VERTEX_BUFFER.__TEXEL_HEIGHT]
				);
				shader_set_uniform_f(
					_u_fSDFRange,
					(_data[__SCRIBBLE_VERTEX_BUFFER.__SDF_RANGE] ?? 0)
				);
				shader_set_uniform_f(
					_u_fSDFThicknessOffset,
					__scribble_state.__sdf_thickness_offset
						+ (_data[__SCRIBBLE_VERTEX_BUFFER.__SDF_THICKNESS_OFFSET] ?? 0)
				);
	...
	static __freeze = function() {
		if (!__frozen) {
			...
			repeat (array_length(__vertex_buffer_array)) {
				vertex_freeze(
					__vertex_buffer_array[_i][__SCRIBBLE_VERTEX_BUFFER.__VERTEX_BUFFER]
				);
				++_i;
			}
			...
			if (SCRIBBLE_VERBOSE) {
				__scribble_trace(
					"Incrementally froze page vertex buffers, time taken = ",
					(get_timer() - _t) / 1000,
					"ms"
				);
			}
	...
	static __get_vertex_buffer = function(_texture, _fontName) {
		...
		if (_data == undefined) {
			...
			if (__SCRIBBLE_VERBOSE_GC) {
				__scribble_trace("Adding vertex buffer ", _vbuff, " to tracking");
			}
			...
			_data[@ __SCRIBBLE_VERTEX_BUFFER.__TEXEL_WIDTH] = texture_get_texel_width(
				_texture
			);
			_data[@ __SCRIBBLE_VERTEX_BUFFER.__TEXEL_HEIGHT] = texture_get_texel_height(
				_texture
			);
	...
	static __ensure_glyph_grid = function() {
		if (__glyph_grid == undefined) {
			...
			if (__SCRIBBLE_VERBOSE_GC) {
				__scribble_trace("Adding glyph grid ", __glyph_grid, " to tracking");
			}
	...
	static __finalize_vertex_buffers = function(_freeze) {
		var _i = 0;
		repeat (array_length(__vertex_buffer_array)) {
			var _vbuff = __vertex_buffer_array[_i][
				__SCRIBBLE_VERTEX_BUFFER.__VERTEX_BUFFER
			];
			vertex_end(_vbuff);
			if (_freeze) {
	...
	static __flush = function() {
		...
		repeat (array_length(__vertex_buffer_array)) {
			...
			if (_index >= 0) {
				if (__SCRIBBLE_VERBOSE_GC) {
					__scribble_trace(
						"Manually removing vertex buffer ",
						_vbuff,
						" from tracking"
					);
				}
