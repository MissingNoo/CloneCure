...
function __input_hotswap_tick() {
	...
	with (_global.__players[0]) {
		...
		if (
			_global.__game_input_allowed //Application input is allowed
			&& (
				(__last_input_time < 0)
					|| (_global.__current_time - __last_input_time > INPUT_HOTSWAP_DELAY)
			) //And enough time has passed since the last input
			&& (
				(__rebind_state <= 0)
					|| !is_array(__rebind_source_filter)
					|| (array_length(__rebind_source_filter) <= 0)
			)
		) {
			//And we're not rebinding, or the rebinding source filter is empty
			...
			if (_new_source != undefined) {
				...
				if (is_method(_global.__hotswap_callback)) {
					...
					is_numeric(_global.__hotswap_callback)
					&& script_exists(_global.__hotswap_callback)
				) {
					...
					__input_error(
						...
						typeof(_global.__hotswap_callback),
						")"
					);
				}
			}
...
function __input_hotswap_tick_input() {
	...
	if (_global.__any_gamepad_binding_defined) {
			...
			if (_global.__gamepad_connections_native[_active_gamepad_index]) {
				...
				if (is_struct(_gamepad)) {
					...
					if (
						...
						&& __input_hotswap_axis_delta(_gamepad)
					) {
						_player.__last_input_time = _global.__current_time;
						return undefined;
					}
		...
		if (
			!(
				_global.__frame - _global.__window_focus_frame
					< __INPUT_GAMEPADS_FOCUS_TIMEOUT
			) //Prevent resting axes from triggering source swap
		) {
			//Search last-to-first on platforms with low-index virtual controllers (Steam Input, ViGEm)
			static _sort_order = !INPUT_ON_WEB
			&& (
				__INPUT_ON_MACOS
					|| (!_global.__using_steamworks && __INPUT_ON_WINDOWS)
					|| (_global.__using_steamworks && __INPUT_ON_LINUX)
			)
				? -1
				: 1;
			...
			repeat (_gamepad_count) {
				if (
					(_gamepad_index != _active_gamepad_index)
					&& _global.__gamepad_connections_native[_gamepad_index]
				) {
						...
						if (_gamepad.__get_any_pressed()) {
							if (!__INPUT_SILENT) {
								__input_trace(
									...
									INPUT_GAMEPAD[_gamepad_index],
									" \"",
									_global.__gamepads[_gamepad_index].__description,
									"\" button press"
								);
							}
							_player.__last_input_time = _global.__current_time;
	...
	if (_global.__touch_allowed) {
		...
		if (INPUT_HOTSWAP_ON_MOUSE_BUTTON) {
			var _mouse_button = device_mouse_check_button_pressed(
				_global.__pointer_index,
				mb_any
			);
...
function __input_hotswap_axis_delta(_gamepad) {
	...
	if (
		(
			(
				abs(_gamepad.__get_value(gp_shoulderlb))
					> _player.__axis_threshold_get(gp_shoulderlb).__mini
			)
				&& (
					abs(_gamepad.__get_delta(gp_shoulderlb))
						> __INPUT_DELTA_HOTSWAP_THRESHOLD
				)
		)
		|| (
			(
				abs(_gamepad.__get_value(gp_shoulderrb))
					> _player.__axis_threshold_get(gp_shoulderrb).__mini
			)
				&& (
					abs(_gamepad.__get_delta(gp_shoulderrb))
						> __INPUT_DELTA_HOTSWAP_THRESHOLD
				)
		)
		|| (
			(
				abs(_gamepad.__get_value(gp_axislv))
					> _player.__axis_threshold_get(gp_axislv).__mini
			) && (abs(_gamepad.__get_delta(gp_axislv)) > __INPUT_DELTA_HOTSWAP_THRESHOLD)
		)
		|| (
			(
				abs(_gamepad.__get_value(gp_axislh))
					> _player.__axis_threshold_get(gp_axislh).__mini
			) && (abs(_gamepad.__get_delta(gp_axislh)) > __INPUT_DELTA_HOTSWAP_THRESHOLD)
		)
		|| (
			(
				abs(_gamepad.__get_value(gp_axislv))
		...
		|| (
			(
				abs(_gamepad.__get_value(gp_axisrh))
					> _player.__axis_threshold_get(gp_axisrh).__mini
			) && (abs(_gamepad.__get_delta(gp_axisrh)) > __INPUT_DELTA_HOTSWAP_THRESHOLD)
		)
		|| (
			(
				abs(_gamepad.__get_value(gp_axisrv))
					> _player.__axis_threshold_get(gp_axisrv).__mini
			) && (abs(_gamepad.__get_delta(gp_axisrv)) > __INPUT_DELTA_HOTSWAP_THRESHOLD)
		)
	) {
		return true;
	}
