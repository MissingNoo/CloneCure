...
function lexicon_index_definitions(_filePath) {
	...
	repeat (_len) {
		...
		var _fallbackLocale = (_struct[$ _languages[_i]][$ "fallbackLocale"] != undefined)
			? _struct[$ _languages[_i]].fallbackLocale
			: undefined;
		var _fallbackLanguage = (
			_struct[$ _languages[_i]][$ "fallbackLanguage"] != undefined
		)
			? _struct[$ _languages[_i]].fallbackLanguage
			: undefined;
		var _files;

		if (_struct[$ _languages[_i]][$ "files"] != undefined) {
			_files = __lexicon_definitions_parse_files(
				_filePath,
				_struct[$ _languages[_i]].files
			);
		} else if (_struct[$ _languages[_i]][$ "file"] != undefined) {
			_files = __lexicon_definitions_parse_files(
				_filePath,
				_struct[$ _languages[_i]].file
			);
		}
