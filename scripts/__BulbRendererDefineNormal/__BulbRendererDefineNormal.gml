...
function __BulbRendererDefineNormal() {
	...
	GetNormalMapSurface = function() {
		if (!normalMap) {
			__BulbError(
				"Cannot call .GetNormalMapSurface(), `normalMap` is not set to `true`"
			);
		}
		...
		if (
			(__normalSurface != undefined)
			&& (
				(surface_get_width(__normalSurface) != __surfaceWidth)
					|| (surface_get_height(__normalSurface) != __surfaceHeight)
			)
		) {
