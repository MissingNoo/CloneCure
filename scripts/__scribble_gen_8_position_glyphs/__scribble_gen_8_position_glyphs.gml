...
function __scribble_gen_8_position_glyphs() {
	...
	repeat (__pages) {
		...
		if (SCRIBBLE_PIN_ALIGNMENT_USES_PAGE_SIZE) {
			...
			var _alignment_width = (_model_max_width == infinity)
				? __width
				: _model_max_width;
			var _pin_alignment_width = (_model_max_width == infinity)
				? __width
				: _model_max_width;
		}
		...
		repeat (1 + _page_end_line - _page_start_line) {
			...
			var _line_disable_justify = _line_grid[#
				_j,
				__SCRIBBLE_GEN_LINE.__DISABLE_JUSTIFY
			];

			var _line_glyph_start = _word_grid[#
				_line_word_start,
				__SCRIBBLE_GEN_WORD.__GLYPH_START
			];
			var _line_glyph_end = _word_grid[#
				_line_word_end,
			...
			repeat (1 + _line_word_end - _line_word_start) {
				...
				if (_word_bidi != _stretch_bidi) {
					_stretch_grid[#
						_line_stretch_count,
						__SCRIBBLE_GEN_STRETCH.__WORD_START
					] = _stretch_word_start;
					_stretch_grid[#
						_line_stretch_count,
						__SCRIBBLE_GEN_STRETCH.__WORD_END
					] = _w - 1;
					_stretch_grid[# _line_stretch_count, __SCRIBBLE_GEN_STRETCH.__BIDI] =
						_stretch_bidi;
					_line_stretch_count++;
			...
			if (_w > 0) {
				...
				_stretch_grid[# _line_stretch_count, __SCRIBBLE_GEN_STRETCH.__WORD_END] =
					_w - 1;
				_stretch_grid[# _line_stretch_count, __SCRIBBLE_GEN_STRETCH.__BIDI] =
					_stretch_bidi;
				_line_stretch_count++;
			...
			if ((_line_halign == __SCRIBBLE_FA_JUSTIFY) && _line_disable_justify) {
				_line_halign = __SCRIBBLE_PIN_LEFT;
			}
			...
			if (
				...
				&& (_line_halign != fa_left)
				&& (_line_halign != __SCRIBBLE_PIN_LEFT)
			) {
				if (
					(_line_word_end >= 1)
					&& (
						_word_grid[# _line_word_end, __SCRIBBLE_GEN_WORD.__BIDI_RAW]
							== __SCRIBBLE_BIDI.WHITESPACE
					)
					&& (
						_word_grid[# _line_word_end - 1, __SCRIBBLE_GEN_WORD.__BIDI_RAW]
							!= __SCRIBBLE_BIDI.WHITESPACE
					)
				) {
					_line_adjusted_width -= _word_grid[#
						_line_word_end,
						__SCRIBBLE_GEN_WORD.__WIDTH
					];

					_word_grid[# _line_word_end, __SCRIBBLE_GEN_WORD.__WIDTH] = 0;
					var _word_glyph = _word_grid[#
						_line_word_end,
						__SCRIBBLE_GEN_WORD.__GLYPH_START
					]; //Assume that whitespace words only have one glyph
			...
			switch (_line_halign) {
				case fa_left:
					_glyph_x += (_overall_bidi == __SCRIBBLE_BIDI.R2L)
						? (_alignment_width - _line_adjusted_width)
						: 0;
					break;
				case __SCRIBBLE_PIN_LEFT:
					_glyph_x += (_overall_bidi == __SCRIBBLE_BIDI.R2L)
						? (_pin_alignment_width - _line_adjusted_width)
						: 0;
					break;
				case fa_center:
				...
				case __SCRIBBLE_FA_JUSTIFY:
					...
					if (_j != _page_end_line) {
						...
						if (_line_word_count > 1) {
							...
							var _justification_extra_spacing =
								(_pin_alignment_width - _line_adjusted_width)
								/ (_line_word_count - 1);
						}
					}
			...
			repeat (_line_stretch_count) {
				var _stretch_word_start = _stretch_grid[#
					_k,
					__SCRIBBLE_GEN_STRETCH.__WORD_START
				];
				var _stretch_word_end = _stretch_grid[#
					_k,
					__SCRIBBLE_GEN_STRETCH.__WORD_END
				];
				...
				repeat (1 + _stretch_word_end - _stretch_word_start) {
					...
					_glyph_x +=
						_word_grid[# _w, __SCRIBBLE_GEN_WORD.__WIDTH]
						+ _justification_extra_spacing;
