...
function input_check_quick_pressed_2d(
	_verb_l,
	_verb_r,
	_verb_u,
	_verb_d,
	...
	if (
		_verb_struct_l.__inactive
		|| _verb_struct_r.__inactive
		|| _verb_struct_u.__inactive
		|| _verb_struct_d.__inactive
	) {
		return false;
	}
	...
	repeat (INPUT_QUICK_BUFFER) {
		...
		if (
			(sqrt(_hx * _hx + _hy * _hy) < _min_threshold) //This historic point was within the deadzone
			|| (dot_product(_dx, _dy, _hx, _hy) < -_min_threshold)
		) {
			//Catches edge cases where the thumbstick travels across the deadzone in a single frame
