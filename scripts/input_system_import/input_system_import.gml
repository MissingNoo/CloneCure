...
function input_system_import(_string) {
	...
	if (!is_struct(_json[$ "__accessibility"])) {
		if (is_struct(_json[$ "accessibility"])) {
			...
			_json.__accessibility.__momentary_state = _json.__accessibility[$
				"momentary_state"
			];
			_json.__accessibility.__momentary_verbs = _json.__accessibility[$
				"momentary_verbs"
			];
			_json.__accessibility.__cooldown_state = _json.__accessibility[$
				"cooldown_state"
			];
			_json.__accessibility.__cooldown_verbs = _json.__accessibility[$
				"cooldown_verbs"
			];
		} else {
	...
	if (!is_struct(_json[$ "__mouse"])) {
		if (is_struct(_json[$ "mouse"])) {
			...
			input_mouse_capture_set(
				_json.__mouse[$ "capture"],
				_json.__mouse[$ "sensitivity"]
			);
		} else {
	...
	if (array_length(_players_array) != array_length(_global.__players)) {
		__input_error(
			...
			array_length(_players_array),
			" players in JSON but we are expecting ",
			array_length(_global.__players),
			" players"
		);
		return;
	}
