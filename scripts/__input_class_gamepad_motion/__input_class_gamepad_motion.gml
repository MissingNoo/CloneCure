	...
	static __tick = function() {
		...
		switch (os_type) {
			case os_switch:
				...
				__accel_x =
					_sign_x * switch_controller_acceleration(__index, _axis_x, _sensor);
				__accel_y = -switch_controller_acceleration(
					__index,
					switch_controller_axis_z,
					_sensor
				);
				__accel_z =
					_sign_z * switch_controller_acceleration(__index, _axis_z, _sensor);

				__gyro_x =
					_sign_x
					* degtorad(
						switch_controller_angular_velocity(__index, _axis_x, _sensor) / 5
					)
					* 570.6;
				__gyro_y =
					-degtorad(
				...
				__gyro_z =
					_sign_z
					* degtorad(
						switch_controller_angular_velocity(__index, _axis_z, _sensor) / 5
					)
					* 570.6;
				break;
