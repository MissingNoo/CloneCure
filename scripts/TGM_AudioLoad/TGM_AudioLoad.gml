...
function audio_load_ogg(_filePath) {
	if (file_exists(_filePath)) {
		if (string_lower(filename_ext(_filePath)) == ".ogg") {
			var audio_data = {format: "ogg", audio: audio_create_stream(_filePath)};
			return audio_data;
		} else {
			return -3;
		}
...
function audio_load_raw(_filePath, _sampleRate, _bitsPerSample, _channels) {
	if (file_exists(_filePath)) {
		...
		var _channelsMode = (_channels == 2)
			? audio_stereo
			: ((_channels == 1) ? audio_mono : audio_3d);
		...
		var _audioData = {
			...
			audio: audio_create_buffer_sound(
				_audioBuff,
				_format,
				_sampleRate,
				0,
...
function audio_load_wav(_filePath) {
	if (file_exists(_filePath)) {
		...
		// header byte offset
		var _headerLength = 44,
			_hoChunkId = 0,
			_hoChunkSize = 4,
			_hoFormat = 8,
			_hoSubchunkId1 = 12,
			_hoSubchunkId1Size = 16,
			_hoAudioFormat = 20,
			_hoChannelsNum = 22,
			_hoSampleRate = 24,
			_hoByteRate = 28,
		...
		for (var i = 0; i <= _headerLength; i++) {
			...
			if (i >= _hoAudioFormat && i < _hoChannelsNum) {
				_audioFormat += buffer_peek(_audioBuff, i, buffer_u8);
			} // 1 is PCM
		...
		if (string_lower(_chunkId) != "riff" || _audioFormat != 1) {
			return -3;
		}

		// create buffer sound
		var _format = (_bitsPerSample == 8) ? buffer_u8 : buffer_s16;
		var _channels = (_channelsNum == 2)
			? audio_stereo
			: ((_channelsNum == 1) ? audio_mono : audio_3d);
...
function audio_stream_get_format(_streamData) {
	if (is_struct(_streamData)) {
		return _streamData.audio.format;
	}
	return "";
}
