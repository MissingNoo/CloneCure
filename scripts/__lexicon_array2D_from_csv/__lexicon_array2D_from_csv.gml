...
function __lexicon_array2D_from_csv() {
	...
	var _cellDelimiter = ((argument_count > 1) && (argument[1] != undefined))
		? argument[1]
		: ",";
	var _string_delimiter = ((argument_count > 2) && (argument[2] != undefined))
		? argument[2]
		: "\"";
	...
	repeat (_size) {
		...
		if (_value == _string_delimiter_ord) {
			...
			if (_value == 0) {
				...
				var _prev_value = buffer_peek(
					_buffer,
					buffer_tell(_buffer) - 2,
					buffer_u8
				);
				if (
					(_prev_value != _cellDelimiter_ord)
					&& (_prev_value != 10)
					&& (_prev_value != 13)
				) {
					_read = true;
			...
			if (!_in_string) {
				...
				if (_read || (_value == _cellDelimiter_ord)) {
					...
					if (_string_cell) {
						if (
							(string_byte_at(_string, 1) == _string_delimiter_ord)
							&& (
								string_byte_at(_string, string_byte_length(_string))
									== _string_delimiter_ord
							)
						) {
