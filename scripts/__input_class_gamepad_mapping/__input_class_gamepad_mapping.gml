...
function __input_class_gamepad_mapping(_gm, _raw, _type, _sdl_name) constructor {
	...
	static __tick = function(_gamepad, _scan) {
		...
		} //Don't update the previous value until we get our first scanned value
		...
		if (
			...
			|| (
				__global.__allow_gamepad_tester
					&& __global.__gamepad_tester_data.__enabled
			...
			switch (__type) {
				...
				case __INPUT_MAPPING.HAT_TO_AXIS:
					__value =
						(
							(
								gamepad_hat_value(_gamepad, __raw_positive)
									& __hat_mask_positive
							) > 0
						)
						- (
							(
								gamepad_hat_value(_gamepad, __raw_negative)
									& __hat_mask_negative
							) > 0
						);
					break;
			...
			if (__limited_range) {
				__value = 2 * __value - 1;
			} //Expand 0 -> 1 range to the full -1 -> +1
			if (__extended_range) {
				__value = 0.5 + 0.5 * __value;
			} //Reduce -1 -> +1 range to 0 -> 1
			if (__reverse) {
				__value = -__value;
			}
			if (__clamp_negative) {
	...
	static __calibrate = function(_success) {
		if ((__type != __INPUT_MAPPING.BUTTON) || __input_axis_is_directional(__gm)) {
			...
			if (
				...
				|| (
					(__value_delta != 0.0)
						&& (abs(__value_delta) != 0.5)
						&& (abs(__value_delta) != 1.0)
				)
			) {
