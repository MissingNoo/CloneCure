...
global.__ldtk_config = {
	...
	mappings: {
		// if a mapping doesn't exist - ldtk name (with a prefix) is used
		levels: {
			// ldtk_level_name -> gm_room_name
		},
		layers: {
			// ldtk_layer_name -> gm_room_layer_name
			Entities: "Instances",
		},
		enums: {
			// ldtk_enum_name -> { ldtk_enum_value -> gml_value }
		},
		entities: {
			// ldtk_entity_name -> gm_object_name
		},
		fields: {
			// ldtk_entity_name -> { ldtk_entity_field_name -> gm_instance_variable_name }
		},
		tilesets: {
			// ldtk_tileset_name -> gm_tileset_name
		},
		intgrids: {
			// ldtk_intgrid_layer_name -> global.ldtk_intgrids' key
		},
	},
};
...
function LDtkLoad(level_name) {
	...
	if (is_undefined(argument[0]) || level_name == "") {
	...
	for (var i = 0; i < array_length(data.levels); ++i) {
		...
		if (level_name == "") {
			// level mapped to the current room by default
	...
	for (var i = 0; i < array_length(level.layerInstances); i++) {
		...
		var ignore_layer = gm_layer_id == -1 && layer_type != "IntGrid";
		...
		switch (layer_type) {
			...
			case "Entities": // instances
				...
				for (var e = 0; e < array_length(this_layer.entityInstances); ++e) {
						...
						switch (field_type) {
							...
							case "Array<Point>":
								for (var j = 0; j < array_length(field_value); j++) {
									field_value[@ j] = __LDtkPreparePoint(
										field_value[j],
										tile_size
									);
								}
							...
							case "EntityRef":
								...
								array_push(
									entity_refs,
									{inst, gm_field_name, ref: field_value.entityIid}
								);
							...
							default:
								if (string_pos("LocalEnum", field_type)) {
									...
									if (string_pos("Array<", field_type)) {
										...
										for (
											var j = 0;
											j < array_length(field_value);
											j++
										) {
											field_value[@ j] = __LDtkPrepareEnum(
												enum_name,
												field_value[j]
											);
										}
									} else {
										field_value = __LDtkPrepareEnum(
											enum_name,
											field_value
										);
									}
								} else {
									// everything else is just the value itself as it was parsed
					...
					__LDtkTrace(
						$"Loaded Entity! {
							object_get_name(inst.object_index)
						}, GM instance id={inst}"
					);
				}

				__LDtkTrace(
					"Loaded an Entities Layer! name=%, gm_name=%",
					_layer_name,
					gm_layer_name
				);
				break;
			...
			case "IntGrid":
				if (config.ignore_intgrids) {
					__LDtkTrace(
						"IntGrid layers are ignored because of config.ignore_intgrids."
					);
					break;
				}
			...
			case "Tiles": // tile map!
				...
				if (tilemap == -1) {
					...
					tilemap = layer_tilemap_create(
						gm_layer_id,
						this_layer.__pxTotalOffsetX,
						this_layer.__pxTotalOffsetY,
						gm_tileset_id,
						cwid,
						chei
					);
				...
				for (var t = 0; t < array_length(this_layer.gridTiles); ++t) {
					...
					var tile_src_x = this_tile.src[0], tile_src_y = this_tile.src[1];
				...
				__LDtkTrace(
					"Loaded a Tile Layer! name=%, gm_name=%",
					_layer_name,
					gm_layer_name
				);
				break;
...
function LDtkReloadFields() {
	if (!global.__ldtk_config.escape_fields) {
		__LDtkTrace(
			"Warning: LDtkReloadFields() is called, but the `escape fields` config is turned off.\Did you mean to enable the config or not call the function? (Variables are loaded automatically by default)"
		);
		return -1;
	}
...
function __LDtkDeepInheritVariables(src, dest) {
	...
	for (var i = 0; i < array_length(var_names); ++i) {
		...
		if (is_struct(var_value) && is_struct(dest[$ var_name])) {
...
function __LDtkPreparePoint(point, tile_size) {
	if (!is_struct(point) && point == pointer_null) {
