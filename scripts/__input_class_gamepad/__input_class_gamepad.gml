...
function __input_class_gamepad(_index) constructor {
		...
		if (__INPUT_SDL2_SUPPORT) {
			...
			if (__INPUT_ON_MACOS) {
				if ((__mapping != "") && (__mapping != "no mapping")) {
					if (!__INPUT_SILENT) {
						__input_trace(
							"Gamepad ",
							__index,
							" has a custom mapping, clearing GameMaker's native mapping string"
						);
					}
				...
				gamepad_test_mapping(
					__index,
					gamepad_get_guid(__index)
						+ ","
						+ gamepad_get_description(__index)
						+ ","
				);
			} else {
				if (!__INPUT_SILENT) {
					__input_trace(
						"Gamepad ",
						__index,
						(__blacklisted ? " is blacklisted" : " has a custom mapping"),
	...
	static __set_mapping = function(
		...
		if (_override) {
			...
			if (__mac_cleared_mapping && __INPUT_ON_MACOS) {
				if (_mapping_type == __INPUT_MAPPING.AXIS) {
					_raw_index += 6;
				}
				if (_mapping_type == __INPUT_MAPPING.BUTTON) {
					_raw_index += __global.__use_gp_extended ? 28 : 17;
				}
			}
		}

		//Create a new mapping. It holds button state and definition information
		var _mapping = new __input_class_gamepad_mapping(
			_gm,
			_raw_index,
			_mapping_type,
			_sdl_name
		...
		if (_raw_index != undefined) {
			__mapping_raw_to_gm[$ _raw_index] = _mapping;
		} //_raw_index can be undefined when setting up hat-on-axis
	...
	static __tick = function(_connected) {
		if (!_connected) {
			...
			if (__INPUT_ON_WINDOWS) {
				...
				if (__xinput_trigger_range < 1) {
					var _trigger_max = max(
						gamepad_axis_value(__index, __XINPUT_AXIS_LT),
						gamepad_axis_value(__index, __XINPUT_AXIS_RT)
					);
				...
				if (__stadia_trigger_test) {
					...
					var _axis_lt = 5;
					if (
						gamepad_button_check(__index, 12)
						&& (gamepad_axis_value(__index, _axis_lt) >= 0)
					) {
						if (
							(
								gamepad_axis_value(__index, 1)
									== gamepad_axis_value(__index, 2)
							)
							&& (
								gamepad_axis_value(__index, _axis_lt)
									== gamepad_axis_value(__index, _axis_lt - 1)
							)
						) {
							//Stadia default
							if (!__INPUT_SILENT) {
							...
							//Stadia analogue
							if (
								(gamepad_axis_value(__index, _axis_ry) != 0)
								&& (gamepad_axis_value(__index, _axis_ry) >= -0.5)
							) {
							...
							__set_mapping(
								gp_axisrv,
								_axis_ry,
								__INPUT_MAPPING.AXIS,
								"righty"
							);

							var _mapping = __set_mapping(
								gp_shoulderlb,
								_axis_lt,
								__INPUT_MAPPING.AXIS,
								"lefttrigger"
			...
			if (__saffun_axis_test == true) {
				...
				if (
					(floor(abs(gamepad_axis_value(__index, 2)) * 10) != _resting_value)
					|| (floor(abs(gamepad_axis_value(__index, 3)) * 10) != _resting_value)
				) {
		...
		if (_deadzone != 0.0) {
			gamepad_set_axis_deadzone(__index, 0.0);
		}
		...
		if (!_connected) {
			//Flag for removal
			if (
				__global.__frame - __disconnection_frame
				>= __INPUT_GAMEPADS_DISCONNECTION_TIMEOUT
			) {
				return false;
			}
		}
				...
				if (__INPUT_ON_SWITCH) {
					if (
						...
						switch_controller_vibrate_hd(
							__index,
							switch_controller_motor_single,
							_vibration_high,
							250,
							_vibration_low,
							160
	...
	static __swap_ab = function() {
		if (
			__input_gamepad_type_swap_ab(__simple_type)
			&& is_struct(__mapping_gm_to_raw[$ string(gp_face1)])
			&& is_struct(__mapping_gm_to_raw[$ string(gp_face2)])
		) {
			if (__INPUT_DEBUG) {
				__input_trace("  (Swapping A and B)");
			}
			var _a_mapping = __mapping_gm_to_raw[$ string(gp_face1)].__raw;
			__set_mapping(
				gp_face1,
				__mapping_gm_to_raw[$ string(gp_face2)].__raw,
				__INPUT_MAPPING.BUTTON,
				"a"
			);
	...
	static __virtual_set = function() {
		...
		if (__INPUT_ON_LINUX) {
			...
			repeat (__index + 1) {
				if (
					(gamepad_get_description(_i) == "Valve Streaming Gamepad")
					|| __input_string_contains(
						gamepad_get_guid(_i),
						"03000000de280000fc11",
						"03000000de280000ff11"
					)
				) {
		...
		if (!(_gamepad_is_virtual && is_numeric(__steam_handle) && (__steam_handle > 0))) {
			...
			__steam_handle_index = steam_input_get_gamepad_index_for_controller(
				__steam_handle
			);
	...
	static __led_set = function() {
		...
		if (
			!__INPUT_LED_PATTERN_SUPPORT
			|| (__INPUT_ON_WINDOWS && (!is_numeric(__steam_handle)))
		) {
			return;
		}
		...
		//MFi gamepad case
		if ((__raw_type == "AppleController") && __INPUT_ON_IOS) {
		...
		switch (__simple_type) {
			...
			case INPUT_GAMEPAD_TYPE_JOYCON_RIGHT:
				if (
					...
					|| (
						!INPUT_SWITCH_HORIZONTAL_HOLDTYPE
							&& (__simple_type != INPUT_GAMEPAD_TYPE_SWITCH)
					)
				) {
		...
		if (_led_layout != undefined) {
			__led_pattern = {
				...
				__pattern: __global.__gamepad_led_pattern_dict[$ _led_type][@
					__index + _led_offset
				],
	...
	static __vibration_initialize = function() {
		__vibration_support =
			__global.__vibration_allowed && (__xinput || !__INPUT_ON_WINDOWS);
		if (__vibration_support) {
			if (os_type == os_ps5) {
				ps5_gamepad_set_vibration_mode(
					__index,
					ps5_gamepad_vibration_mode_compatible
				);
			} else {
				if (
					(__INPUT_ON_WINDOWS || __INPUT_ON_SWITCH)
					&& __input_string_contains(__raw_type, "JoyCon", "SwitchHandheld")
				) {
	...
	static __trigger_effect_apply = function(_trigger, _effect, _strength) {
		...
		if (os_type == os_ps5) {
			return _effect.__apply_ps5(__index, _trigger, _strength);
		}
		if (__global.__using_steamworks) {
			return __input_steam_apply_trigger_effect(
				__steam_handle,
				_effect,
				_trigger_index,
				_strength
