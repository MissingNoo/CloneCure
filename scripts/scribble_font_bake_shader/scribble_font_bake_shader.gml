...
function scribble_font_bake_shader(
	...
	if (_src_font_data.__fontType == __SCRIBBLE_FONT_TYPE.__RASTER_WITH_EFFECTS) {
		__scribble_error(
			"Source font cannot already have been modified by scribble_font_bake_outline_and_shadow()"
		);
		return undefined;
	}
	...
	//Create a new font
	var _new_font_data = new __scribble_class_font(
		_new_font_name,
		_glyph_count,
		undefined
	);
	...
	if (_markAsRasterEffect) {
		_new_font_data.__fontType = __SCRIBBLE_FONT_TYPE.__RASTER_WITH_EFFECTS;
	}
	...
	ds_grid_multiply_region(
		...
		_glyph_count - 1,
		SCRIBBLE_GLYPH.U1,
		_sprite_u1 - _sprite_u0
	); //Note we're adjusting U0 and U1 in the same pass
	ds_grid_multiply_region(
		_new_glyphs_grid,
		0,
		SCRIBBLE_GLYPH.V0,
		_glyph_count - 1,
		SCRIBBLE_GLYPH.V1,
		_sprite_v1 - _sprite_v0
	); //Note we're adjusting V0 and V1 in the same pass
	ds_grid_add_region(
		_new_glyphs_grid,
		0,
		SCRIBBLE_GLYPH.U0,
