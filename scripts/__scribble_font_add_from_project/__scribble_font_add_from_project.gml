...
function __scribble_font_add_from_project(_font) {
	...
	if (SCRIBBLE_VERBOSE) {
		__scribble_trace("Adding \"", _name, "\" as standard font");
	}

	var _scribble_state = __scribble_initialize().__state;
	if (_scribble_state.__default_font == undefined) {
		if (SCRIBBLE_VERBOSE) {
			__scribble_trace("Setting default font to \"" + string(_name) + "\"");
		}
	...
	try {
		...
		var _ascender_offset = SCRIBBLE_USE_ASCENDER_OFFSET
			? _font_info.ascenderOffset
			: 0;
		...
		if (SCRIBBLE_VERBOSE) {
			__scribble_trace("Processing font \"" + _name + "\"");
		}
		...
		if (SCRIBBLE_VERBOSE) {
			__scribble_trace(
				"  \""
					...
					+ string(_texture_h)
					+ ", texel = "
					+ string_format(_texture_tw, 1, 10)
					+ " x "
					+ string_format(_texture_th, 1, 10)
					+ ", uvs = "
					+ string_format(_texture_uvs[0], 1, 10)
					+ ","
					+ string_format(_texture_uvs[1], 1, 10)
					+ " -> "
					+ string_format(_texture_uvs[2], 1, 10)
					+ ","
					+ string_format(_texture_uvs[3], 1, 10)
			);
		}
		...
		var _font_data = new __scribble_class_font(
			_name,
			_size,
			_sdf ? __SCRIBBLE_FONT_TYPE.__SDF : __SCRIBBLE_FONT_TYPE.__RASTER
		);
		...
		repeat (_size) {
			...
			if ((_unicode >= 0x3000) && (_unicode <= 0x303F)) {
				//CJK Symbols and Punctuation
				var _bidi = __SCRIBBLE_BIDI.SYMBOL;
			} else if ((_unicode >= 0x3040) && (_unicode <= 0x30FF)) {
				//Hiragana and Katakana
				var _bidi = __SCRIBBLE_BIDI.ISOLATED_CJK;
			} else if ((_unicode >= 0x4E00) && (_unicode <= 0x9FFF)) {
				//CJK Unified ideographs block
				var _bidi = __SCRIBBLE_BIDI.ISOLATED_CJK;
			} else if ((_unicode >= 0xFF00) && (_unicode <= 0xFF0F)) {
				//Fullwidth symbols
				var _bidi = __SCRIBBLE_BIDI.SYMBOL;
			} else if ((_unicode >= 0xFF1A) && (_unicode <= 0xFF1F)) {
				//More fullwidth symbols
				var _bidi = __SCRIBBLE_BIDI.SYMBOL;
			} else if ((_unicode >= 0xFF5B) && (_unicode <= 0xFF64)) {
				//Yet more fullwidth symbols
				var _bidi = __SCRIBBLE_BIDI.SYMBOL;
			} else {
			...
			if (SCRIBBLE_USE_KERNING) {
				...
				if (is_array(_kerning_array)) {
					...
					repeat (array_length(_kerning_array) div 2) {
						...
						if (_first > 0) {
							_font_kerning_map[?
								((_unicode & 0xFFFF) << 16) | (_first & 0xFFFF)
							] = _kerning_array[_j + 1];
						}
						_j += 2;
					}
			...
			_font_glyph_data_grid[# _i, SCRIBBLE_GLYPH.Y_OFFSET] =
				_yoffset - _ascender_offset;
			_font_glyph_data_grid[# _i, SCRIBBLE_GLYPH.WIDTH] = _w;
			_font_glyph_data_grid[# _i, SCRIBBLE_GLYPH.HEIGHT] = _h;
			_font_glyph_data_grid[# _i, SCRIBBLE_GLYPH.FONT_HEIGHT] =
				_h + _sdf_height_offset;
		...
		var _GM_scaling =
			_font_info.size
			/ _font_glyph_data_grid[# _font_glyphs_map[? 32], SCRIBBLE_GLYPH.HEIGHT];
		if (_GM_scaling > 1) {
			__scribble_trace(
				"Warning! Font \"",
				_name,
			...
			if (SCRIBBLE_ATTEMPT_FONT_SCALING_FIX) {
