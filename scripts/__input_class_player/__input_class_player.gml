...
function __input_class_player() constructor {
	...
	static __profile_destroy = function(_profile_name) {
		...
		if (__INPUT_DEBUG_PROFILES) {
			__input_trace(
				"Profile \"",
				_profile_name,
				"\" destroyed for player ",
				__index
			);
		}

		if (__profile_name == _profile_name) {
			__profile_name = undefined;
		}
	...
	static __profile_get_auto = function() {
		...
		if ((__global.__source_mode == INPUT_SOURCE_MODE.MULTIDEVICE) && (__index == 0)) {
			return INPUT_AUTO_PROFILE_FOR_MULTIDEVICE;
		}
		...
		if (_count == 1) {
			switch (__source_array[0].__source) {
				case __INPUT_SOURCE.KEYBOARD:
					return INPUT_AUTO_PROFILE_FOR_KEYBOARD;
					break;
				case __INPUT_SOURCE.MOUSE:
					return INPUT_AUTO_PROFILE_FOR_MOUSE;
					break;
				case __INPUT_SOURCE.TOUCH:
					return INPUT_AUTO_PROFILE_FOR_TOUCH;
					break;
				case __INPUT_SOURCE.GAMEPAD:
					return INPUT_AUTO_PROFILE_FOR_GAMEPAD;
					break;
	...
	static __profile_import = function(_string, _profile_name) {
		...
		repeat (array_length(__global.__basic_verb_array)) {
			...
			if (!is_array(_existing_alternate_array)) {
				...
				_existing_alternate_array = array_create(
					INPUT_MAX_ALTERNATE_BINDINGS,
					undefined
				);
			...
			if (
				...
				&& (array_length(_alternate_array) != INPUT_MAX_ALTERNATE_BINDINGS)
			) {
				__input_error(
	...
	static __sources_clear = function() {
		if ((__rebind_state > 0) && (array_length(__source_array) > 0)) {
			__binding_scan_failure(INPUT_BINDING_SCAN_EVENT.SOURCE_CHANGED);
		}
		...
		if (__INPUT_DEBUG_SOURCES) {
			__input_trace("Sources cleared for player ", __index);
		}
	};
	...
	static __source_add = function(_source) {
		...
		if (
			(_source == INPUT_TOUCH)
			|| (INPUT_MOUSE_ALLOW_VIRTUAL_BUTTONS && (_source == INPUT_MOUSE))
		) {
			__input_virtual_player_set(self);
		}

		if (__INPUT_DEBUG_SOURCES) {
			__input_trace("Assigned source ", _source, " to player ", __index);
		}
	};
	...
	static __source_remove = function(_source) {
		...
		if (__global.__touch_allowed) {
			if (_source == INPUT_MOUSE) {
				_source = INPUT_TOUCH;
			}
		} else {
			if (_source == INPUT_TOUCH) {
				_source = INPUT_MOUSE;
			}
		}
		...
		repeat (array_length(__source_array)) {
			if (__source_array[_i] == _source) {
				...
				if (__INPUT_DEBUG_SOURCES) {
					__input_trace("Removed source ", _source, " from player ", __index);
				}
			} else {
				++_i;
			}
	...
	static __source_contains = function(_source, _touch_remap = true) {
		...
		var _i = 0;
		repeat (array_length(__source_array)) {
			if (__source_array[_i].__equal_to(_source)) {
				return true;
			}
			++_i;
		}
	...
	static __sources_any_rebind_allowed_input = function() {
		...
		repeat (array_length(__source_array)) {
			if (
				__source_array[_i].__scan_for_binding(
					__index,
					true,
					__rebind_ignore_struct,
					__rebind_allow_struct
				)
			) {
				return true;
			}
	...
	static __sources_validate_binding = function(_binding) {
		var _i = 0;
		repeat (array_length(__source_array)) {
			if (__source_array[_i].__validate_binding(_binding)) {
				return true;
			}
			++_i;
		}
	...
	static __binding_get = function(_profile_name, _verb, _alternate, _allowFallback) {
		...
		if (_profile_name == undefined) {
			...
			var _keyboard_profile_allowed =
				__global.__keyboard_allowed && __global.__any_keyboard_binding_defined;
			var _mouse_profile_allowed =
				__global.__mouse_allowed && __global.__any_mouse_binding_defined;
			var _gamepad_profile_allowed =
				__global.__gamepad_allowed && __global.__any_gamepad_binding_defined;
			...
			switch (_binding_struct.__get_source_type()) {
				case __INPUT_SOURCE.KEYBOARD:
					if (
						...
						var _binding_gamepad_name = input_gamepad_constant_get_name(
							ord(input_binding_get_name(_binding_struct))
						);
						if (_binding_gamepad_name != "unknown") {
							if (__global.__strict_binding_check) {
								__input_error(
									"Gamepad constant value ",
									_binding_gamepad_name,
						...
						if (__global.__strict_binding_check) {
							__input_error(
								"Keyboard binding \"",
								input_binding_get_name(_binding_struct),
							...
							(_profile_name == INPUT_AUTO_PROFILE_FOR_MOUSE)
								|| (
									INPUT_ASSIGN_KEYBOARD_AND_MOUSE_TOGETHER
										&& (
											_profile_name
												== INPUT_AUTO_PROFILE_FOR_KEYBOARD
										)
								)
						)
					) {
				...
				case __INPUT_SOURCE.TOUCH:
					if (!(_profile_name == INPUT_AUTO_PROFILE_FOR_TOUCH)) {
						if (__global.__strict_binding_check) {
							__input_error(
								"Touchscreen binding \"",
								input_binding_get_name(_binding_struct),
				...
				case __INPUT_SOURCE.GAMEPAD:
					if (!(_profile_name == INPUT_AUTO_PROFILE_FOR_GAMEPAD)) {
						if (__global.__strict_binding_check) {
							__input_error(
								"Gamepad binding \"",
								input_binding_get_name(_binding_struct),
	...
	static __binding_reset = function(_profile_name, _verb, _alternate) {
		...
		//Verify the profile
		var _default_profile_struct = __global
			.__default_player
	...
	static __verb_group_ensure = function(_verb_group) {
		if (!variable_struct_exists(__verb_group_state_dict, _verb_group)) {
			__verb_group_state_dict[$ _verb_group] = true;
		}
	};
		...
		if (is_struct(__verb_state_dict[$ _verb_name])) {
			...
			if (__INPUT_DEBUG_VERBS) {
				__input_trace(
					...
					"\" not found on player ",
					__index,
					", creating a new one as a complex verb (type=",
					_type,
					")"
				);
			}
	...
	static __add_chord_state = function(_verb_name, _chord_defintion) {
		...
		if (is_struct(__chord_state_dict[$ _verb_name])) {
			...
			__chord_state_dict[$ _verb_name] = new __input_class_chord_state(
				_verb_name,
				_chord_defintion
			);
		}
	...
	static __axis_threshold_get = function(_axis_name) {
		...
		if (__INPUT_DEBUG_BINDING) {
			__input_trace("Warning! No axis threshold found for axis ", _axis_name);
		}

		if (__input_axis_is_directional(_axis_name)) {
			return __axis_threshold_set(
				_axis_name,
				INPUT_DEFAULT_AXIS_MIN_THRESHOLD,
				INPUT_DEFAULT_AXIS_MAX_THRESHOLD
			);
		} else {
			return __axis_threshold_set(
				_axis_name,
				INPUT_DEFAULT_TRIGGER_MIN_THRESHOLD,
				INPUT_DEFAULT_TRIGGER_MAX_THRESHOLD
			);
		}
	};
	...
	static __get_invalid_gamepad_bindings = function(_profile_name = undefined) {
		...
		repeat (array_length(__source_array)) {
			...
			if (is_struct(_profile_verb_struct)) {
				var _gamepad_mapping_array = input_gamepad_get_map(
					__source_array[_s].__gamepad
				);
					...
					repeat (INPUT_MAX_ALTERNATE_BINDINGS) {
						if (is_struct(_alternate_array[_a])) {
							...
							if (!_found) {
								array_push(
									_output,
									{verb: _verb_name, alternate: _a, gm: _verb_input}
								);
							}
						}
	...
	static __export = function(_output_string, _prettify) {
		...
		repeat (array_length(_profile_name_array)) {
			...
			_new_profiles_dict[$ _profile_name] = __profile_export(
				_profile_name_array[_f],
				false,
				false
			);
			++_f;
	...
	static __import = function(_string) {
		...
		if (variable_struct_exists(_json, "__gamepad_type_override")) {
			...
			if (
				!is_string(_gamepad_type_override)
				&& !is_undefined(_gamepad_type_override)
			) {
		...
		if (variable_struct_exists(_json, "__vibration_strength")) {
			...
			__input_trace(
				...
				INPUT_VIBRATION_DEFAULT_STRENGTH
			);
			__vibration_strength = INPUT_VIBRATION_DEFAULT_STRENGTH;
		}
		...
		if (variable_struct_exists(_json, "__trigger_effect_strength")) {
			...
			__input_trace(
				...
				INPUT_TRIGGER_EFFECT_DEFAULT_STRENGTH
			);
	...
	static __trigger_effect_set = function(_trigger, _effect, _set) {
		...
		if ((_gamepad < 0) || !is_struct(_effect)) {
			return;
		}
		...
		var _intercepted =
			__global
				.__gamepads[_gamepad]
				.__trigger_effect_apply(_trigger, _effect, __trigger_effect_strength)
			== false;
	...
	static __motion_data_get = function() {
		if (
			(__global.__source_mode == INPUT_SOURCE_MODE.MIXED)
			&& (__gyro_gamepad == undefined)
		) {
			...
			repeat (array_length(__global.__gamepads)) {
				...
				repeat (array_length(_motion_names)) {
					__mixed_motion[$ _motion_names[_name]] += _source_motion[$
						_motion_names[_name]
					];
					++_name;
				}
			...
			if (!_using_motion) {
				__mixed_motion.acceleration_y = -1.0;
			}
		...
		if (
			(__global.__source_mode != INPUT_SOURCE_MODE.MULTIDEVICE)
			|| (__gyro_gamepad == undefined)
		) {
		...
		if (
			(_gamepad_index < 0)
			|| !is_struct(__global.__gamepads[_gamepad_index].__motion)
		) {
			return undefined;
		}
	...
	static __tick_basic_verbs = function() {
		var _v = 0;
		repeat (array_length(__global.__basic_verb_array)) {
			__verb_state_dict[$ __global.__basic_verb_array[_v]].__tick(
				__verb_group_state_dict,
				__active
			);
			++_v;
	...
	static __tick_chord_verbs = function() {
		...
		repeat (array_length(__global.__chord_verb_array)) {
			...
			if (__chord_state_dict[$ _chord_name].__evaluate(__verb_state_dict)) {
				...
				__verb_state_dict[$ _chord_name].__tick(
					__verb_group_state_dict,
					__active
				);
			}
	...
	static __tick_vibration = function() {
		if (
			...
			//Don't vibrate if we're likely to have multiple gamepads assigned
			...
			__global
				.__gamepads[_gamepad_index]
				.__vibration_set(
					__vibration_strength * _left,
					__vibration_strength * _right
				);
		}
	};
	...
	static __tick_binding_scan = function() {
		...
		if (__rebind_state == 1) {
			//Waiting for the player to release all buttons
			...
			//Now grab the first button pressed
	...
	static __binding_scan_success = function(_binding) {
		...
		if (is_method(__rebind_success_callback)) {
			...
			is_numeric(__rebind_success_callback)
			&& script_exists(__rebind_success_callback)
		) {
			...
			__input_error(
				...
				typeof(__rebind_success_callback),
				")"
			);
		}
	};
	...
	static __binding_scan_failure = function(_error_code) {
		...
		if (is_method(__rebind_failure_callback)) {
			...
			is_numeric(__rebind_failure_callback)
			&& script_exists(__rebind_failure_callback)
		) {
			...
			__input_error(
				...
				typeof(__rebind_failure_callback),
				")"
			);
		}
	};
