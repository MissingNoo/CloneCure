...
function __scribble_gen_7_build_pages() {
	...
	var _simulated_model_height = _wrap_no_pages
		? infinity
		: (_model_max_height / __fit_scale);
	...
	_page_data.__glyph_start = _word_grid[#
		_line_grid[# 0, __SCRIBBLE_GEN_LINE.__WORD_START],
	...
	repeat (_line_count) {
		...
		var _starts_manual_page = _line_grid[#
			_i,
		...
		if (
			!_starts_manual_page
			&& (
				(_line_y + _line_height < _simulated_model_height)
					|| (_page_start_line >= _i)
			)
		) {
			...
			_page_data.__glyph_start = _word_grid[#
				_line_grid[# _i, __SCRIBBLE_GEN_LINE.__WORD_START],
				__SCRIBBLE_GEN_WORD.__GLYPH_START
			];
