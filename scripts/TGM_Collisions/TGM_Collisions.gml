...
function point_in_arc(_px, _py, _x, _y, _angle, _dist, _fov) {
	return point_distance(_px, _py, _x, _y) < _dist
		&& abs(angle_difference(_angle, point_direction(_x, _y, _px, _py))) < _fov / 2;
}
...
function move_and_collide_simple(_hspd, _vspd, _object) {
	...
	if (_col != noone) {
		repeat (abs(_hspd) + 1) {
			if (place_meeting(x + _vx, y, _object)) {
				break;
			}
			x += _vx;
		}
	...
	if (_col != noone) {
		repeat (abs(_vspd) + 1) {
			if (place_meeting(x, y + _vy, _object)) {
				break;
			}
			y += _vy;
		}
...
function instance_nearest_nth(_x, _y, _object, _number) {
	...
	var _px = _x, _py = _y, _inst = noone, _pri = ds_priority_create(), _i = 0;
	with (_object) {
		if (self == _object) {
			continue;
		}
