...
function input_debug_all_input(
	_ignore_array = undefined,
	_allow_array = undefined,
	_source_filter = undefined
) {
	...
	if (!is_array(_allow_array) && (_allow_array != undefined)) {
		_allow_array = [_allow_array];
	}
	if (!is_array(_ignore_array) && (_ignore_array != undefined)) {
		_ignore_array = [_ignore_array];
	}

	var _ignore_struct = undefined;
	var _allow_struct = undefined;

	if (is_array(_ignore_array)) {
		//"Ignore" array used if there's no allow array
		_ignore_struct = {};
		...
		repeat (array_length(_ignore_array)) {
			...
			if (
				is_string(_value)
				&& (_value != "mouse wheel up")
				&& (_value != "mouse wheel down")
			) {
				_value = ord(_value);
			}
	...
	if (is_array(_allow_array)) {
		//"Allow" array takes precedence
		_allow_struct = {};
	...
	if (_source_filter == undefined) {
		_source_filter = INPUT_ASSIGN_KEYBOARD_AND_MOUSE_TOGETHER
			? [INPUT_KEYBOARD]
		...
		with (_source_filter[_i]) {
			...
			if ((self == INPUT_MOUSE) && INPUT_MOUSE_ALLOW_SCANNING) {
				if (_global.__mouse_allowed && !_global.__window_focus_block_mouse) {
					...
					if (
						_filter_func(mb_middle, _ignore_struct, _allow_struct)
					...
					if (
						_filter_func(mb_right, _ignore_struct, _allow_struct)
					...
					if (
						_filter_func(mb_side1, _ignore_struct, _allow_struct)
					...
					if (
						_filter_func(mb_side2, _ignore_struct, _allow_struct)
			...
			if (
				(__source == __INPUT_SOURCE.GAMEPAD)
				&& _global.__any_gamepad_binding_defined
			) {
				...
				repeat (array_length(_check_array)) {
					...
					if (input_gamepad_is_axis(__gamepad, _check)) {
						...
						var _threshold = __input_axis_is_directional(_check)
							? INPUT_DEFAULT_AXIS_MIN_THRESHOLD
							: INPUT_DEFAULT_TRIGGER_MIN_THRESHOLD;

						if (
							(abs(_value) > _threshold)
						...
						if (
							input_gamepad_check(__gamepad, _check)
							&& _filter_func(_check, _ignore_struct, _allow_struct)
						) {
