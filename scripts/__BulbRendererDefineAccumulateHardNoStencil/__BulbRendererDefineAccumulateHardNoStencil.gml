...
function __BulbRendererDefineAccumulateHardNoStencil() {
	__AccumulateHardLights = function(
		_boundaryL,
		_boundaryT,
		_boundaryR,
		_boundaryB,
		...
		static _shdBulbHardShadows_u_vLight = shader_get_uniform(
			__shdBulbHardShadows,
			"u_vLight"
		);
		static _shdBulbHardShadows_u_fNormalCoeff = shader_get_uniform(
			__shdBulbHardShadows,
			"u_fNormalCoeff"
		);
		static _shdBulbHardShadowsSunlight_u_vLightVector = shader_get_uniform(
			__shdBulbHardShadowsSunlight,
			"u_vLightVector"
		);
		static _shdBulbHardShadowsSunlight_u_fNormalCoeff = shader_get_uniform(
			__shdBulbHardShadowsSunlight,
			"u_fNormalCoeff"
		);
		static _shdBulbLightWithNormalMap_u_vInfo = shader_get_uniform(
			__shdBulbLightWithNormalMap,
			"u_vInfo"
		);
		static _shdBulbLightWithoutNormalMap_u_fIntensity = shader_get_uniform(
			__shdBulbLightWithoutNormalMap,
			"u_fIntensity"
		);
		static _shdBulbSunlightWithNormalMap_u_vSunInfo = shader_get_uniform(
			__shdBulbSunlightWithNormalMap,
			"u_vInfo"
		);
		static _sshdBulbSunlightWithoutNormalMap_u_fIntensity = shader_get_uniform(
			__shdBulbSunlightWithoutNormalMap,
			"u_fIntensity"
		);
		...
		if (_rendererNormalMap) {
			...
			texture_set_stage(
				shader_get_sampler_index(__shdBulbLightWithNormalMap, "u_sNormalMap"),
				surface_get_texture(GetNormalMapSurface())
			);
			shader_set_uniform_f(
				shader_get_uniform(__shdBulbLightWithNormalMap, "u_vCamera"),
				_cameraCX,
				_cameraCY,
				_cameraW / 2,
				_cameraH / 2
			...
			shader_set_uniform_f(
				shader_get_uniform(__shdBulbLightWithNormalMap, "u_vCameraVector"),
				_cameraCos,
				_cameraSin
			);
			shader_set_uniform_f(
				shader_get_uniform(__shdBulbLightWithNormalMap, "u_fSpecularIntensity"),
				hdr ? BULB_SPECULAR_MAP_INTENSITY : 0
			);
		...
		if (array_length(__sunlightArray) > 0) {
			...
			shader_set_uniform_f(
				_shdBulbHardShadowsSunlight_u_fNormalCoeff,
				_normalCoeff
			);
		}
						...
						if (__IsOnScreen(_boundaryL, _boundaryT, _boundaryR, _boundaryB)) {
							if (castShadows) {
								...
								if (_rendererNormalMap && normalMap) {
									shader_set(__shdBulbLightWithNormalMap);
									shader_set_uniform_f(
										_shdBulbLightWithNormalMap_u_vInfo,
										x,
										y,
										normalMapZ,
										intensity
									...
									shader_set_uniform_f(
										_shdBulbLightWithoutNormalMap_u_fIntensity,
										intensity
									);
								}
				...
				with (_weak.ref) {
					if (visible) {
						...
						draw_sprite_ext(
							...
							_boundaryL,
							_boundaryT,
							_boundaryR - _boundaryL,
							_boundaryB - _boundaryT,
							0,
							c_black,
							0
						);
						...
						shader_set(__shdBulbHardShadowsSunlight);
						shader_set_uniform_f(
							_shdBulbHardShadowsSunlight_u_vLightVector,
							dcos(angle),
							-dsin(angle)
						);
						vertex_submit(_staticVBuffer, pr_trianglelist, -1);
						...
						if (_rendererNormalMap && normalMap) {
							shader_set(__shdBulbSunlightWithNormalMap);
							shader_set_uniform_f(
								_shdBulbSunlightWithNormalMap_u_vSunInfo,
								dcos(angle),
								-dsin(angle),
								normalMapZ,
								intensity
