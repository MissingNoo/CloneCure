...
function __input_gamepad_set_vid_pid() {
	if (INPUT_ON_WEB) {
		...
		if (__INPUT_ON_OPERAGX) {
			...
			if ((_vendor_pos > 0) && (_product_pos > 0)) {
				...
				if (_firefoxy) {
					...
					__input_trace(
						"Gamepad __description could not be parsed. Bindings for this gamepad may be incorrect (was \"",
						__description,
						"\")"
					);
				}
		...
		if (__INPUT_ON_WINDOWS) {
			...
			__xinput =
				(__index < 4)
				&& (gamepad_get_description(__index) == "XInput STANDARD GAMEPAD");
			...
			if (!__INPUT_SILENT) {
				__input_trace(
					"Warning! OS type check fell through unexpectedly (os_type = ",
					os_type,
					")"
				);
			}
