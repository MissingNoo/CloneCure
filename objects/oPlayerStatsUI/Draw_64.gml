...
stats
	.foreach(function(name, pos, data) {
		...
		if (hud_spr != -1) {
			...
			scribble(string_upper($"{name}"))
				.scale(text_scale)
				.draw(_x + line_start, _y + text_y);
			draw_sprite_ext(
				hud_spr,
				0,
				_x + icon_x_offset,
		...
		switch (name) {
			case "back":
				...
				draw_sprite_centered_ext(
					spr,
					0,
					_x + sprite_get_width(spr) / 1.6,
					_y + sprite_get_height(spr),
					4,
					4,
					0,
					c_white,
			...
			default:
				...
				if (!is_undefined(value)) {
					scribble($"[fa_right]{value >= 0 ? "+" : ""}{value}%")
						.scale(text_scale)
						.draw(_x + _w - line_end, _y + text_y);
				}
				break;
