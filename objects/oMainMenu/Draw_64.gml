...
ui.foreach(function(name, pos, data) {
	...
	if (string_contains(name, "button")) {
		...
		if (buttons[$ name].on_area) {
			scribble($"[Fnt][fa_center][fa_top]{lexicon_text(buttons[$ name].text)}")
				.scale(0.30)
				.draw(_xc - 10, _y + _h + 5);
		}
	}
	...
	switch (name) {
		case "chars":
			...
			//}
			for (
				var row = 1, layer_dist = 0, xoffset = 0, yoffset = 0, i = 0;
				...
				if (
					_x + xoffset
					> _ox + _w - sprite_get_width(sTitleAme) - (layer_dist * 5)
				) {
			...
			break;
		case "version":
			scribble("[Fnt]Version 0.7.1740637402")
		...
		case "panel_btn":
			if (array_length(bars) < 30) {
				array_push(
					...
					[
						-100,
						irandom_range(_y, _y + _h),
						irandom_range(40, 100),
						irandom_range(7, 20)
					]
				);
			}
