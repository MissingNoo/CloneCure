	...
	switch (name) {
		...
		case "u3":
			...
			switch (ups[opt].lex) {
				...
				case "Stats":
					self[$ $"u{opt}"] ??= scribble(
						lexicon_text($"{ups[opt].lex}.{ups[opt].name}.desc")
					);
					draw_sprite_centered_ext(
						ups[opt].sprite,
						0,
			...
			if (desc.get_page_count() > 1) {
				...
				if (selected) {
					var left_right =
						...
						+ mouse_wheel_down();
					desc.page(
						clamp(desc.get_page() + left_right, 0, desc.get_page_count())
					);
				}
			}
			break;
		case "Eliminate":
			if (shop_level("Eliminate") == 0) {
				break;
			}
